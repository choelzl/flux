"""ONNX -> Flux Workload IR frontend (docs/04.md §2 L1, docs/05.md Phase 1).
...
what wasn't supported, rather than silently skipping nodes and producing an incomplete workload.
...
def onnx_model_to_workload_ir(model: onnx.ModelProto, workload_id: str) -> dict[str, Any]:
    ...
    if len(graph.input) != 1:
        raise NotExpressibleError(
            f"graph {graph.name!r} has {len(graph.input)} inputs; this frontend only handles a "
            "single-input MLP."
        )
    if not graph.node:
        raise NotExpressibleError(f"graph {graph.name!r} has no nodes.")

    batch_size, current_size = _static_2d_shape(graph.input[0], context=f"graph input {graph.input[0].name!r}")
    ...
    current_name = graph.input[0].name
    ...
    for i, node in enumerate(graph.node):
        ...
        current_name = node.output[0] if node.output else current_name

    return {
        "schema_version": "0.1.0",
        "id": workload_id,
        "provenance": {"source": "onnx", "importer": "flux-onnx@0.1"},
        "ops": ops,
    }
