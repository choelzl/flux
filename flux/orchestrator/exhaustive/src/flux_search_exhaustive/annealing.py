"""Simulated annealing over the flat-mapping space, folded into the exhaustive package (D434): the
same `Strategy` shape over the same candidates, classical serial-chain annealing instead of
enumeration.

Simulated annealing (docs/search.md) over the same flat-mapping search space
...
docs/architecture.md's "explicit seeds everywhere" determinism principle is easier to keep honest with a
single chain than with several run in parallel and merged.

**Real wall-clock budget as a genuine search-loop stopping criterion** (docs/gap-analysis.md G16,
docs/decisions.md D69): `run_simulated_annealing`'s own optional `wall_clock_budget_s` makes real,
measured elapsed time — not a fabricated dollar cost, this repo has none to fabricate against —
an actual, enforced third `done()` condition alongside `max_iterations`/`min_temperature`, checked
before every real evaluator call, not just reported after the fact once the search already ran to
completion.
"""
...
from .engine import (  # noqa: F401  the shared engine (D429)
    EvaluatedCandidate, EvaluatorProtocol, SearchState, classify_outcome, drive_propose_observe_loop)
...
class SimulatedAnnealingMappingStrategy:
    """docs/search.md's `Strategy` Protocol via simulated annealing. `propose(state, k)` requires
    ...
    def observe(self, results: list[Result | Exception]) -> None:
        ...
        result, error = classify_outcome(outcome, self._metric)
        if result is None:
            # a refusal (raised, or a Result without the metric): recorded, temperature still
            # cools, the current state does not move (D168; the rule lives in the engine, D434)
            self.evaluated.append(EvaluatedCandidate(candidate=proposed, result=None, error=error))
            self._temperature *= self._cooling_rate
            return

        self.evaluated.append(EvaluatedCandidate(candidate=proposed, result=result, error=None))
        value = result.value_of(self._metric)
        self._maybe_accept(proposed, value)
        self._maybe_record_best(proposed, value, result)
        self._temperature *= self._cooling_rate
    ...
    def done_reason(self) -> str | None:
        """Which of `done()`'s own two conditions actually triggered, if either has —
        `None` while still running. A real, public accessor (not private-attribute reach-through)
        so a driver loop can report *why* a search stopped, not just *that* it did
...
_EvaluatorProtocol = EvaluatorProtocol       # the shared engine's real Protocol (D429)
...
def run_simulated_annealing(
    ...
    *,
    ...
    `wall_clock_budget_s` (docs/decisions.md D69, docs/gap-analysis.md G16) is a real, enforced
    third stopping condition, checked *before* every real evaluator call — not merely reported
    after the fact — using real, measured wall-clock time (`time.perf_counter()`), the only
    genuinely real cost signal this repo has (no paid API ever runs here, so a dollar-cost budget
    would have nothing real to check against). The returned report's own `stopped_reason` names
    which of the three conditions actually ended the search (`"wall_clock_budget"`,
    `"max_iterations"`, or `"min_temperature"`) and `wall_clock_s` records real total elapsed
    time regardless of which one did — both real, measured facts about *this* run, not estimates.
    """
    ...
    budget = Budget(wall_clock_s=wall_clock_budget_s)

    stopped_early, wall_clock_s = drive_propose_observe_loop(
        strategy, state, evaluator, metric, budget=budget,
        wall_clock_budget_s=wall_clock_budget_s)
    stopped_reason = ("wall_clock_budget" if stopped_early
                      else strategy.done_reason() or "max_iterations")
