...
through the fidelity chain (docs/calibration.md) for confidence — e.g. coarse-grain SystemC, then
cycle-accurate RTL — rather than trusting the screening ranking alone. This is the
analytic→...→RTL evaluator cascade docs/roadmap.md Phase 4 calls for, minus the synthesis stage
(`evaluator/hammer/` is still blocked on tooling — see its README).
...
topology DSE (`interfaces/chia_nodes.flux_search`'s `search_kind` picks which generator builds the
...
layering: search doesn't know about CHIA, interfaces/ adapts search onto it).
...
naturally sequential stage-by-stage cascade through increasingly expensive real simulators (coarse
SystemC, then cycle-accurate RTL) — exactly where a real wall-clock budget matters most in
practice, and exactly where a real per-stage checkpoint already exists to check it at.
"""
...
from flux_frontier import overlaps
...
class EscalationStep:
    ...
    result: Result | None
    error: str | None
    # Which candidate this stage evaluated. `None` means "the screening winner" — the only
    ...
    candidate: Any = None
    # Position of this stage in `escalation_evaluators`. Recorded rather than inferred: stage names
    # may legitimately repeat (two SystemC configs, or `["rtl", "rtl"]`), and D112's attempt to
    # recover position from append order was wrong whenever a stage failed to measure some
...
class ArchitectureDSEReport:
    ...
    def to_dict(self) -> dict[str, Any]:
        """JSON-safe serialization — the shape `interfaces/mcp/`'s `flux_search` tool returns to an
        ...
        return {
            ...
            # flipped it, and could only recover the flip by reimplementing the stage grouping.
            "escalated_winner": (
    ...
    def escalated_winner(self, *, minimize: bool = True) -> tuple[Any, Result] | None:
        ...
        used or no stage succeeded.

        Uses the deepest stage that measured more than one contender, so candidates are always
        ...
        """
        # Group by recorded `stage_index`, never by name or by inferred position: stage names
        # legitimately repeat (two SystemC configs, `["rtl", "rtl"]`), and inferring position from
        # append order breaks as soon as a stage refuses a candidate — both routes end in comparing
        ...
        for step in self.escalation:
            ...
            if self.metric not in step.result.metrics:
                continue  # a stage that didn't produce the metric can't rank anything (D112)
            by_index.setdefault(step.stage_index, []).append(step)
        by_stage = [(steps[0].stage, steps) for _, steps in sorted(by_index.items())]

        # Only a stage that successfully measured EVERY contender yields a fair comparison. A stage
        # truncated by `wall_clock_budget_s` (or one that raised for some candidates) is a subset:
        # picking a winner from it while discarding a *complete* shallower stage silently returned
        # a worse design and reported `escalation_changed_the_winner=True` (D112). The docstring's
        # "a cutoff leaves a comparable set" claim only holds between stages, not inside one.
        widest = max((len(steps) for _, steps in by_stage), default=0)
        comparable = [steps for _, steps in by_stage if len(steps) > 1 and len(steps) == widest]
        if not comparable:
            return None
        deepest = comparable[-1]  # stage-major append order: the last complete stage is the deepest
    ...
    def escalation_agrees_with_screening(self, *, tolerance: float = 0.0) -> bool | None:
        ...
        against; otherwise whether the deepest such stage's value is within `tolerance` (absolute)
        ...
        *winner's* screening estimate answers nothing. Measured: a winner escalated to exactly its
        ...
        successful = [
            ...
            # A stage that didn't produce the metric can't confirm anything about it, and indexing
...
def contenders(
    ...
    *Pareto-front-relevance* escalation trigger docs/calibration.md has named since D8 and D99
    ...
    become the winner no matter what a slower stage measures, so buying it is wasted budget;
    ...
    for p in scored:
        ...
        # Closed-interval overlap with the leader's band: the one rule in flux_frontier (D439).
        if overlaps((est.ci_low, est.ci_high), (best_est.ci_low, best_est.ci_high)):
            out.append(p)
    return out
...
def run_architecture_dse(
    ...
    *,
    ...
    `(stage_name, evaluator)` in `escalation_evaluators`, in order. A candidate (or escalation
    stage) the evaluator refuses is recorded as a failure, not a crash — same "fail loudly per
    ...
    **escalation cascade only** — checked against real, measured elapsed time before each stage's
    ...
    way; only the *escalation* stages after the cutoff are missing.
    ...
    # into (evaluator/abi/protocol.py), and it never states the length invariant that `zip` here
    ...
    # D112 named this; fixed here): reachable without a stub — `evaluator/rtl` returns an empty
    ...
    from flux_search_exhaustive.engine import classify_outcome   # the one refusal rule (D434)

    swept = [
        p if p.result is None or classify_outcome(p.result, metric)[0] is not None
        else SweepPoint(candidate=p.candidate, result=None,
                        error=classify_outcome(p.result, metric)[1])
        for p in swept
    ]
    ...
    if winner is not None:
        ...
        # Stage-major order: every contender is measured at stage 1 before any reaches stage 2, so a
        ...
        for stage_index, (stage_name, stage_evaluator) in enumerate(escalation_evaluators):
            ...
            for target_candidate, abi_candidate in targets:
                if wall_clock_budget_s is not None and time.perf_counter() - start_time >= wall_clock_budget_s:
                    ...
                    break
                try:
                    stage_result = stage_evaluator.evaluate(abi_candidate, budget, frozenset({metric}))
                    escalation.append(EscalationStep(
                        stage=stage_name, result=stage_result, error=None, candidate=target_candidate,
                        stage_index=stage_index,
                    ))
                except Exception as exc:  # noqa: BLE001 - recorded, not fatal
                    escalation.append(EscalationStep(
                        stage=stage_name, result=None, error=str(exc), candidate=target_candidate,
                        stage_index=stage_index,
                    ))
