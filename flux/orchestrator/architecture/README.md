...
evaluator, ranks by a metric, then escalates the winner through the fidelity chain (docs/calibration.md)
...
- **NoC topology/dimensionality** (`noc_candidates.py`) — comparing e.g. a 2D mesh against a 3D
  mesh at equal node count, screened by real Booksim2 (`evaluator/booksim`). Genuinely
...
L5/L6 layering). `interfaces/chia_nodes.flux_search` is the CHIA-connected surface: one node, a
...
translated by `evaluator/stream` into real Stream `intra_core_tiling` (D103). It is what made
...
- `tests/integration/test_architecture_dse_live.py` — real ZigZag screening across widths
  ...
  stages agree on the winner; only the analytic screening's absolute number is off, a known
...
D25) — restricted to `mesh` after finding a real, separate `evaluator/booksim` `torus`-routing
