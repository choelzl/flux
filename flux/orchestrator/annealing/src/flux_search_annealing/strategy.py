...
class SimulatedAnnealingMappingStrategy:
    ...
    def observe(self, results: list[Result | Exception]) -> None:
        ...
        # A Result lacking the searched metric is a per-candidate failure, not a crash: evaluators
        # may legally omit the metric asked for (`evaluators/rtl` returns an empty metrics dict for
        # anything but `latency_cycles`), and indexing it killed the whole search. Treated as a
        # refusal — recorded, temperature still cools, current state doesn't move (D168).
        refusal = outcome.refusal_for(self._metric)
        if refusal is not None:
            self.evaluated.append(EvaluatedCandidate(candidate=proposed, result=None, error=refusal))
            self._temperature *= self._cooling_rate
            return
    ...
    *,
    ...
    while not strategy.done():
        if wall_clock_budget_s is not None and time.perf_counter() - start_time >= wall_clock_budget_s:
            stopped_reason = "wall_clock_budget"
            break
    ...
    if stopped_reason is None:
        stopped_reason = strategy.done_reason() or "max_iterations"
