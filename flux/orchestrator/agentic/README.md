...
this package's own history**: `evaluator/booksim`'s translator accepted `topology="torus"` as
...
`evaluator/booksim/README.md` for the bug/fix write-up.
...
`interfaces/chia_nodes/src/flux_chia_nodes/agentic.py`, exposed as MCP tools by `interfaces/mcp/`'s
...
`flux_agentic_dse_loop` (`interfaces/chia_nodes/src/flux_chia_nodes/dse_loop.py`,
...
faked, see `docs/decisions.md` D22). `mapping`: `evaluator/timeloop`'s translator genuinely
...
spatial-split on the two dims `evaluator/timeloop`'s fixed architecture boilerplate can express
