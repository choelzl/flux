...
from .strategies import GridStrategy, Proposal, candidate_key, strategy_spec
...
def _classify(result: Any, objective: Objective, *, phase: str = "screen") -> tuple[str, str | None]:
    ...
    escalation trial must carry at least ONE objective metric — stages are partial by design
    (`rtl` refines latency, `openroad` supplies area; neither serves the other's metric), and
    refusing a stage for the metrics it never claimed would make multi-fidelity composition
    ...
    if phase == "screen":
        ...
        if not present:
            return "refused", (
                f"stage produced none of the objective metrics {sorted(objective.metric_names())}"
            )
    for constraint in objective.metric_constraints:
        if result.refusal_for(constraint.metric) is not None:
            continue  # deferred or stage-absent: checked where the metric exists
...
def frontier_payload(trials: list[Any], objective: Objective) -> list[dict[str, Any]]:
    out = []
    for t in pareto_frontier(trials, objective):
        ...
        out.append({
            ...
            "fidelity": t.stage if t.stage else "screen",
...
def run_campaign_steps(
    ...
    *,
    ...
    elif strategy_spec(objective.strategy_kind) is not None:
        # A strategy an application registered by name (D428): the interconnect
        # application's generative strategy, or anything registered since.
        spec = strategy_spec(objective.strategy_kind)
        if spec.needs_llm and make_llm is None:
            raise CampaignError(
                f"a {objective.strategy_kind} campaign needs make_llm — same reason as "
                "agentic: the campaign package is LLM-client-agnostic"
            )
        # Store-wide or per-campaign visited keys is the strategy's call (D300).
        visited = (store.visited_keys_all() if spec.store_wide_visited
                   else store.visited_keys(campaign_id))
        strategy = spec.factory(
            objective, base_arch, visited,
            make_llm(objective.llm_model) if spec.needs_llm else None,
            history=_screen_history(store, campaign_id, objective),
            knowledge=knowledge,
        )
...
def _run_escalation(
    ...
    """Stage-major cascade over the contender set, exactly `run_architecture_dse`'s shape (D105):
    every contender through stage 0, then every contender through stage 1. Recorded by stage_index,
    never name (two stages may share a backend). Idempotent on resume: an already-escalated
    (candidate, stage_index) pair is skipped, so a second call buys nothing new."""
    ran = 0
    # WAVES (docs/decisions.md D265): a stage can ELIMINATE a contender by constraint — the
...
def _escalate_wave(
    ...
    ran = 0
    for stage_index, stage_name in enumerate(objective.escalation_backends):
        # Stop-targets naming deferred metrics can only be judged here, against the composite
        # (docs/decisions.md D226's residue, closed): checked BEFORE each stage, so a target the
        # previous stage already satisfied saves every deeper stage's spend.
        if objective.stop.target:
            composites = _build_composites(store, campaign_id, objective)
            if composites and _stop_target_met(composites, objective):
        ...
        for target in contenders:
            ...
            if store.remaining(campaign_id, objective.budget).exhausted:
                stop = True
                break
            if store.already_escalated(campaign_id, target.candidate_key, stage_index):
                continue
            if stage_evaluator is None:
                stage_evaluator = make_evaluator(stage_name)
                if objective.search["kind"].startswith("composition_"):
                    ...
                    from .composed import ComposedEvaluator, MemoryLevelAreaStage

                    if stage_name == "cacti":
                        # cacti characterizes ONE macro and its energy is per-access (D252):
                        # extraction + area-only narrowing, or the stage cannot run at all /
                        # would corrupt the composite's energy — see MemoryLevelAreaStage.
                        ...
                        stage_evaluator = MemoryLevelAreaStage(
                            stage_evaluator, level=objective.search["level"],
                            scale_from_nm=objective.search.get("cacti_scale_from_nm"))
                    stage_evaluator = ComposedEvaluator(
                        _ComponentCache(stage_evaluator, store.results,
                                        evaluator_prefix=stage_name))
            ...
            seq = store.begin_trial(
                ...
                deterministic=True, stage=stage_name, stage_index=stage_index,
            )
            batch.append((seq, arch))
            ran += 1
        _run_stage_batch(store, campaign_id, objective, stage_evaluator, workload, batch,
                        escalation_parallelism)
        if stop:
            return ran
    return ran
...
def _evaluate_one_stage(stage_evaluator, objective, workload, arch):
    ...
    t0 = time.perf_counter()
    try:
        result = stage_evaluator.evaluate(
...
def _run_stage_batch(store, campaign_id, objective, stage_evaluator, workload, batch,
                    parallelism: int) -> int:
    """Measure a stage's batch, concurrently when asked, and record each outcome as it lands.

    Why this is worth doing where screening's batching was not enough: the tools at a measured
    stage — OpenROAD, Yosys, Verilator — are single-threaded processes, so one escalation pins one
    ...
    if parallelism <= 1 or len(batch) == 1:
        for seq, arch in batch:
            result, error, secs = _evaluate_one_stage(stage_evaluator, objective, workload, arch)
            _record_stage_outcome(store, campaign_id, objective, seq, result, error, secs)
        return 0

    from concurrent.futures import ThreadPoolExecutor

    with ThreadPoolExecutor(max_workers=min(parallelism, len(batch))) as pool:
        futures = [pool.submit(_evaluate_one_stage, stage_evaluator, objective, workload, arch)
...
def _record_stage_outcome(store, campaign_id, objective, seq, result, error, secs) -> None:
    if error is not None or result is None:
        store.complete_trial(campaign_id, seq, status="error", result=None,
                             error=error or "stage evaluation returned no result",
                             wall_clock_s=secs)
        return
...
class _CompositeMeasurement:
    """Per-candidate estimates assembled from the deepest stage that measured each metric across
...
def composite_frontier(store, campaign_id, objective) -> list[dict[str, Any]]:
    ...
    the DEEPEST stage whose ok trials cover every contender on that metric (screening counts as
    the shallowest source, for screen-measured metrics only). A stage truncated by budget covers a
...
def _build_composites(store, campaign_id, objective) -> list["_CompositeMeasurement"]:
    ...
    # A contender the stage RESOLVED negatively — it ran and violated a constraint, e.g. a
    # fabric that placed at 483 MHz against a 600 MHz floor — is decided, not missing. Left in
    # the coverage set it would block the composite frontier forever, since no stage can ever
    ...
    for t in escalated:
        by_stage.setdefault(t.stage_index, {})[t.candidate_key] = t
    ...
    for om in objective.metrics:
        ...
        for idx in sorted(by_stage):
            stage_trials = by_stage[idx]
            covers = all(
                key in stage_trials and stage_trials[key].result.refusal_for(om.metric) is None
                for key in contender_keys
            )
            if covers:
                chosen = (stage_trials[next(iter(contender_keys))].stage or f"stage{idx}", stage_trials)
        if chosen is None:
...
def _report(store, campaign_id, *, objective, trials_run) -> CampaignStepReport:
    ...
    return CampaignStepReport(
        ...
        frontier=frontier_payload(screen_ok, objective.screened_view()),
        escalated_frontier=composite_frontier(store, campaign_id, objective),
