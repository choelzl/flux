...
def _point_key(result: Any, objectives: Sequence[ObjectiveMetric]) -> tuple[float, ...]:
    """The oriented (minimise-form) point values, one per objective."""
    return tuple(_oriented(result, om)[1] for om in objectives)


def point_dominates(a: Any, b: Any, objectives: Sequence[ObjectiveMetric]) -> bool:
    """Classic weak dominance with strict improvement, on point values only -- the one
    rule in `flux_frontier` (D429), over the oriented values."""
    from flux_frontier import dominates

    return dominates(_point_key(a, objectives), _point_key(b, objectives))
...
def interval_dominates(a: Any, b: Any, objectives: Sequence[ObjectiveMetric]) -> bool:
    ...
    on none -- the one rule in `flux_frontier` (D439), over the oriented triples."""
    from flux_frontier import interval_dominates as _rule

    return _rule([_oriented(a, om) for om in objectives], [_oriented(b, om) for om in objectives])
...
def pareto_frontier(trials: Sequence[Any], objective: Objective) -> list[Any]:
    ...
    return pareto(list(trials), key=lambda t: _point_key(_as_result(t), objective.metrics))
