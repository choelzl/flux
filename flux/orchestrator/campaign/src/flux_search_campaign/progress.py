...
def campaign_tally(db: str | Path) -> dict[str, int]:
    ...
    try:
        with CampaignStore(db) as store:      # the store answers; no SQL here (D442)
            return {
                "attempted": store.candidate_count(),
                "screened": store.candidate_count(phase="screen"),
                "measured": store.candidate_count(phase="escalate", status="ok"),
                "refused": store.candidate_count(status="constraint_violated"),
                "failed": store.candidate_count(status="error"),
                "proposed": store.candidate_count(strategy_kind_like="%generative%"),
            }
    except Exception:  # noqa: BLE001 -- an unreadable store tallies nothing
        return dict(_EMPTY)
