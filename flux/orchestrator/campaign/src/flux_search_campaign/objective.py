...
class MetricConstraint:
    ...
    # deeper stage can produce — "the fabric must clock at 600 MHz", which needs real
...
def parse_objective(doc: dict[str, Any]) -> Objective:
    ...
    if kind == "architecture_width":
        ...
        if www is not None and (not isinstance(www, int) or www < 1):
            raise InvalidObjectiveError(
                ...
                "(required for a cacti area stage, D252)"
            )
    ...
    # A registered strategy says which search kinds it pairs with (D442; the interconnect
    # strategy proposes fabrics for a stated client/bank/width problem, D269, so it declares
    # interconnect_topology and nothing else). The rule is the strategy's, read here.
    from .strategies import strategy_spec

    spec = strategy_spec(strategy["kind"])
    if spec is not None and spec.search_kinds is not None and kind not in spec.search_kinds:
        raise InvalidObjectiveError(
            f"strategy.kind={strategy['kind']!r} with search.kind={kind!r} — it pairs with "
            f"search.kind in {sorted(spec.search_kinds)} and only those"
        )
