...
`evaluator/rtl` at all — that adapter refuses workloads with more than one einsum op by
...
class MemoryLevelAreaStage:
    """Adapts a single-macro memory characterizer (CACTI) into a composition stage
    ...
    2. **Metric narrowing to `area_mm2`**: CACTI's energy_pj is PER-ACCESS energy and its
       ...
       cacti's per-access energy over zigzag's workload energy (deeper stage wins per metric)
       — a silently wrong frontier, the exact class this repo's adapters refuse to produce.
       This stage answers the one question it is mounted for: the searched level's silicon
...
class ComposedEvaluator:
    ...
    def __init__(self, inner: Any, *, calibration_db_path: str | None = None) -> None:
        ...
        # share engines, and one evaluator instance serves a whole stage sweep, so an identical
        # (slice, engine, metrics) request is answered once per instance. This is what the
        # store-backed CachingEvaluator cannot do at a partial stage: its covering rule demands
        # every REQUESTED metric in the stored row, and a stage result that legally omits a
        # metric (openroad stores area+power, the stage request names latency too) never
    ...
    def evaluate(
        ...
        escalation = Escalation(
            ...
            next_stage=recommending[0].escalation.next_stage if recommending else None,
