...
`exhaustive/annealing.py` (in `flux-search-exhaustive`, D434): the same `Strategy` Protocol via
classical serial-chain simulated annealing over the exact same flat-mapping representation.
Validated against exhaustive search's *proven* true optimum
...
workload, screens with a fast evaluator, escalates the winner through the fidelity chain. See
...
set, and escalation stages compose mixed-fidelity frontiers. Three strategies: `grid`, `agentic`,
...
Not implemented: every other strategy this doc names (CP/MIP, gradient, Bayesian,
evolutionary); they have no directory until one is real (D428 removed the empty placeholders).
