"""The frontier package (D362, D365; one package for dominance, the frontier and the decision
since D429/D439): two-objective bookkeeping, N-objective dominance, the UCT tree policy, and
the decision arithmetic (`decide`).

A frontier is the set of points no other point beats on both axes. `spread` picks which of
them to confirm on an expensive stage; `best_within` applies a budget on the cost axis. Written
...
from .decide import cheapest_meeting, corner, knee_ranked, normalizer  # noqa: F401  (D439)
from .dominance import dominates, interval_dominates, overlaps, pareto  # noqa: F401  (D429/D439)
