...
Stream (multi-core, `evaluator/stream`) both report `latency_cycles` for the exact same
...
def test_real_stream_multicore_result_ranks_ahead_of_real_zigzag_single_core(populated_store):
    ...
    mlp-gemm0.yaml's one GEMM across two real compute cores (Stream, 908.0 cycles under the
    current nixchip toolchain; 1148.0 before the D361 pin bump rebuilt Stream -- reproduced
    bit-identically twice before this pin moved, D374) genuinely beats the single-core
    baseline (ZigZag, 1554.0 cycles) — not assumed, ranked.
    """
    standings = rank_results_for_entry(
        populated_store, _entry("mlp-gemm0-simple-npu-1d-dual-core-v1"), repo_root=FLUX_ROOT,
    )
    assert len(standings) >= 2
    assert standings[0].value == pytest.approx(908.0)
