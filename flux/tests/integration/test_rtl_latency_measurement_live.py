...
def test_a_masked_ragged_k_group_computes_the_right_matrix_at_its_predicted_latency(B, C, K, lanes):
    """`K % lanes != 0` is the case `evaluator/rtl` refuses outright, so these candidates have no
...
def test_the_ragged_case_is_one_the_reference_evaluator_genuinely_refuses():
    ...
    than asserted: `evaluator/rtl` must actually reject the same candidate the generated design
...
def test_a_generated_design_measures_a_candidate_the_reference_cannot_express():
    ...
    `lanes=12` against `K=32` is a candidate `evaluator/rtl` refuses outright, so it has no RTL
