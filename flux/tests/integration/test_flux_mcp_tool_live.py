...
Requires the real `chia` package (see `interfaces/chia_nodes/README.md` for the submodule gotcha),
...
def test_tool_registers_every_flux_node(flux_tool):
    ...
    one kept a hand-written list as an independent check -- until D470, below.
    """
    ...
    # Against the registry itself, not a hand-written list. The list this test carried
    # rotted twice: four tools short at D119, and again once D452 made `setup()` one loop
    # over `FluxTool.tools()` (nlu_dse_loop, recall_facts, mine_knowledge,
    # author_design_spec, champsim_run, ...) -- the nightly was red on it for a month (D470).
    # What THIS test is for is the wire: every registered tool reaches a real `list_tools`
    # reply, under the prefixed name. Node <-> tool parity, the check a hand list gave
    # independently, lives in `tests/unit/test_mcp_surface_parity.py` on every push.
    expected = {f"{flux_tool.name}_{suffix}" for suffix in FluxTool.tools()}
    assert expected, "the registry names no tools"
    assert names == expected, (f"missing on the wire: {sorted(expected - names)}; "
                               f"extra on the wire: {sorted(names - expected)}")
...
def test_evaluate_tool_call_runs_the_real_thermal_backend_over_mcp(flux_tool):
    """`evaluator/thermal` (docs/decisions.md D64/D65) reached over the real MCP wire for the
...
def test_evaluate_tool_call_runs_the_real_chiplet_d2d_interconnect_over_mcp(flux_tool):
    """`evaluator/booksim`'s real chiplet inter-die (D2D) `anynet` path (docs/decisions.md
    D66/D67) reached over the real MCP wire for the first time — same real gap this closes for
    `evaluator/thermal` above, and the same reasoning: no dedicated tool (the plain KNCube NoC
