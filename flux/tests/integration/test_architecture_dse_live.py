...
synthesis stage (still blocked on tooling, see evaluator/hammer/README.md).

The interesting, honest result (found by actually running this, not assumed): ZigZag's screening
picks the right *winner* (width=16 is fastest by every stage), but its absolute number doesn't
...
the analytic screening, is the actual point of a coarse-grain pre-check stage.
"""
...
def test_escalation_ran_both_stages_on_only_the_winner(report):
    assert [step.stage for step in report.escalation] == ["systemc", "rtl"]
...
def test_systemc_and_rtl_agree_with_each_other_exactly(report):
    """The actual point of the coarse-grain stage: independent confirmation from a completely
...
def test_zigzag_screening_does_not_match_real_hardware_a_known_gap_not_a_bug(report):
    ...
    correct (width=16 wins by every stage); only the *absolute* screening number is miscalibrated
...
def test_a_real_wall_clock_budget_stops_the_escalation_cascade_partway_through():
    ...
    are each real, slow enough (several real seconds) that a budget between the two real stages'
    ...
    # ~21s for this exact sweep; SystemC alone (the first stage) is the dominant real cost of the
    ...
    # THE CLAIM: the budget cuts the cascade off before the second, more expensive stage. Which
    # stages fit inside 12 real seconds is a property of the machine, not of the loop — this
    ...
    # the declared order.
    ran = [step.stage for step in budgeted.escalation]
    assert "rtl" not in ran, f"the budget did not cut off before the second stage: {ran}"
    assert ran == ["systemc", "rtl"][:len(ran)], f"stages ran out of order: {ran}"
