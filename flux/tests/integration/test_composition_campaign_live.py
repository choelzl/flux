...
that measures a multi-op workload through evaluator/rtl at all (that adapter refuses chains
...
def test_the_true_ten_class_head_is_a_typed_refusal_at_width_16(workload, base_arch):
    ...
    evaluator/rtl's own typed NotExpressibleError on the sliced last layer, not a crash three
...
def test_a_composition_campaign_escalates_the_chain_through_real_rtl(base_arch, tmp_path):
    ...
    assignments, and the best one is escalated through evaluator/rtl — three real Verilator
