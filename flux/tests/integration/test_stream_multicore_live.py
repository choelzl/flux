...
def test_real_dual_core_architecture_runs_through_real_stream(evaluator):
    ...
    assert result.metrics["latency_cycles"].value == pytest.approx(1148.0)
    ...
    assert util["transfer_bound_cycles"] == pytest.approx(320.0)
...
def test_real_layer_fusion_changes_the_real_latency(evaluator):
    ...
    Fusion is a real *pipelining* win here: tiling the shared row dim lets the second layer start
    on tile 0's output while the first layer works on tile 1, instead of running the whole chain
    layer-by-layer.
    """
    ...
    assert unfused.metrics["latency_cycles"].value == pytest.approx(1080.0)

    fused = evaluator.evaluate(
        Candidate(workload=workload, arch=arch, mapping=_fusion_mapping(1)), Budget(), frozenset()
    )
    assert fused.metrics["latency_cycles"].value == pytest.approx(976.0)
...
def test_a_full_size_tile_reproduces_the_unfused_latency_exactly(evaluator):
    ...
    assert full_tile.metrics["latency_cycles"].value == pytest.approx(1080.0)
...
def test_the_fusion_tile_space_is_really_non_monotone(evaluator):
    ...
    assert unfused == pytest.approx(3768.0)
    assert latency(16) == pytest.approx(3768.0)  # full tile == unfused, exactly
    assert latency(8) == pytest.approx(3568.0)   # the interior optimum
    assert latency(2) == pytest.approx(3824.0)   # worse than not fusing at all
