...
# mentor/knowledge, not knowledge/: the package moved in the mentor restructure and this root
# was left behind -- every test here then failed on a directory that no longer exists, which
# the nightly never surfaced because it failed by construction until D371.
KNOWLEDGE_ROOT = FLUX_ROOT / "mentor" / "knowledge"
...
def test_index_is_built_from_all_five_hand_picked_chapters(index):
    # The shared corpus now holds several standards; this file's claim is about the RISC-V one.
    stems = {Path(c.source_path).stem for c in index._chunks
             if c.standard_id == "riscv-unpriv"}  # noqa: SLF001 - test-only introspection
    assert stems == {"preface", "naming", "zicsr", "m-st-ext", "zifencei"}


def _corpus_chunks(index):
    """The committed corpus only. The same index also carries `mentor/knowledge/library/`
    (D407: the operator's own papers, gitignored, thousands of chunks on the machine that has
    them and none on CI) -- a claim about the corpus must not depend on which machine runs it
    (D470)."""
    return [c for c in index._chunks if c.standard_id != "library"]  # noqa: SLF001
...
def test_index_has_a_realistic_number_of_chunks(index):
    ...
    assert 100 < len(_corpus_chunks(index)) < 400
...
def test_every_chunk_has_real_provenance(index):
    for chunk in index._chunks:  # noqa: SLF001 - test-only introspection
        assert chunk.source_path.startswith(
            "mentor/knowledge/library/" if chunk.standard_id == "library"
            else "mentor/knowledge/corpus/"), chunk.source_path
        assert chunk.text.strip()
    riscv = [c for c in index._chunks  # noqa: SLF001
             if c.standard_id == "riscv-unpriv"]
    assert riscv and all(
        c.source_path.startswith("mentor/knowledge/corpus/riscv-unpriv/") for c in riscv)
