...
`evaluator/rtl`'s Verilator requirement has, since SystemC doesn't need Docker or Verilator).
...
real evaluators and checks they agree — the actual point of a coarse-grain pre-check stage.
"""
...
def test_gemm_workload_evaluates_through_real_systemc_with_default_lanes(evaluator):
    ...
    # the closed-form derivation in evaluator/systemc/README.md, not a coincidence.
    ...
    assert result.escalation.next_stage == "rtl"
...
def test_2d_architecture_is_rejected():
    """architecture_ir_to_lanes is evaluator/rtl's own function, reused directly — this just
...
def test_agrees_exactly_with_the_real_rtl_evaluator(evaluator, arch_path):
    """The actual point of this stage: a real coarse-grain simulation reaching the same answer
