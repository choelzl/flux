...
def test_matches_pinned_real_values_from_a_prior_verified_run(evaluator, workload):
    ...
    # 565226, was 550513: the nixchip pin bump that brought Pythia (D361) also rebuilt gem5,
    # and the new build's number is deterministic -- reproduced bit-identically across two
    # independent full runs before this pin moved (D374). A pinned value belongs to a toolchain.
    assert result.metrics["latency_cycles"].value == pytest.approx(565226.0)
