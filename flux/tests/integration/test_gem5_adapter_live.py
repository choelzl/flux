...
`chia.simulators.gem5.Gem5Node`, the second evaluator here (after `evaluator/cacti`) adapted
...
`evaluator/gem5/README.md` — a second, broken Python 3.12 install earlier on this sandbox's PATH
...
`TimingSimpleCPU` at 1.2GHz, described by the adapter's own standard-library config
(`gem5_config.py`, D446) — fully deterministic (no LLM, no randomness anywhere in this path)
...
def _cpu_only_arch(cores: int = 1, freq_ghz: float = 1.2) -> dict:
    ...
    evaluated" shape evaluator/booksim/noxim/cacti already use).
    """
...
def test_matches_pinned_real_values_from_a_prior_verified_run(evaluator, workload):
    ...
    # 567137, was 565226 (and 550513 before the nixchip pin bump that brought Pythia, D361):
    # D446 moved this adapter off gem5's deprecated `se.py` onto its own standard-library config
    # (`gem5_config.py`), and a different board is a different measurement -- 6,326 instructions
    # against 5,840, because the standard library's SE-mode startup is not se.py's. Reproduced
    # bit-identically across two independent runs of the pinned v25.1.0.1 build before this pin
    # moved. A pinned value belongs to a toolchain AND to the configuration it measured.
    assert result.metrics["latency_cycles"].value == pytest.approx(567137.0)
    assert result.bottleneck.per_level_utilisation["sim_insts"] == pytest.approx(6326.0)
    assert result.bottleneck.per_level_utilisation["ipc"] == pytest.approx(0.011154271366530485)
...
def test_full_generic_riscv_soc_is_rejected_its_cpu0_node_has_four_cores(evaluator, workload):
    ...
    compute node" structural rule — but that node carries `cores: 4`, and evaluator/gem5 v0.1
