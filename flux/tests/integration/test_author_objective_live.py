...
def test_an_unfulfillable_request_fails_closed_with_the_real_validator_error(onnx_workload):
    ...
    base_arch = yaml.safe_load(
        (FLUX_ROOT / "core/ir/architecture/examples/simple-npu-1d-v1.yaml").read_text())
    # area_mm2 requires an escalation stage; forbidding escalation makes it unfulfillable
    prose = (
    ...
    if authored.success:
        # the LLM may 'succeed' only by deviating from the request (adding a stage or swapping
