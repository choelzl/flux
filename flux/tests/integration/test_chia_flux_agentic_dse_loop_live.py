...
(b) the winner passes an independent validity check, and its conformance is checked honestly —
    ...
    *different* candidate's real residual (the well-established width=8 1554-vs-529-cycle
    ...
    dims (`M`/`C`) `evaluator/timeloop`'s fixed spatial-constraint boilerplate can express —
...
nixpkgs#bison` (see `evaluator/booksim/README.md`). Requires real Docker Timeloop for the
`axis="memory_size"` tests (see `evaluator/timeloop/README.md`).
"""
...
def test_agentic_dse_loop_rejects_a_reference_backend_incompatible_with_the_mapping_axis():
    """`evaluator/rtl` and `evaluator/systemc` both model one fixed, hand-written loop schedule
    and categorically reject any explicit Mapping IR — a real per-adapter limitation
    (`evaluator/rtl/README.md`), not something axis="mapping" can route around. Checked as a
...
def test_agentic_dse_loop_noc_topology_axis_local_call(tmp_path):
    ...
    bug in `evaluator/booksim`'s adapter found and fixed while establishing this repo's first
