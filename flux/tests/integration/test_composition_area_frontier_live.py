...
3. every contender is escalated through two composed stages — rtl (latency) and openroad
...
def test_the_real_latency_area_frontier_over_per_layer_assignments(tmp_path):
    ...
    # -- 2. the campaign: latency screened (calibrated), area measured only at the openroad stage
    doc = {
    ...
    # honestly wide, and BOTH stages ran all 4
    ...
    for t in escalate:
        by_stage.setdefault(t.stage_index, []).append(t)
    assert len(by_stage[0]) == 4 and len(by_stage[1]) == 4

    # -- 4. component caching held: 4 assignments x 2 engines per stage, but only 4 distinct
    ...
    assert frontier, "a full-coverage two-stage escalation must produce a composite"
