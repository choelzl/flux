"""Campaign escalation through a REAL RTL stage (docs/decisions.md D217/D218): ZigZag screens the
...
def test_escalation_buys_only_contenders_and_resume_buys_nothing(tmp_path):
    ...
    assert escalated[0].status == "ok" and escalated[0].stage == "rtl"
    assert escalated[0].stage_index == 0

    # The RTL number is real and differs from screening (ZigZag's documented overestimation
    # bias): both fidelities visible in the report, per-stage provenance in the store.
    ...
    # and the escalation bookkeeping is keyed by (candidate, stage_index)).
    constructed.clear()
