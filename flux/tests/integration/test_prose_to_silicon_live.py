...
standalone-agentic target names, composed and REAL end to end:
    ...
    prose --qwen--> validated composition Objective (D232 + D239's authoring seam)
          ...
          --rtl + openroad stages--> the SAME pinned silicon frontier D237 measured directly.
...
def test_one_sentence_becomes_the_pinned_silicon_frontier(tmp_path):
    ...
    prose = (
        ...
        "real placed silicon area_mm2 — area is measured only at the escalation stages. "
    ...
    # every assignment stayed a contender (wide extrapolated CIs) and both stages ran all four
    assert len(store.ok_trials(cid, phase="screen")) == 4
    escalate = store.ok_trials(cid, phase="escalate")
    assert sorted(
        (t.stage_index, tuple(t.candidate["assignment"][op] for op in ("mm0", "mm1")))
        for t in escalate
