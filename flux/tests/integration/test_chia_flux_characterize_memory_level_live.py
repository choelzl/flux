...
this repo's real, multi-level Architecture IR documents and `evaluator/cacti`'s single-macro
...
def test_matches_evaluators_cacti_own_pinned_gbuf_result_directly(full_arch):
    ...
    evaluator/cacti's own test file pins for this exact gbuf level (512 KiB, 128-bit word,
...
def test_metrics_default_to_every_cacti_metric_not_the_generic_default_metrics_baseline(full_arch):
    ...
    area_mm2/power_w, evaluator/cacti's two most interesting numbers, and CACTI reports no
