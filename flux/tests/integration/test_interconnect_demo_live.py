...
Deliberately the SMALLEST real run: one step, no model. With no model the fixed plan takes the
menu's first unrun option, which is the anneal walk (D314: declared variants so it is reachable
without a model) -- thousands of fabrics screened, its best four placed for real, ~10 minutes of
Yosys and OpenROAD. It is not measuring the quality of anything — it is asserting the demo still
works. (Until D470 this file pinned the older schedule, "1 of 6 scopes enumerated" and more than
ten fabrics, and had been failing on that alone.)
"""
...
def test_it_says_what_it_did_not_explore(completed):
    """A study that looked at less has to say so. One step cannot enumerate every family, so
    the coverage line must count them and name the unexplored ones."""
    assert re.search(r"COVERAGE: \d+ of \d+ families enumerated", completed), completed[-2000:]
...
def test_a_fresh_store_is_populated_by_the_run(completed):
    ...
    The anneal step places its best four (`place=4` in the declared variant), so the store
    holds at least those."""
