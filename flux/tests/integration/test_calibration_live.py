...
`corpus/public/` and `corpus/holdout/` partitions via `flux_store.CorpusStore` (docs/stores.md,
docs/roadmap.md), the same enforced two-method access surface (`public_entries()` /
...
and the finding that `energy_pj`'s residual, once evaluator/zigzag's placeholder-cost issue was
...
# Filtered to the mlp-gemm0.yaml width-axis family specifically (docs/decisions.md D58/D59): the
# corpus now has two other real, separate families sharing the same *metric* but not the same
# *workload* — the memory-size axis's energy_pj entries (D58) and, since D59, a second real
# workload (mlp-ffn0.yaml) whose own corpus entry also declares objective=latency_cycles.
# Filtering by objective.metric alone (D58's original fix) is no longer specific enough; filtering
# by `workload_path` is the real, structural fix this test actually needs — this test is about
# calibrating *mlp-gemm0.yaml's own* residuals, not "every public entry using this metric."
# Also excludes the real Stream-only multi-core entry (docs/decisions.md D82): its architecture
# has no top-level `hierarchy` at all (real per-core structure lives inside
# `interconnect.multi_core` instead), so neither ZigZag nor Timeloop — the two backends this
# calibration test compares — can express it at all; it shares mlp-gemm0.yaml's own
# workload_path but is not part of this test's own single-core width-axis family.
_PUBLIC_ENTRIES = [
...
def populated_store(tmp_path, evaluated):
    with CalibrationStore(tmp_path / "cal.db") as store:
        for arch_name in CALIBRATION_ARCHS:
            ...
            # existed because evaluator/zigzag's architecture_translator.py used a flat,
...
def test_held_out_architecture_breaks_the_ratio_pattern(evaluated):
    """This is why calibration (and holdout discipline, docs/roadmap.md) matters: naively
...
def test_an_exactly_measured_point_is_corrected_and_does_not_escalate(evaluated, populated_store):
    """UPDATED CONTRACT (docs/decisions.md D106, verified here in D122). This test previously
    asserted that even an in-domain exact-match point escalates, because the underlying residual
    is ~204% wide. That was correct when a known bias was treated as uncertainty — and it is
    exactly what D106's debiasing exists to eliminate. Treating a *measured, consistent* 3.03x
    offset as a reason to go buy more data is the behaviour that made more calibration produce
    wider intervals.

    So the property worth pinning is the new one, and it is a stronger claim than the old:
    correction on an exactly-measured point reproduces its own reference to within a fraction of
    a percent, and escalation — which buys a measurement — is not recommended for a point whose
    measurement already exists. The held-out point in the test above is where escalation must and
    does fire.
    """
    arch_name = CALIBRATION_ARCHS[0]
    r = evaluated["results"][arch_name]
    ...
    assert calibrated.domain.in_domain is True
    # The raw model is ~3x off; the corrected value lands on the real reference.
...
def test_energy_pj_residuals_are_much_wider_than_latencys(evaluated, populated_store):
    ...
    evaluator/zigzag/architecture_translator.py's per-memory cost model is anchored to real
