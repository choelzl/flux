...
def test_real_cacti_puts_silicon_area_on_the_system_frontier(tmp_path):
    ...
    objective measured only at a real-CACTI escalation stage: the runner extracts the searched
    gbuf level per engine (the cacti adapter refuses multi-memory archs), narrows the stage to
    ...
    assert frontier, "the cacti stage must cover every contender"
