"""The conformance suite (docs/architecture.md): "any new evaluator backend must pass a shared suite
...
# 2-D support (docs/decisions.md D215): 4096 MACs over the full 8x8 array at 100% utilisation is
# the compute roofline exactly. Probed on both runners before being pinned, like every number here.
EXPECTED[("timeloop", "mlp-gemm0", "simple-npu-v1")] = 64.0

# mlp-ffn0 (docs/decisions.md D59, a real 2-layer workload): real, empirically probed, not
# guessed from source. ZigZag's translator supports multi-layer workloads (workload_translator.py
# iterates every einsum op) and succeeds on both NPU-shaped architectures.
EXPECTED[("zigzag", "mlp-ffn0", "simple-npu-1d-v1")] = 1560.0
EXPECTED[("zigzag", "mlp-ffn0", "simple-npu-v1")] = 208.0
# Timeloop's adapter now supports multi-op workloads too (docs/decisions.md D62 — one real,
# separate Docker invocation per layer, aggregated: 256.0 + 256.0 = 512.0 cycles, re-verified
# against each layer's own independently-run number, not just the pinned total). Succeeds on
# the same architectures mlp-gemm0 x timeloop does and fails on the remaining two for real,
# pre-existing, unrelated architecture-translation reasons (generic-riscv-soc-v1 has no compute
# attrs.dims; my-npu-v3 is 3-D, and Timeloop containers have no third mesh axis) — re-probed
# against all four architectures after each change, not assumed to still hold.
EXPECTED[("timeloop", "mlp-ffn0", "simple-npu-1d-v1")] = 512.0
# Two layers on the 8x8 array, one real invocation per layer, aggregated: 32 + 32 = 64.
...
def test_backends_that_both_succeed_on_the_same_pair_agree_on_what_they_were_given(
    ...
    identically," not just "both runs happened to finish." Two pairs qualify as of
    docs/decisions.md D62 (mlp-gemm0 x simple-npu-1d-v1, mlp-ffn0 x simple-npu-1d-v1 — Timeloop's
    real multi-op support closing the second one); this test was written to keep working as more
    do, and now genuinely exercises that generality rather than covering it only in theory.
    """
