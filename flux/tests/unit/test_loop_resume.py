...
from flux_loop.actions import Action, DirectedSearch, Outcome
...
def test_every_action_the_menu_wires_up_actually_exists():
    ...
    # `build_menu`, not `main`: the study became a callable sub-flow and the command line moved
    # out from around it (D345), and the menu became the problem's on the loop (D446). The
    # registry lives with the study, which is the half a calling orchestrator reaches.
    main = next(n for n in ast.walk(tree)
                if isinstance(n, ast.FunctionDef) and n.name == "build_menu")
