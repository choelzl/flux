...
def campaign(tmp_path):
    ...
    def _trial(phase, width, *, status="ok", result=None, error=None, stage=None, stage_index=None):
        seq = store.begin_trial(
            ...
            strategy_kind="grid", seed=0, deterministic=True, stage=stage, stage_index=stage_index,
        )
    ...
    esc8 = _trial("escalate", 8, result=_result(330.0), stage="rtl", stage_index=0)
    esc16 = _trial("escalate", 16, result=_result(165.0), stage="rtl", stage_index=0)
...
def test_frontier_outcomes_read_through_an_injected_reader(campaign):
    ...
    from flux_knowledge_mining.mining import FrontierOutcome, mine_frontier_outcomes
