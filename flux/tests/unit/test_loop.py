...
class Scripted:
    ...
    def propose(self, prompt, schema=None):
        ...
        if prompt.startswith("design lo"):
            return json.dumps([3, 1, 0, 0])            # first two right, last two wrong
        if prompt.startswith("design hi"):
            return json.dumps([5, 9, 2, 6])            # exactly right
        if "`lo-v` was refused" in prompt or "was refused" in prompt:
            ...
            if '"0, 0]' in prompt or "0, 0]" in prompt:
                return json.dumps({"edits": [{"find": "0, 0]", "replace": "4, 0]"}],
                                   "why": "fix 3rd"})
            return json.dumps({"edits": [{"find": "4, 0]", "replace": "4, 1]"}],
                               "why": "fix 4th"})
        if '"next"' in prompt or "Choose the next part" in prompt:
            return json.dumps({"next": "hi", "method": "direct"})
        return "{}"
...
def test_a_resumed_campaign_reloads_proven_parts_and_best_attempts(tmp_path):
    ...
    class OnlyHi(Scripted):
        def propose(self, prompt, schema=None):        # lo never gets fixed this run
            if prompt.startswith("design lo"):
                return json.dumps([3, 1, 0, 0])
            if "was refused" in prompt:
                return json.dumps({"edits": []})       # unusable -> rewrite -> same
            return super().propose(prompt, schema)
...
def test_the_prototype_stage_resumes_from_the_best_on_record_and_stops_when_nothing_measures(tmp_path):
    ...
    assert "a rule written since: no bare return" in prompts[0] and "1 short" not in prompts[0]
...
def test_the_timing_tree_shows_the_loops_structure(tmp_path):
    """Sub-loops are their own category: a step contains plan / generate-loop /
    judge, the generate-loop contains generate / build / test / patch, and the
    evaluation stage contains compose and the stages."""
    import flux_profile
    ...
    assert ("DSE: step 1",) in paths
    assert any(k[:2] == ("DSE: step 1", "generation: hi") or
               k[:2] == ("DSE: step 1", "generation: lo") for k in paths)
    assert any(len(k) == 3 and k[0] == "DSE: step 1" and k[1].startswith("generation")
               and k[2].startswith("test: build") for k in paths)
    assert any(len(k) == 3 and k[1].startswith("generation") and k[2].startswith("repair")
               for k in paths)
...
def test_the_compute_sandbox_runs_numpy_and_refuses_the_rest():
    ...
    assert got["bad import"].startswith("refused: import of 'os'")
...
def test_prompts_put_the_static_prefix_first_and_carry_compute_results(tmp_path):
    ...
    class Proposer(Scripted):
        ...
        def propose(self, prompt, schema=None):
            ...
            for marker in ("design lo", "design hi"):
                if marker in turn:
                    return super().propose(marker + turn.split(marker, 1)[1], schema)
            return super().propose(turn.lstrip(), schema)
    ...
    assert all("COMPUTE TOOL" in p for p in design_turns)         # the tool is offered
    # ...and accepted by the schema of every turn that offers it (the planner's own
    # schema is not a generation turn and stays as the problem wrote it)
    offered = [s for p, s in prompts if "COMPUTE TOOL" in p and isinstance(s, dict)]
    assert offered and all("compute" in s.get("properties", {}) for s in offered)


def test_a_computations_only_patch_reply_stays_in_patch_mode():
    from flux_loop import LoopRequest, LoopState, Candidate, BuildError, _generate_with_model
    ...
    class P(Problem):
        ...
        def parse_design(self, reply, subgoal):
            return Candidate("c", "value = 1", subgoal=subgoal), ""

        def build(self, cand, subgoal, state):
            return cand.artifact

        def fast_check(self, built, cand, subgoal, state):
            return (0, "") if "value = 4" in built else (1, "value should be 4")
    ...
    class Proposer:
        ...
        def propose(self, prompt, schema=None):
            ...
            seen.append(prompt)
            if self.turn == 1:
                return "anything"                                   # the design
            if self.turn == 2:                                      # compute only
                return json.dumps({"edits": [], "compute": [{"name": "sq", "code": "print(2*2)"}]})
            return json.dumps({"edits": [{"find": "value = 1", "replace": "value = 4"}]})

    st = LoopState(request=LoopRequest(repair_attempts=4), say=lambda _m: None,
                   proposer=Proposer(), feedback=None, workdir=".")
    cand, built, reason = _generate_with_model(P(), None, "", st, None)
    assert reason == "" and cand.artifact == "value = 4"
    assert any("RESULTS OF YOUR COMPUTATIONS" in p and "[sq]\n4" in p for p in seen)
...
def test_repair_turns_carry_a_progress_line_and_revert_on_regression():
    ...
    class Proposer:
        ...
        def propose(self, prompt, schema=None):
            ...
            return json.dumps({"edits": [{"find": current, "replace": f"v{self.turn - 1}"}]})
    ...
    class Proposer:
        ...
        def propose(self, prompt, schema=None):
            ...
            if "PROTOTYPE" in prompt or "prototype" in prompt.lower() and "rtl" not in prompt:
                ...
                return json.dumps({"prototype": code})
            return "rtl"
...
def test_a_rewrite_that_drops_its_tables_gets_them_back_and_a_rewrite_near_a_good_seed_is_refused(tmp_path):
    ...
    class P(Problem):
        ...
        def prefers_edits(self, subgoal, state, best_score, new_text):
            return "nearly right: send edits" if best_score <= 1 else None
    ...
    cand, built, reason = _generate_with_model(P(), "a", "", st, None)
    assert "your NEW prototype was not run: nearly right: send edits" in prompts[2]
...
def test_gradient_carries_the_trend_and_reverts_after_a_streak():
    ...
    # D484: the revert hands back the BEST attempt's failure text, not the refused edit's
    ...
    assert g.revert_due("c") and g.best_failure == "x=-Inf got -Inf wanted NaN"
...
def test_gradient_treats_a_refused_attempt_as_no_measurement():
    ...
    assert g.best == (30182.0, "a") and g.worse == 1 and g.prev == 30182.0
    ...
    assert g.worse == 2 and g.revert_due("e")
    assert g.revert()[0] == "c"
...
def test_problem_is_the_four_roles_combined():
    ...
    assert {"generate", "design_prompt", "patch_prompt", "prototype_spec"} <= set(vars(GeneratorRole))
