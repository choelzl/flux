...
of SRAM is worth is the reader's judgement. What it CAN do is lay the trade-off out on one stage,
...
def test_an_over_budget_design_is_refused_before_any_simulation():
    """The loop's gate (`World.judge`) refuses a design over the budget in microseconds, so no
    wave ever carries one (D362); the scorer no longer needs to know the budget at all."""
    ...
    world = prefetcher_problem(max_storage_bytes=storage_bytes(DEFAULT)).world
    state = LoopState(request=LoopRequest(), say=lambda _m: None, proposer=None, feedback=None)
    within = world._cand((DEFAULT, ("bingo",), {}), "llm")
    over = world._cand((big, ("bingo",), {}), "llm")
    assert world.judge(world.build(within, None, state), within, None, state).ok
    verdict = world.judge(world.build(over, None, state), over, None, state)
    assert not verdict.ok and "over the storage budget" in verdict.why
