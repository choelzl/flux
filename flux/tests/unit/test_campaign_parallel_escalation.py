"""Concurrent escalation (docs/decisions.md D290): the measured stage's tools are single-threaded
processes, so escalating one contender at a time leaves most of a machine idle. The runner may
measure a stage's contenders together, and every property of the serial path has to survive it —
...
def _doc(base_arch, widths, *, budget=64):
    return {
        ...
        "backends": {"screening": "fake", "escalation": ["fake_stage"]},
...
def _run(doc, tmp_path, name, screen, stage, **kwargs):
    ...
    run_campaign_steps(
        store, cid,
        make_evaluator=lambda n: stage if n == "fake_stage" else screen,
        **kwargs)
    return store, cid
...
def test_parallel_escalation_gives_the_same_results_as_serial(base_arch, tmp_path):
    widths = [1, 2, 4, 8]
    serial_stage = _SlowStageEvaluator(delay=0.05)
    par_stage = _SlowStageEvaluator(delay=0.05)
    ...
    p_store, p_cid = _run(_doc(base_arch, widths), tmp_path, "par", _ScreenEvaluator(),
                          par_stage, escalation_parallelism=4)
...
def test_it_actually_runs_concurrently(base_arch, tmp_path):
    ...
    stage = _SlowStageEvaluator(delay=0.2)
    _run(_doc(base_arch, [1, 2, 4, 8]), tmp_path, "conc", _ScreenEvaluator(), stage,
         escalation_parallelism=4)
    assert stage.calls == 4
    assert stage.max_live > 1, "escalations never overlapped"


def test_parallelism_one_keeps_the_serial_path_exactly(base_arch, tmp_path):
    stage = _SlowStageEvaluator(delay=0.01)
    _run(_doc(base_arch, [1, 2, 4, 8]), tmp_path, "one", _ScreenEvaluator(), stage,
         escalation_parallelism=1)
    assert stage.max_live == 1, "parallelism=1 must not overlap anything"
...
def test_one_failing_candidate_does_not_take_the_batch_with_it(base_arch, tmp_path):
    ...
    stage = _SlowStageEvaluator(delay=0.02, poison_width=4)
    store, cid = _run(_doc(base_arch, [1, 2, 4, 8]), tmp_path, "iso", _ScreenEvaluator(), stage,
                      escalation_parallelism=4)
...
def test_no_trial_is_left_running(base_arch, tmp_path):
    ...
    store, cid = _run(_doc(base_arch, [1, 2, 4, 8], budget=6), tmp_path, "run",
                      _ScreenEvaluator(), stage, escalation_parallelism=4)
