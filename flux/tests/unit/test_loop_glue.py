...
def test_ollama_proposer_is_the_one_proposer_with_reasoning_off_unless_asked(monkeypatch):
    ...
    assert isinstance(p, OpenAIChatProposer) and p.model == "qwen3:4b" and p.think is False
    assert not p.hosted and p.base_url.endswith("/v1")
