...
def test_a_deferred_metric_no_stage_can_measure_is_rejected(base_arch):
    """The other direction: area_mm2 at escalation with stages that only measure latency —
    nothing would ever produce it, caught at authoring with the stage suggestion."""
    bad = json.loads(json.dumps(_COMPOSITION_DOC))
    ...
    authored = flux_author_objective(
        "per-layer engine sizing", _WORKLOAD, base_arch, llm=scripted)
    assert authored.success and authored.attempts == 2
    assert "no escalation stage" in scripted.prompts[1]
