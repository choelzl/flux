...
def test_notes_persist_as_campaign_events_and_reload_on_resume(tmp_path):
    ...
    objective = {"study": "prefetcher", "seed": 0}
    first = Records(db, objective=objective, log=lambda _msg: None)
    assert first.store is not None
    first.note("hold the region size at 2 KB")
    first.close("completed")

    second = Records(db, objective=objective, log=lambda _msg: None)
...
def test_with_no_model_role_guidance_is_still_recorded_and_reported(tmp_path):
    """The loop's own drain, which every study uses since D446: a typed line is persisted into
    the campaign record and reported in the lessons, and on a run with no model role to hand it
    to it says so rather than implying it reached a prompt."""
    ...
    db = str(tmp_path / "campaign.db")
    records = Records(db, objective={"study": "prefetcher"}, log=lambda _msg: None)
    state = LoopState(request=LoopRequest(db=db), say=lambda _msg: None, proposer=None,
                      feedback=channel, records=records)
    ...
    assert any(line.startswith("[human]") and "reached no prompt" in line
               for line in state.lessons)
    assert records.notes() == ["stop growing the pht"]
    # with a model role in the run, the same line says where it goes instead
    from flux_feedback import scripted_channel

    told = LoopState(request=LoopRequest(), say=lambda _m: None, proposer=object(),
                     feedback=scripted_channel("prefer fewer ways"))
    assert told.drain() and any("goes into the next prompt" in line for line in told.lessons)
    channel.close()
    fake.close()
    records.close("completed")
