...
def test_the_area_stage_extracts_the_level_and_narrows_to_area(base_arch):
    ...
    composite (deeper stage wins per metric)."""
    from flux_search_campaign import MemoryLevelAreaStage, slice_workload
    ...
    inner = _Inner()
    stage = MemoryLevelAreaStage(inner, level="gbuf")
    stage.evaluate(Candidate(workload=slice_workload(_WORKLOAD, "big"), arch=engine,
                            mapping=None), Budget(),
...
def test_the_runner_wires_the_cacti_stage_through_extraction(base_arch, tmp_path):
    """The full loop with a cacti escalation stage: area joins the composite at cacti
...
def test_the_scaled_stage_characterizes_at_the_native_node_and_annotates(base_arch):
    """The stage rewrites the extracted arch to the native node, scales the returned area by
    ...
    stage = MemoryLevelAreaStage(inner, level="gbuf", scale_from_nm=32,
    ...
    inner_p = _Inner()
    plain = MemoryLevelAreaStage(inner_p, level="gbuf", scale_from_nm=32,
    ...
    inner2 = _Inner()
    stage32 = MemoryLevelAreaStage(inner2, level="gbuf", scale_from_nm=32)
