...
def test_wrong_design_is_refused_by_exhaustion_and_the_record_teaches(tmp_path):
    ...
    from flux_nlu import NluRequest, run_study
    from flux_nlu.flow import mentor_for
    from flux_nlu.verify import tools_missing
    ...
    req = NluRequest(db=db, ops=("exp",), llm_rounds=1, test_rounds=0,
                     repair_attempts=0, screen_only=True, agentic=False)
    out = run_study(req, proposer=Scripted(), log=lambda _m: None)
    assert out.decision is None
    assert any("ULP" in why for _n, why in out.refused)
    # the counterexample: decoded number AND raw bits, and the reference named (D419)
    assert any("(0x" in why and "wanted" in why for _n, why in out.refused)
    ...
    ctx = mentor_for(("exp",)).text("record", SimpleNamespace(records=r))
...
def test_agentic_admits_freezes_and_composes(monkeypatch, tmp_path):
    ...
    calls = {"gen": [], "judged": []}
    prob = NluProblem(ops=("recip", "rsqrt", "exp"), test_rounds=0)
    ...
    def fake_compose(admitted, state):
        ...
        from flux_nlu.problem import op_stub, wrap_per_op
...
def test_the_plan_is_told_whether_a_prototype_comes_first():
    ...
    def fake_run_loop(problem, req, **kw):
        seen["prototype"] = req.prototype
    ...
    for choice in (True, False):
        with _pytest.MonkeyPatch.context() as mp:
            mp.setattr(flux_loop, "run_loop", fake_run_loop)   # flow imports it lazily, by name
            with _pytest.raises(KeyboardInterrupt):
                flow.run_study(NluRequest(db="", ops=("exp",), llm_rounds=1, prototype=choice),
                               proposer=object())
        assert seen["prototype"] is choice
...
def test_proposer_sends_the_schema_as_ollama_format(monkeypatch):
    """The schema reaches ollama's `format` field (constrained decoding), and a
    plain call still sends none."""
    import json as _j
    ...
    class _Resp:
        ...
        def read(self): return _j.dumps({"response": "{}", "done_reason": "stop"}).encode()

    def fake_urlopen(req, timeout):
        sent.update(_j.loads(req.data))
        return _Resp()

    monkeypatch.setattr(urllib.request, "urlopen", fake_urlopen)
    p = NativeOllamaProposer(model="stub")
    p.propose("hi", {"type": "object"})
...
def test_best_design_per_operator_survives_the_pass(monkeypatch, tmp_path):
    ...
    prob = NluProblem(ops=("recip",), test_rounds=0)
    ...
    said = []
    prob2 = NluProblem(ops=("recip",), test_rounds=0)
...
def test_generation_loop_tests_and_edits_until_it_passes(monkeypatch, tmp_path):
    ...
    class Proposer:
        def propose(self, prompt, schema=None):
            calls["n"] += 1
            return _j.dumps({"name": f"v{calls['n']}", "latency": 0, "method": "m",
                             "source": good})
...
def test_a_breaking_edit_is_reverted_not_chased(monkeypatch, tmp_path):
    ...
    class Proposer:
        def propose(self, prompt, schema=None):
            return _j.dumps({"name": "v", "latency": 0, "method": "m",
...
def test_generator_places_requested_tables_into_the_module(monkeypatch, tmp_path):
    ...
    seen = {}
    prob = NluProblem(ops=("exp",), test_rounds=0)
...
def test_the_campaigns_pre_loop_rows_still_reload(tmp_path):
    ...
    best-so-far designs -- NluProblem maps those rows itself."""
    ...
    prob = NluProblem(ops=("recip", "exp"), test_rounds=0)
    said = []
...
"""
    ...
    prompt, schema = prob.prototype_spec("recip", st)
