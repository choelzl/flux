...
def test_to_dict_is_json_safe():
    ...
    workload = flux_ir.load_document(FLUX_ROOT / "core/ir/workload/examples/mlp-gemm0.yaml")
    arch = flux_ir.load_document(FLUX_ROOT / "core/ir/architecture/examples/simple-npu-1d-v1.yaml")
    candidate = generate_flat_mapping_candidates(workload, arch, for_op="mlp.gemm0")[0]
    d = candidate.to_dict()
    json.dumps(d)  # raises if the temporal_order tuple leaked through unconverted
    assert d["spatial_dim"] == candidate.spatial_dim
    assert d["temporal_order"] == list(candidate.temporal_order)
    assert d["mapping"] == candidate.mapping
