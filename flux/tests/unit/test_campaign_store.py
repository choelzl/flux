...
def test_escalation_bookkeeping_is_by_stage_index_not_name(store):
    """Two stages may share a backend name (the D112 ["rtl", "rtl"] lesson) — idempotence keys on
    the position."""
    cid = _start(store)
    seq = _begin(store, cid, key="w4", phase="escalate", stage="rtl", stage_index=0)
    store.complete_trial(cid, seq, status="ok", result=_result(), error=None, wall_clock_s=1.0)
