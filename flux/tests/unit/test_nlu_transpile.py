...
The three hand-written families of `nlu_fixtures` (once the D475 floor generator, kept only
here since D507) are the FIXTURES: they are written in the subset and have known-good numpy
faces to compare against. They are not in the flow: the flow's own work is what the campaign
is for."""
...
def prototype_of(op: str) -> tuple[str, object, object]:
    ...
    fam = F.FIXTURES[op]
    cfg, model, tables = fam["config"], fam["model"], fam["tables"]
    ...
    class Proto:
        def propose(self, prompt, *, schema=None, tools=None, budget=None):
            ...
            if "PROTOTYPE FIRST" in prompt:
                return Reply.of(json.dumps({"prototype": src}))
...
def test_a_pipelined_transpile_is_bit_exact_at_its_latency_and_the_mux_equalises(tmp_path):
    ...
    from flux_nlu.world import wrap_per_op
