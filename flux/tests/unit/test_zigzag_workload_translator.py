...
tests/integration/test_zigzag_adapter_live.py for that). docs/evaluator-abi.md: adapters must fail
...
DMA_WORKLOAD = FLUX_ROOT / "core/ir/workload/examples/soc-dma-desc-fetch.yaml"
...
def test_op_with_dynamic_bound_is_rejected():
    ...
    consume (docs/gap-analysis.md G5). Must fail loudly, not silently pick a bound."""
