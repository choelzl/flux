...
def test_the_loops_vocabulary_is_stage():
    ...
    for hook in ("analytic_stages", "uncached_stages", "evaluator_name", "cutoff", "route"):
...
def test_a_campaign_written_before_the_rename_still_opens(tmp_path):
    ...
    seq = store.begin_trial(cid, phase="screen", candidate={"w": 8}, candidate_key="k1",
                            workload_hash="", arch_hash=None, mapping_hash=None,
                            strategy_kind="loop", seed=None, deterministic=True,
                            stage="placement", stage_index=2)
    ...
    conn.execute("ALTER TABLE trials RENAME COLUMN stage_index TO rung_index")
    conn.commit()
    conn.close()

    store = CampaignStore(db)
    columns = {row[1] for row in store._conn.execute("PRAGMA table_info(trials)")}
    assert "stage" in columns and "stage_index" in columns
    assert "rung" not in columns and "rung_index" not in columns
    (trial,) = store.trials(cid)
    assert trial.stage == "placement" and trial.stage_index == 2, "the data came with it"
