...
def test_catalog_introspects_the_real_mcp_surface():
    ...
    assert "evaluate" in catalog and "calibrate" in catalog and "nlu_dse_loop" in catalog
...
def test_catalog_subset_filters_and_rejects_unknown_names():
    catalog = build_catalog(["evaluate", "flux_calibrate"])  # both name forms accepted
    assert set(catalog) == {"evaluate", "calibrate"}
...
class ScriptedProposer:
    ...
    def propose(self, prompt, *, schema=None, tools=None, budget=None):
        ...
        return Reply.of(self.replies.pop(0))
...
def test_wall_clock_budget_stop_still_gets_a_conclude_only_round(tmp_path, monkeypatch):
    ...
    class SlowScripted(ScriptedProposer):
        def propose(self, prompt, *, schema=None, tools=None, budget=None):
            import time as _t
            _t.sleep(0.06)
            return Reply.of(super().propose(prompt))
...
# ---------- omni on the one loop (docs/decisions.md D458) ----------

def test_omni_runs_on_flux_loop_and_the_loop_keeps_its_record(tmp_path, monkeypatch):
    """The rim is the loop's now: executed steps are trials on the document's `run` stage with
    omni's own strategy and evaluator, gate refusals are refused trials, and the
    conclusion is what the next run of this prompt reads back."""
    import flux_omni.pilot as pilot
    ...
    report = pilot.run_omni("add on the loop", proposer, workdir=tmp_path, db_path=db)
    assert report.done and len(report.outcomes) == 1

    block = pilot.campaign_for("add on the loop")
    rec = Records(db, objective={"study": "omni", **{k: v for k, v in block.items() if k != "name"}},
                  name=block["name"], log=lambda _m: None)
    assert rec.resumed, "the campaign is keyed by the prompt, so the same task resumes it"
    ran = rec.known_rows(stage="run")            # the document's one stage
    assert [r.candidate.get("tool") for r in ran] == ["add"]
    assert "elapsed_s" in ran[0].metrics
    refused = rec.refusal_rows()
    assert any("unknown argument" in r.reason for r in refused), [r.reason for r in refused]
    assert [c["conclusion"] for c in rec.conclusions(limit=1)] == ["sum is 3"]


def test_an_operator_note_reaches_the_next_round_through_the_loop(tmp_path, monkeypatch):
    """The feedback drain is the loop's too: a note typed between rounds lands in the next
    prompt, not in the next run."""
    import flux_omni.pilot as pilot
    monkeypatch.setattr(pilot, "build_catalog", lambda subset=None: _stub_catalog())
    ...
    class Channel:
        ...
        def drain(self):
            ...
            if self.sent:
                return []
            self.sent = True
            return [Note("prefer the cheap tool", time.time())]

    proposer = ScriptedProposer([
        json.dumps({"steps": [{"tool": "add", "args": {"a": 1, "b": 1}}]}),
        json.dumps({"steps": [], "done": True, "conclusion": "done"}),
    ])
    pilot.run_omni("noted", proposer, workdir=tmp_path, max_rounds=2, feedback=Channel())
    assert any("prefer the cheap tool" in p for p in proposer.prompts), proposer.prompts


def test_the_second_run_of_a_prompt_is_told_what_the_first_concluded(tmp_path, monkeypatch):
    import flux_omni.pilot as pilot
    monkeypatch.setattr(pilot, "build_catalog", lambda subset=None: _stub_catalog())
    db = str(tmp_path / "omni.db")
    first = ScriptedProposer([json.dumps({"steps": [], "done": True,
                                          "conclusion": "the sum is three"})])
    pilot.run_omni("say the sum", first, workdir=tmp_path / "a", db_path=db)
    second = ScriptedProposer([json.dumps({"steps": [], "done": True, "conclusion": "again"})])
    pilot.run_omni("say the sum", second, workdir=tmp_path / "b", db_path=db)
    assert "the sum is three" in second.prompts[0], "round 1 read the earlier verdict back"
