...
def test_omni_runs_on_flux_loop_and_the_loop_keeps_its_record(tmp_path, monkeypatch):
    """The rim is the loop's now: executed steps are trials on the `executed` stage with
    ...
    rec = Records(db, objective={"study": "omni", "prompt": "add on the loop", "tools": []},
                  log=lambda _m: None)
    assert rec.resumed, "the campaign is keyed by the prompt, so the same task resumes it"
    ran = rec.known_rows(stage="executed")
