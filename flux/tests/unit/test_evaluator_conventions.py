...
def test_no_evaluator_goes_through_chia():
    """A canary rather than a constraint: the two that did (cacti, gem5) left with the dead
    periphery (D540); an evaluator that starts importing CHIA does so by accident."""
    through_chia = {d.name for d in EVALUATORS if _has_code(d) and _touches_chia(d)}
    assert through_chia == set(), (
...
def test_the_central_readme_states_the_rule():
    ...
    assert "wraps its tool directly" in text and "no evaluator imports CHIA" in text
