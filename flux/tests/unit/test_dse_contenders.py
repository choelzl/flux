...
class _Stage:
    """An escalation stage with its own opinion, so the escalated winner can differ from the
    screening winner."""
    ...
    def evaluate(self, candidate, budget, metrics):
        ...
        return _result(self.values[name], evaluator="stage@1")
...
def test_default_still_escalates_only_the_winner():
    ...
    report = run_architecture_dse(
        ...
        metric="latency_cycles", escalation_evaluators=[("rtl", stage)], budget=Budget(),
    )
    assert stage.seen == ["a"]                       # only the screening winner was bought
...
def test_contender_escalation_buys_every_unresolved_candidate_and_can_flip_the_winner():
    ...
    out `b` — and the higher-fidelity stage says `b` is actually better. Winner-only escalation
    would have shipped `a` with no way to notice."""
    cands = [_Cand("a"), _Cand("b"), _Cand("c")]
    stage = _Stage({"a": 10.0, "b": 5.0, "c": 999.0})
    report = run_architecture_dse(
        workload=_WORKLOAD, candidates=cands,
    ...
    assert set(stage.seen) == {"a", "b"}          # c's budget was correctly not spent
...
def test_escalation_steps_carry_candidate_identity_for_json():
    ...
    report = run_architecture_dse(
        workload=_WORKLOAD, candidates=cands,
        screening_evaluator=_Screen({"a": (100.0, (50.0, 200.0)), "b": (102.0, (60.0, 210.0))}),
        metric="latency_cycles", escalation_evaluators=[("rtl", _Stage({"a": 1.0, "b": 2.0}))],
...
class _PartialStage:
    """A stage that measures only the first `limit` candidates it sees — models a wall-clock
    cutoff landing mid-stage, or a stage that raises for some candidates."""
    ...
    def evaluate(self, candidate, budget, metrics):
        ...
        if self.limit is not None and len(self.seen) >= self.limit:
            raise RuntimeError("stage exhausted (models a mid-stage cutoff)")
        self.seen.append(name)
        return _result(self.values[name], evaluator="stage@x")

    def evaluate_batch(self, candidates, budget, metrics):
        return [self.evaluate(c, budget, metrics) for c in candidates]


def test_a_truncated_stage_does_not_override_a_complete_shallower_one():
    """Review finding: `escalated_winner` took the deepest stage even when it measured only a
    ...
    complete = _PartialStage({"a": 100.0, "b": 90.0, "c": 50.0, "d": 10.0})       # all four
    truncated = _PartialStage({"a": 101.0, "b": 91.0, "c": 51.0, "d": 11.0}, limit=2)  # first two
    ...
    assert winner.name == "d", "a subset stage must not outrank a complete one"


def test_repeated_stage_names_are_not_merged_into_one_comparison():
    """Review finding: `by_stage` keyed by stage NAME merged two different fidelities that happen
    ...
    shallow = _Stage({"a": 5.0, "b": 50.0})    # says a
    deep = _Stage({"a": 40.0, "b": 30.0})      # says b — and is the deeper stage
    report = run_architecture_dse(
        workload=_WORKLOAD, candidates=cands, screening_evaluator=screen,
        metric="latency_cycles", budget=Budget(), escalate_contenders=True,
        escalation_evaluators=[("rtl", shallow), ("rtl", deep)],  # same NAME, different stages
    )
    winner, _ = report.escalated_winner()
    assert winner.name == "b", "the deepest stage decides; same-named stages must not merge"
...
def test_to_dict_exposes_the_escalated_winner():
    ...
    report = run_architecture_dse(
        workload=_WORKLOAD, candidates=cands,
        screening_evaluator=_Screen({"a": (100.0, (50.0, 200.0)), "b": (102.0, (60.0, 210.0))}),
        metric="latency_cycles", escalation_evaluators=[("rtl", _Stage({"a": 10.0, "b": 5.0}))],
        budget=Budget(), escalate_contenders=True,
    )
    payload = report.to_dict()
...
def test_a_result_missing_the_metric_is_a_failure_not_a_crash():
    """Review finding (D112): `evaluator/rtl` returns an empty metrics dict when the requested
