...
def test_points_from_campaign_reads_trials_in_order(tmp_path):
    ...
    rec.trial(DEFAULT, ("bingo",), stage="screen", strategy="s",
              metrics={"geomean_speedup": 1.03, "storage_bytes": 35096.0}, error=None, wall_s=0)
    rec.trial(DEFAULT.replace(pht_size=8192), ("bingo",), stage="screen", strategy="s",
              metrics={"geomean_speedup": 1.05, "storage_bytes": 60000.0}, error=None, wall_s=0)
    rec.trial(DEFAULT, ("bingo",), stage="screen", strategy="s", metrics=None,
              error="refused", wall_s=0)
    rec.trial(DEFAULT.replace(pht_size=8192), ("bingo",), stage="decide", strategy="confirmed",
