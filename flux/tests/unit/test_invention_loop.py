...
The round (`flux_prefetcher.invent`, the world's since review 2 step R3) is exercised end to end with a fake model, compiler and simulator: what is asserted is
...
from flux_prefetcher import invent as node  # noqa: E402
...
class FakeWorld:
    ...
    def __init__(self):
        self.asked: list[str] = []
        self.measured: list[tuple[str, tuple[str, ...], str]] = []   # (binary, types, stage)
    ...
    def measure(self, jobs, parallelism=1, binary=None):
        ...
        for job in jobs:
            ...
            stage = "decide" if job["simulation"] >= 100_000_000 else "screen"
            self.measured.append((Path(binary).parent.parent.name, types, stage))
            ipc = 1.0
            if types:
...
def world(tmp_path, monkeypatch):
    ...
    class _Scripted:                       # the one proposer (D508), answering from the fake world
        def __init__(self, *a, **k): ...

        def propose(self, prompt, **kw):
            return flux_llm.Reply(w.ask(prompt))

    monkeypatch.setattr(flux_llm, "OpenAIChatProposer", _Scripted)
...
def test_the_loop_invents_against_the_tallest_stack_and_repairs_an_inert_design(world, tmp_path):
    report = node.invent(
        ...
        scratch_root=str(tmp_path), workers=2, confirm_best=True)
...
def test_an_inert_design_is_not_repaired_when_repairs_are_off(world, tmp_path):
    report = node.invent(
        ...
        keep_dir=str(tmp_path / "kept"), scratch_root=str(tmp_path), workers=2)
