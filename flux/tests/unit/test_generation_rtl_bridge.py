...
def test_the_gemm_schedule_predicts_the_reference_evaluators_own_cycle_count():
    ...
    is 529 cycles (real Verilator, `evaluator/rtl`). The derived schedule has to predict exactly
...
def test_a_partial_k_group_is_supported_by_masking(lanes, kg):
    ...
    the final group is a real design, and it is the case worth supporting: `evaluator/rtl`
...
def test_the_accumulator_is_sized_from_the_worst_case_not_the_drawn_vectors():
    ...
    # accumulator its worst case needs (8+8 product + 6 lane bits), so the physical stage places
