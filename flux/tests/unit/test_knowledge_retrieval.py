...
def test_default_index_cites_paths_from_the_repo_root():
    """Provenance paths are relative to `flux/`; the default index computed its root one
    level short and cited `knowledge/corpus/...` (a path that does not exist)."""
    from flux_knowledge.retrieval import _cached_default_index

    idx = _cached_default_index()
    corpus = [c for c in idx._chunks if c.standard_id != "library"]
    assert corpus
    assert all(c.source_path.startswith("mentor/knowledge/corpus/") for c in corpus), \
        sorted({c.source_path.split("/")[0] for c in corpus})
