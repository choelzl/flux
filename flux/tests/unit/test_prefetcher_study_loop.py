...
def study(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(loop, "_fingerprint", lambda b: {"champsim": "planted"}, raising=False)
    return tmp_path


def _run(tmp_path, **kw):
    from flux_prefetcher.flow import run_study
    from flux_prefetcher.study import PrefetcherRequest

    fields = dict(db=str(tmp_path / "p.db"), budget=6, llm_round=0, compose_rounds=1,
                  tune_partners=0, stage=2, decide_on_finalists=2, include_invented=False,
                  parallelism=4)
    fields.update(kw)
    backend = fields.pop("measure_batch", _backend)
    return run_study(PrefetcherRequest(**fields), measure_batch=backend, log=lambda _m: None)
...
def test_the_record_holds_the_search_and_seeds_the_next_run(study):
    ...
    first = _run(study)
    rec = Recorder(str(study / "p.db"), {
        "study": "bingo-l2-prefetcher", "traces": list(BENCHMARKS), "stage": 2,
        "retention_floor": first.provenance and 0.90, "compose_rounds": 1, "seed": 0,
        "problem": "maximise geomean IPC speedup, then minimise storage"}, lambda _m: None)
    assert rec.resumed
    known = rec.known(stage="screen")
...
def test_the_gate_refuses_an_over_budget_design_that_reaches_it(study):
    ...
    from flux_prefetcher.loop import PrefetcherProblem
    from flux_prefetcher.study import PrefetcherRequest

    budget = storage_bytes(DEFAULT)
    big = DEFAULT.replace(pht_size=DEFAULT.pht_size * 8)
    assert storage_bytes(big) > budget
    problem = PrefetcherProblem(PrefetcherRequest(db="", max_storage_bytes=budget))
...
def test_a_simulator_that_fails_every_candidate_reports_no_result(study):
    ...
    out = run_study(PrefetcherRequest(db=str(study / "crash.db"), budget=4, llm_round=0,
                                      compose_rounds=0, tune_partners=0, stage=1,
                                      include_invented=False),
                    measure_batch=only_the_baseline, log=lambda _m: None)
