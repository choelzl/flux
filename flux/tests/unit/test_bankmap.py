...
def _problem(**params):
    ...
    D533; review 2 step R3). A test's ask is not the document's campaign: no name, so the
    record -- when a test asks for one -- is keyed by the ask."""
    import yaml
...
def test_db_records_trials_and_reseeds_the_tried_list(tmp_path):
    ...
    rec = Records(db, objective=prob.objective(request_for(prob.task, db=db)))
    assert rec.resumed
