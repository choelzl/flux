...
def test_bool_dtype_accepted():
    ...
    from flux_codegen_systemc_harness import cpp_type

    assert cpp_type(spec.ports[0]) == "bool"
    assert cpp_type(spec.ports[1]) == "bool"
...
def test_a_sized_port_becomes_an_sc_int_of_that_width():
    ...
    assert [cpp_type(p) for p in spec.ports] == ["sc_int<16>", "sc_int<40>", "int"]
