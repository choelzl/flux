...
def test_the_space_is_discovered_from_the_problem_and_carries_no_duplicates():
    ...
    for spec in wide:
        ...
        # THE shared definition of fabric identity, not a copy of it (docs/decisions.md D311).
        # A local copy here asserted a coarser rule than the enumerator used, and the difference
        # mattered: a Clos at n=4,m=4 and a staged fabric can share blocks, depth and concurrency
        # while differing in PATH CHOICE — the Clos rotates among middle switches and the staged
        # one cannot, worth up to 52% of throughput (D302) — so merging them deleted a real design
        # from the space.
