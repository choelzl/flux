...
the evaluator registry (`flux_evaluator_abi.registry`, D426; `flux_evaluator_abi` is its facade) is unreachable from the CLI, from every CHIA node, and from every MCP tool —
...
def _implemented_adapter_dirs() -> dict[str, Path]:
    ...
    application's own code (`champsim_bingo`). Both are found through `localSrcDirs`, so
    a moved adapter cannot escape this check -- a package the shell cannot import is not an
    adapter at all.
    """
    found = {}
...
def test_the_evaluators_directory_is_findable():
    ...
    assert "champsim_bingo" in set(implemented)          # an application's own evaluator (D521: the interconnect's two went with it)
    assert "applications/" in str(implemented["champsim_bingo"])
