...
def test_a_phases_output_reaches_the_task_details_pane():
    ...
    try:
        ...
        with flux_profile.phase("test: build nlu_exp") as out:
            out["error"] = "x" * 13000                     # bounded, like a parameter
...
def test_task_details_scroll_on_their_own_region():
    ...
    snap = bus.snapshot()
    top, _i, _o, _r = task_rows(snap, None, list_rows=4, detail_rows=6, detail_scroll=0)
    down, _i, _o, _r = task_rows(snap, None, list_rows=4, detail_rows=6, detail_scroll=10)
