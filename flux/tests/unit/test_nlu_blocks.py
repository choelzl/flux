...
from flux_loop.check import group_sites
from flux_loop.prototype import reminder_for, spec_prompt, verified_operators
...
def test_sandbox_errors_come_back_explained_and_tables_as_numbers():
    ...
    from flux_nlu.world import World as _P
...
def test_per_element_control_flow_is_refused_statically_with_every_site():
    ...
    from nlu_fixtures import nlu_problem
    from flux_loop.pyint.rules import hardware_subset_violations
    ...
    assert group_sites(["line 3: a", "line 1: b", "line 2: a"]) == ["lines 2, 3: a", "line 1: b"]
...
def test_the_blocks_audit_their_contracts_on_the_models_data():
    ...
    prob = nlu_problem(ops=("recip",), ulp_budget=1, clock_period_ps=1250.0, seed=1, test_rounds=0)
    st = LoopState(request=LoopRequest(), say=lambda _m: None, proposer=None, feedback=None)
...
def test_every_repair_turn_names_the_blocks_that_exist():
    ...
    prob = nlu_problem(ops=("recip",), ulp_budget=1, clock_period_ps=1250.0, seed=1, test_rounds=0)
    st = LoopState(request=LoopRequest(), say=lambda _m: None, proposer=None, feedback=None)
    text = reminder_for(prob.prototype(), "recip", st)
    for fn in B.BLOCKS:
...
def test_verified_operators_are_blocks_for_the_next_part():
    ...
    prob = nlu_problem(ops=("exp", "recip", "sigmoid"), ulp_budget=1, clock_period_ps=1250.0, seed=1, test_rounds=0)
    st = LoopState(request=LoopRequest(), say=lambda _m: None, proposer=None, feedback=None)
    st.prototypes = {"exp": COMPOSED_EXP, "recip": COMPOSED_RECIP}
    assert sorted(verified_operators(st, "sigmoid")) == ["exp", "recip"]
    assert verified_operators(st, "exp") == {"recip": verified_operators(st, "exp")["recip"]}   # never itself
...
def test_a_multiply_rounded_composition_is_told_its_precision_floor():
    ...
    got = World._rounding_residue(sig, {"over_budget": 106, "max_ulp": 3})
    ...
    assert World._rounding_residue("def design(x):\n    return pack_fp16(0, 0, x, 10)\n",
                                        {"over_budget": 50, "max_ulp": 2}) == ""      # one rounding: not this
    assert World._rounding_residue(sig, {"over_budget": 106, "max_ulp": "class-mismatch"}) == ""
    ...
    class Composer:
        def propose(self, prompt, *, schema=None, tools=None, budget=None):
            ...
            if "PROTOTYPE FIRST" in prompt:
                return Reply.of(json.dumps({"prototype": COMPOSED_EXP}))
