...
from flux_records.extract import Law, laws_text, pairwise_laws
...
def test_records_round_trip_results_and_conclusions(tmp_path):
    ...
    r.phase("scoring")
    r.trial({"policy": "S1", "fabric": "hier"}, "S1+hier", stage="analytic",
            strategy="cross", metrics={"thr": 12.5, "lat": 15.0})
    r.trial({"policy": "S0", "fabric": "hier"}, "S0+hier", stage="analytic",
            strategy="cross", metrics={"thr": 10.0, "lat": 18.0})
    r.trial({"refused": "bad taps"}, "refused:1", stage="gate", strategy="llm",
    ...
    assert r2.resumed
    known = r2.known(stage="analytic", metric="thr")
...
def test_imapping_run_records_and_reads_back(tmp_path):
    ...
    r = Records(db, objective=identity(study.problem, db))
    assert r.resumed
...
def test_records_known_honours_the_metric_direction(tmp_path):
    """`known` ranked every metric bigger-is-better; a latency or an area is best when
    smallest. `higher_is_better=False` flips both the per-key pick and the order."""
    db = str(tmp_path / "dir.db")
    r = Records(db, objective={"study": "dir", "seed": 1})
    r.trial({"policy": "S1"}, "S1", stage="analytic", strategy="s", metrics={"lat": 15.0})
    r.trial({"policy": "S0"}, "S0", stage="analytic", strategy="s", metrics={"lat": 18.0})
    r.trial({"policy": "S0"}, "S0", stage="analytic", strategy="s", metrics={"lat": 12.0})
    up = r.known(stage="analytic", metric="lat")
    down = r.known(stage="analytic", metric="lat", higher_is_better=False)
    assert [(k[0]["policy"], k[1]) for k in up] == [("S0", 18.0), ("S1", 15.0)]
    assert [(k[0]["policy"], k[1]) for k in down] == [("S0", 12.0), ("S1", 15.0)]
