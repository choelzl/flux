...
an error, and `evaluator/rtl` does it routinely (its metrics dict is populated only when
...
def test_refusal_on_an_empty_metrics_dict():
    """`evaluator/rtl`'s real shape for any metric other than latency_cycles — the case that
