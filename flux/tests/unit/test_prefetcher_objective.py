...
def test_met_requirement_compares_against_the_incumbent_not_no_prefetcher():
    ...
    """
    from flux_prefetcher.objective import met_requirement
    ...
    assert met_requirement(shipped, shipped) is False, "returning the incumbent is not an improvement"
    assert met_requirement(better, shipped) is True


def test_without_an_incumbent_measurement_nothing_is_claimed():
    """No reference on this stage means the question is unanswered, not answered `yes`."""
    from flux_prefetcher.objective import met_requirement

    only_decision = score({b: BASE.ipc[b] * 1.20 for b in BENCHMARKS}, BASE, storage=1)
    assert met_requirement(only_decision, None) is False
...
def test_a_worse_decision_than_the_incumbent_is_not_an_improvement():
    ...
    assert met_requirement(smaller, shipped) is False
