...
from flux_prefetcher.measure import Measurer, _score_designs  # noqa: E402
...
def test_each_stage_divides_by_its_own_baseline():
    ...
    absolutes were fiction. Since D446 the stage IS the argument: `PrefetcherProblem.measure_batch`
    picks the baseline belonging to the stage it was asked for, and measures the confirm stage's
    own denominator the first time that stage runs.
    """
    from flux_loop import LoopRequest, LoopState
    from flux_prefetcher.world import CONFIRM_STAGE, SCREEN_STAGE
    from prefetcher_fixtures import prefetcher_problem

    problem = prefetcher_problem().world
    problem.traces = TRACES
    problem.screen = problem.decide_stage = _measurer(SpyBackend())   # ipc 1.05 for everything
    problem.baseline = Baseline(ipc={b: 0.5 for b in BENCHMARKS})    # the screen's denominator
    problem.full_baseline = Baseline(ipc={b: 1.0 for b in BENCHMARKS})
    state = LoopState(request=LoopRequest(), say=lambda _m: None, proposer=None, feedback=None)
    cand = problem._cand((DEFAULT, ("bingo",), {}), "incumbent")

    confirmed = problem.measure_batch([cand], CONFIRM_STAGE, state)
    assert abs(confirmed[0]["geomean_speedup"] - 1.05) < 1e-9, "the FULL baseline (1.0)"
    screened = problem.measure_batch([cand], SCREEN_STAGE, state)
    assert abs(screened[0]["geomean_speedup"] - 2.10) < 1e-9, "the screen's own (0.5)"


def test_the_job_carries_the_binary_the_stage_was_built_for():
