"""Unit tests for flux_search_exhaustive.annealing (folded from flux_search_annealing, D434): propose/observe/done control flow and the
...
from flux_search_exhaustive import SearchState, SimulatedAnnealingMappingStrategy, run_simulated_annealing
...
class _MetriclessEvaluator:
    """Mirrors `evaluator/rtl/adapter.py` exactly: it fills `result_metrics` only when
