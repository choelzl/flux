...
def test_the_integration_sweep_globs_test_files_and_only_test_files(workflow):
    ...
    `test_*.py`, not `*.py`: the wider glob fed conftest.py and _helpers.py to pytest, which
    collects no tests from them and exits 5 — every nightly the repo ever ran failed on that
    arithmetic before any real test executed (docs/decisions.md D371). This test used to PIN
    the wide glob; a guard that enforces a bug is how the bug survives eighteen red runs."""
