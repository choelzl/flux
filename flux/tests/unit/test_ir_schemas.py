...
proving docs/00-decisions.md D1 (general-SoC, not DNN-only) is actually representable, not just
asserted in prose.
"""
...
def test_mapping_not_expressible_in_is_symmetric_between_dnn_and_soc_examples():
    ...
    mapping is inexpressible in Timeloop (docs/04.md §3.3), and a general-SoC compute_kernel
    mapping is inexpressible in either DNN-only tool (docs/00-decisions.md D1)."""
    mapping_examples = FLUX_ROOT / "ir/mapping/examples"
