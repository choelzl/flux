...
BACKENDS = ["zigzag", "timeloop", "rtl", "openroad", "champsim_bingo"]
...
def test_made_evaluator_carries_its_registry_name():
    ev = make_evaluator("rtl")
    assert ev.name == "rtl"
    assert evaluator_name_for("rtl@0.1") == "rtl"


def test_provenance_prefix_prefers_the_longest_name(monkeypatch):
    monkeypatch.setattr(_registry, "_FACTORIES", {})
    register_evaluator("champsim", lambda: object())
    assert evaluator_name_for("champsim_bingo@1") == "champsim_bingo"
    assert evaluator_name_for("champsim@1") == "champsim"
