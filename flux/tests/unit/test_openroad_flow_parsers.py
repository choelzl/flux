...
def test_the_full_abc_mapping_is_the_flow_and_the_fingerprint_says_so():
    """D564: every entry of the flow maps in full by default; the recipe is part of the
    measuring fingerprint, so numbers cached under `-fast` are never taken for these."""
    import inspect

    from flux_evaluator_abi.toolchain import FLOW_RECIPE, toolchain_fingerprint
    from flux_evaluator_openroad.flow import _yosys_synth, run_ppa_flow, run_synthesis_flow
...
def test_every_openroad_run_is_given_its_threads(monkeypatch):
    """D570: OpenROAD ran on one core of sixty-four (its default); every run now carries
    `-threads`, from the environment or the box's cores over the pool's placements."""
    from pathlib import Path

    from flux_evaluator_openroad.flow import _openroad_cmd, openroad_threads

    monkeypatch.delenv("FLUX_OPENROAD_THREADS", raising=False)
    n = openroad_threads()
    assert 1 <= n <= 16
    cmd = _openroad_cmd("openroad", Path("/tmp/x.tcl"))
    assert cmd[:2] == ["openroad", "-no_init"] and cmd[cmd.index("-threads") + 1] == str(n) and cmd[-2:] == ["-exit", "/tmp/x.tcl"]
    monkeypatch.setenv("FLUX_OPENROAD_THREADS", "3")
    assert openroad_threads() == 3
