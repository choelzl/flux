...
def test_escalation_runs_only_on_the_winner():
    ...
    class _RecordingEvaluator:
        def evaluate(self, candidate, budget, metrics):
            ...
            return _result(1000.0 / width, evaluator="escalation-stage@0.0.0")
    ...
    assert report.escalation[0].stage == "coarse"
...
def test_escalation_records_a_failure_without_crashing():
    ...
    class _RefusingEscalation:
        def evaluate(self, candidate, budget, metrics):
            raise ValueError("escalation stage can't express this")
...
def test_to_dict_is_json_safe_and_round_trips_the_winner():
    """interfaces/mcp/'s flux_search tool returns exactly this dict to an MCP client — it must be
    ...
    report = run_architecture_dse(
        _WORKLOAD, _candidates([4, 8, 16]), evaluator,
        escalation_evaluators=[("rtl", _RtlStage())],
    )
    d = report.to_dict()
    ...
    assert decoded["escalation"][0]["stage"] == "rtl"
...
def test_no_budget_runs_every_stage_and_reports_not_stopped_early():
    evaluator = _WidthProportionalEvaluator()
    ...
    report = run_architecture_dse(
        _WORKLOAD, _candidates([4, 8, 16]), evaluator,
        escalation_evaluators=[("coarse", _Stage()), ("fine", _Stage())],
    )
...
def test_zero_wall_clock_budget_skips_every_escalation_stage():
    """The real, direct proof the budget is a genuine, enforced stopping condition for
    escalation: a 0.0s budget must stop before even the first stage's own evaluator is called —
...
def test_a_real_wall_clock_budget_stops_escalation_partway_through():
    """A real, if coarse, timing-based check: a budget between two stages' real durations must let
    the first run and stop before the second."""
    import time as _time

    evaluator = _WidthProportionalEvaluator()

    class _SlowSecondStage:
        def __init__(self):
            self.calls = 0
    ...
    stage = _SlowSecondStage()
    report = run_architecture_dse(
        _WORKLOAD, _candidates([4, 8, 16]), evaluator,
        escalation_evaluators=[("coarse", stage), ("fine", stage)],
        wall_clock_budget_s=0.03,
    )
    assert report.stopped_early is True
    assert len(report.escalation) == 1  # the first (slow) stage ran; the second was cut off
...
def test_a_stage_that_refuses_one_candidate_does_not_mix_fidelities():
    """D112 grouped escalation steps by inferring stage position from append order ("same name as
    the last bucket, and this candidate isn't in it yet"). That inference holds only while every
    stage measures every contender: a stage that refuses one leaves a gap, the next stage's first
    step slots into the previous stage's bucket, and the cross-fidelity comparison D112 exists to
    prevent happens anyway.

    Both stages are named "rtl" (two configs — the exact case D112 called out). The coarse stage
    refuses width 2 and reports 4000.0 for width 4; the deep stage measures both and says width 4
    (4.0) beats width 2 (5.0). The bug reported width 2, having compared 4000.0 against 5.0
    across stages — and the *complete* deep stage was split across two buckets and never used.
    """
    class _Coarse:
        def evaluate(self, candidate, budget, metrics):
    ...
    # Every step carries the stage it actually came from, rather than one inferred later.
    assert [s.stage_index for s in report.escalation] == [0, 0, 1, 1]
...
def test_agreement_is_measured_against_the_winners_own_escalation():
    ...
    report = run_architecture_dse(
        _WORKLOAD, _candidates([2, 4]), _overlapping_screen(),
        escalation_evaluators=[("rtl", _Stage())], escalate_contenders=True,
    )
...
def test_agreement_is_none_when_no_stage_measured_the_winner():
    """The winner refused by every stage is "can't tell", not "disagrees" — the other contender's
