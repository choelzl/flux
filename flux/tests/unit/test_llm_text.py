...
two of them without knowing the other two existed, so `interfaces/chia_nodes`' generators kept an
...
def test_any_object_with_propose_satisfies_the_protocol():
    """`Proposer` is structural on purpose (D508) -- a test's stub or a scripted proposer
    stands in for a server without this package knowing about either."""
