...
class _MetriclessEvaluator:
    """Mirrors `evaluator/rtl/adapter.py`: metrics are populated only for `latency_cycles`, so
...
def test_the_three_mapping_strategies_share_one_engine():
    """`SearchState`, `EvaluatedCandidate` and the drive loop are `flux_search_exhaustive.
    engine`'s (D429); annealing and agentic import them, and agentic's fallback-carrying
    entry is a subclass, so one `isinstance` covers every strategy's outcomes."""
    import flux_search_exhaustive.annealing as ann
    import flux_search_agentic.strategy as ag
    from flux_search_agentic._engine import _EvaluatedEntry, drive_propose_observe_loop as ag_drive
    from flux_search_exhaustive import EvaluatedCandidate, SearchState, drive_propose_observe_loop
    from flux_search_exhaustive.engine import EvaluatorProtocol

    assert ann.SearchState is SearchState and ag.SearchState is SearchState
    assert ann.EvaluatedCandidate is EvaluatedCandidate
    assert issubclass(_EvaluatedEntry, EvaluatedCandidate)
    assert ann.drive_propose_observe_loop is drive_propose_observe_loop
    assert ag_drive is not drive_propose_observe_loop          # the agentic wrapper keeps its signature
    assert ag._EvaluatorProtocol is EvaluatorProtocol
    e = _EvaluatedEntry(candidate=None, result=None, error="x", used_fallback=True, fallback_reason="r")
    assert e.error == "x" and e.used_fallback
    ...
    s = Strategy()
    stopped, secs = drive_propose_observe_loop(s, SearchState({}, {}, "op"), Evaluator(), "m")
    assert not stopped and secs >= 0 and s.seen[0] == 0 and isinstance(s.seen[1], ValueError)
    s = Strategy()
    stopped, _ = drive_propose_observe_loop(s, None, Evaluator(), "m", wall_clock_budget_s=0.0)
    assert stopped and s.seen == []
