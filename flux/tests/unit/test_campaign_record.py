...
why, and which stage produced each number all vanished when the process exited.
...
def test_invention_runs_before_the_simulator_is_built(tmp_path, monkeypatch):
    ...
    problem = loop.PrefetcherProblem(
        PrefetcherRequest(db=str(tmp_path / "x.db"), invent_rounds=2, include_invented=False,
                          screen_only=True), invent=fake_invent)
    ...
    state = LoopState(request=LoopRequest(), say=lambda m: order.append(("say", m[:40])),
                      proposer=None, feedback=None)
...
def test_the_cache_is_keyed_on_the_stock_binary_even_when_a_rebuild_runs(tmp_path, monkeypatch):
    ...
    problem = loop.PrefetcherProblem(PrefetcherRequest(db=str(tmp_path / "x.db"),
                                                       include_invented=True, screen_only=True))
    problem.prepare(LoopState(request=LoopRequest(), say=lambda m: None, proposer=None,
                              feedback=None))
