...
def test_the_standings_of_a_search_carry_its_designs_with_their_numbers(monkeypatch):
    """D573 (Cedric: the macarray's results tab should show the good designs with their
    metrics): a search pass publishes the frontier on the deepest stage, each design with the
    objectives' numbers and its text, the decision marked; the tab renders them and opens
    them."""
    import flux_profile
