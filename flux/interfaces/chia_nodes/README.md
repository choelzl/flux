# interfaces/chia_nodes/ — Flux evaluators as real CHIA library nodes
...
registry `interfaces/cli` uses. Call it directly for a local, in-process evaluation, or via
...
`evaluator/rtl`'s own real self-check rather than discarding it —
...
of its three spatial dims (`docs/decisions.md` D24): `evaluator/timeloop`'s translator now
...
gap: `interfaces/mcp`'s `agentic_dse_loop` MCP tool had never been updated for `axis="memory_size"`
...
Not implemented: Dockerfiles per evaluator backend (gap-analysis G11); `flux_evaluate` doesn't
yet resolve `Candidate.workload`/`arch` from a `flux_store.ResultStore` hash (only inline IR
dicts) — same v0.1 limitation `interfaces/cli` already has; `flux_calibrate`/`flux_conformance_check`
...
**Update — the MCP-tool surface is now built.** `interfaces/mcp/`'s `FluxTool` exposes every
