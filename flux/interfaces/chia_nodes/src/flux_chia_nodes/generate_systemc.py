...
is the same backend `interfaces/chia_nodes/agentic.py`'s five DSE strategies already use, so this node
...
from flux_llm import ask_until, default_local_model, strip_markdown_fence
...
def _module_prompt(spec: DesignSpec, guidance: str | None = None) -> str:
    port_lines = "\n".join(
        f"  {'sc_in' if p.dir == 'in' else 'sc_out'}<{cpp_type(p)}> {p.name};" for p in spec.ports
    )
...
def flux_generate_systemc_module(
    ...
    # `flux_llm.ask_until` (D450) is the round; the compiler, the prompt and what passing
    # means are this module's. The RTL twin is the same five lines with Verilator in them,
    # which is what makes the mirroring D429 deferred cheap to live with.
    held: dict[str, HarnessRunResult | None] = {"result": None}

    def verify(source: str):
        try:
            held["result"] = compile_and_run(source, design_spec)
        except CompileError as exc:
            held["result"] = None
            return (f"g++ compile error:\n{exc.stderr[-2000:]}", "compile error")
        run = held["result"]
        if run.all_passed:
            return ""
        return (f"{run.passed_vectors}/{run.total_vectors} test vectors passed. "
                f"Failing vectors: {'; '.join(run.failing_vector_lines)}",
                "verification failure")

    got = ask_until(
        _module_prompt(design_spec, guidance=guidance),
        ask=lambda p: llm.prompt(p).result,
        parse=strip_markdown_fence, check=verify,
        repair=lambda source, why, _reply: _repair_prompt(design_spec, source or "", why),
        attempts=max_repair_attempts)
    return GenerationResult(
        spec_id=design_spec.id, success=got.ok, attempts=got.attempts,
        final_source=got.value or "", harness_result=held["result"],
        transcript=list(got.transcript),
    )
