...
def flux_generate_rtl_module(
    ...
    # The bounded ask-check-repair round is `flux_llm.ask_until` (D450); what is this
    # module's own is the prompt, the fence stripping, and what "it works" means here:
    # Verilator compiles it and every golden vector passes.
    ...
    def verify(source: str):
        try:
            ...
            held["result"] = None
            return (f"verilator compile error:\n{exc.stderr[-2000:]}", "compile error")
        run = held["result"]
        if run.all_passed:
            return ""
