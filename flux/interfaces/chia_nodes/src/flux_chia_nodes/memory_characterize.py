...
Architecture IR documents and `evaluator/cacti`'s own single-macro contract (docs/decisions.md
...
`evaluator/cacti`'s own tests already does by hand (`_gbuf_only_arch` in
...
def flux_characterize_memory_level(
    ...
    *,
    ...
    multi-node hierarchy outright, as `evaluator/cacti` does), inject `word_width_bits` (a real
    physical property `size_kb` alone can't determine — see `evaluator/cacti/README.md`'s module
    ...
    `energy_pj`) `flux_evaluate`/`flux_check_validity` default to — `evaluator/cacti` doesn't
