...
Wraps `interfaces/cli`'s evaluator registry (the same `make_evaluator(backend)` the `flux eval` CLI
...
third, the MCP-tool surface, wraps this same function — see `interfaces/mcp/` (`FluxTool`).
"""
