...
class GemmRtlReport:
    ...
    `measured_cycles` is directly comparable to what `evaluator/rtl` reports for the same pair —
...
def flux_generate_gemm_rtl_for_architecture(
    ...
    *,
    ...
    `evaluator/rtl`'s own `mac_array.sv` runs — so the measured cycle count is comparable to that
