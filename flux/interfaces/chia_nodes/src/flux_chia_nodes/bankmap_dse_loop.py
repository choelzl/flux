...
def flux_bankmap_dse_loop(
    ...
    *,
    ...
    from ._loop_glue import loop_report, optional_proposer

    from flux_llm import local_proposer

    proposer = optional_proposer(llm_round, lambda: local_proposer())
    result = run_study(request, proposer=proposer, feedback=feedback)
    n = request.concurrent
    return loop_report(result, lambda m: _mapping_payload(m, request), refused_key="mapping",
                       frontier_from=None, extra={
