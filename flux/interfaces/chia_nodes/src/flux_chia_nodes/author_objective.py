...
- "backends": {{"screening": <backend name>, "escalation": [<backend names in stage order>]}}.
  Screening should be a fast analytic backend ("zigzag"); escalation stages may include "rtl"
...
- "search": one of
    ...
    {{"kind": "composition_system", "widths": [<ints>], "level": <memory level name>,
      ...
      "word_width_bits": <int> when an "area_mm2" objective uses a "cacti" escalation stage
...
def _check_backend_capabilities(doc: dict[str, Any]) -> None:
    ...
    stages = list(doc["backends"].get("escalation") or ())
        ...
        if o.get("measured_at", "screen") == "screen":
            if screening in _BACKEND_METRICS and metric not in _BACKEND_METRICS[screening]:
                raise ValueError(
                    ...
                    "escalation stage that measures it (e.g. \"openroad\" for area/power)"
                )
        else:
            known_stages = [r for r in stages if r in _BACKEND_METRICS]
            if known_stages == stages and not any(
                metric in _BACKEND_METRICS[r] for r in stages
            ):
                raise ValueError(
                    f"objective {metric!r} is measured_at=escalation but no escalation stage in "
                    f"{stages} produces it — add a stage that does (e.g. \"openroad\" for "
