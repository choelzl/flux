...
def flux_search(
    ...
    *,
    ...
    names, in order, the stages the winner is confirmed against (e.g. `["systemc", "rtl"]`) — each
    ...
    *escalation* cascade only — screening's own batched/parallel dispatch isn't interruptible the
    ...
    measured elapsed time before each escalation stage's own evaluator call; `report.stopped_early`
    is `True` when the budget cut escalation short (the winner and its screening result are always
    complete either way — only later stages may be missing).
    """
