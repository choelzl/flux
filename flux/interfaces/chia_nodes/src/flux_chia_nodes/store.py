...
def flux_leaderboard(corpus_root: str, entry_id: str, db_path: str) -> list[dict[str, Any]]:
    ...
    `repo_root` for resolving `entry.workload_path` is derived from `corpus_root` -- the nearest
    ancestor under which the entry's repo-relative paths actually exist -- not a second,
    separately-passed path a caller could get out of sync with the first. It used to be
    `corpus_root`'s parent, true while the corpus lived at `<repo_root>/corpus`; the corpus
    lives at `<repo_root>/mentor/benchmarks` since the D426 restructure, and a convention that
    encodes a directory depth rots with the next move (found by the nightly, D470).
    """
    ...
    with ResultStore(db_path) as store:
        standings = rank_results_for_entry(store, entry, repo_root=_repo_root_of(corpus_root, entry))
    return [s.to_dict() for s in standings]


def _repo_root_of(corpus_root: str, entry) -> Path:
    """The ancestor of `corpus_root` where `entry.workload_path` resolves; the parent when
    none does (the old layout, and a corpus whose paths are simply wrong -- the loader's
    error then names the path it tried)."""
    root = Path(corpus_root).resolve()
    for cand in root.parents:
        if (cand / entry.workload_path).is_file():
            return cand
    return root.parent
