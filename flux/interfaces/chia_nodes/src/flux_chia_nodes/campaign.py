...
def _default_make_llm(model: str | None):
    ...
    return structured_proposer(model)
...
def _run(store, campaign_id: str, max_trials: int | None,
    ...
    if escalation_parallelism > 1:
        # Escalation concurrency needs nothing from Ray: a measured stage shells out to
...
def flux_campaign_step(
    ...
    `escalation_parallelism` > 1 measures a stage's contenders concurrently (docs/decisions.md
    D290). This is where the wall-clock actually is: screening is instant, while a measured stage
...
def flux_campaign_frontier(
    ...
    fidelity (screen vs escalation stage). `include_contenders` adds the escalation set — every
