...
def flux_prefetcher_dse_loop(
    ...
    *,
    ...
    from ._loop_glue import optional_proposer

    # None: run_study reports "the proposer did not run" (the model role is optional, D435)
    proposer = optional_proposer(llm_round, lambda: local_proposer(num_predict=NUM_PREDICT))
    ...
    if feedback_channel is not None:
        ...
        channel.start()
    try:
        result = run_study(request, measure_batch=measure, proposer=proposer, invent=invent,
                           feedback=channel)
    finally:
