...
from flux_evaluator_abi import Budget, Candidate, Result, NotExpressibleError
...
# evaluator/rtl and evaluator/systemc both model one fixed, hand-written loop schedule and
...
# (D440: the adapter classes declare `translates`; the registry answers, no set here.)
...
def flux_agentic_dse_loop(
    ...
    *different* reason than mapping's: `rtl`/`systemc` don't reject a varying `attrs.size_kb`,
    ...
    from flux_evaluator_abi import translates

    if axis == "mapping" and "mapping" not in translates(reference_backend):
        raise ValueError(
            f"reference_backend={reference_backend!r} cannot serve as conformance ground truth "
            f"for axis='mapping': it declares no Mapping IR translation -- it models a single, "
    ...
    if (
        ...
        and "memory_size" not in translates(reference_backend)
    ):
        raise ValueError(
            f"reference_backend={reference_backend!r} cannot serve as conformance ground truth "
            f"for axis={axis!r}: it declares no memory-size translation and silently "
    ...
    except NotExpressibleError as exc:
        # The ABI's one NotExpressibleError (D426):
