...
The sequence this closes: D121 made a generated design measure what `evaluator/rtl` measures;
...
def _reference_can_express(workload: dict[str, Any], arch: dict[str, Any]) -> bool:
    """Does `evaluator/rtl` already measure this candidate? Asked by *running* its own
...
def flux_calibrate_against_generated_rtl(
    ...
    *,
    ...
    Refuses by default when `evaluator/rtl` can already measure the candidate: that residual is
    ...
    if not allow_redundant and _reference_can_express(workload, arch):
        return GeneratedReferenceReport(
            ...
            skip_reason=(
                "evaluator/rtl already measures this candidate, so the residual is already "
