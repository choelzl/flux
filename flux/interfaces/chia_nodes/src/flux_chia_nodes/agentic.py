...
`_OllamaProposer` wraps `flux_llm.NativeOllamaProposer` — real, credential-free local inference
via Ollama's native endpoint (the only one whose `think:false` reliably stops qwen3-family
reasoning, D376), never a gated cloud backend (D9). `CostTrackingProposer` (D88) is tested machinery for a
...
class _OllamaProposer:
    """Adapts `flux_llm.NativeOllamaProposer` onto `flux_search_agentic.LLMProposer`'s one-method
    ...
    def __init__(self, model: str) -> None:
        # NativeOllamaProposer, not chia's OllamaLLM: the /v1 OpenAI-compatible endpoint has no
        # way to turn reasoning off, and prompt-level /no_think is ignored by newer qwen3 builds,
        # so every proposal was buying a multi-thousand-token hidden trace at CPU speed (D376 --
        # measured at minutes per round; the native endpoint's think:false makes it seconds).
        # FLUX_LLM_THINK=1 keeps its role as the restore-reasoning A/B arm.
        import os
