...
class FluxTool(ChiaTool):
    ...
    #: the same 61 names and a node registered in one and missing from the other is invisible
    ...
    def search(
        ...
        Args:
            ...
            escalate_contenders: escalate every candidate the screening data cannot rule out
                ...
                stage; `wall_clock_budget_s` bounds it.
            ...
            wall_clock_budget_s: Real, enforced wall-clock budget (docs/decisions.md D71) for the
                *escalation* cascade only — checked before each stage's own evaluator call.
    ...
    def characterize_memory_level(
        ...
        between this repo's real, multi-level Architecture IR documents and `evaluator/cacti`'s
    ...
    def systemc_generate_dse(
        ...
        physical or cycle-accurate evaluator (unlike `evaluator/rtl`/`evaluator/systemc`'s
    ...
    def campaign_start(
        ...
        Args:
            objective: Objective IR document — objectives (metric + direction, optionally
                ...
                store refs), backends (screening + optional escalation stages), search space
    ...
    def campaign_frontier(
        ...
        vs escalation stage), and result row ids for full lineage. `include_contenders` adds the
