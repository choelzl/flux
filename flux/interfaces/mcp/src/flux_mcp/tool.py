...
class FluxTool(ChiaTool):
    ...
    def setup(self) -> None:
        ...
        self.mcp.add_tool(
            self.prefetcher_dse_loop, name=f"{self.name}_prefetcher_dse_loop"
        )
        self.mcp.add_tool(self.champsim_run, name=f"{self.name}_champsim_run")
        self.mcp.add_tool(self.bankmap_dse_loop, name=f"{self.name}_bankmap_dse_loop")
        self.mcp.add_tool(self.nlu_dse_loop, name=f"{self.name}_nlu_dse_loop")
        self.mcp.add_tool(self.macarray_dse_loop, name=f"{self.name}_macarray_dse_loop")
        self.mcp.add_tool(self.interconnect_mapping_dse_loop, name=f"{self.name}_interconnect_mapping_dse_loop")
        self.mcp.add_tool(self.omni_run, name=f"{self.name}_omni_run")
        self.mcp.add_tool(
            self.invent_prefetcher, name=f"{self.name}_invent_prefetcher"
        )
        self.mcp.add_tool(
            self.interconnect_dse_loop, name=f"{self.name}_interconnect_dse_loop"
        )
        self.mcp.add_tool(
    ...
    def prefetcher_dse_loop(
        ...
        Args:
            ...
            strategy: How stage 1 spends its budget. "climb" expands the fastest configuration
                ...
                policy (arXiv:2608.06183) on this study's own move generators and rungs.
    ...
    def invent_prefetcher(
        ...
        The code-space rung of the prefetcher study: everything else searches the parameters of
        ...
        Args:
            ...
            confirm_best: Re-measure the best design at full length before declaring anything. The
                ...
                the expensive rung answers. Only a design that beat the reference on the screen
                is confirmed.
        ...
        Returns:
            ...
            All numbers are from the cheap screening rung and order candidates rather than
            settling them.
        """
    ...
    def nlu_dse_loop(
        ...
        Args:
            ...
            llm_rounds: Model design rounds; 0 re-judges the record with no model.
        ...
        return flux_nlu_dse_loop(
            db_path, ops=ops, ulp_budget=ulp_budget, llm_rounds=llm_rounds,
    ...
    def macarray_dse_loop(
        ...
        Args:
            ...
            preserve_fmax: Area is the objective and the incumbent's clock the floor: the target
                becomes the incumbent's own measured fmax on each rung and the decision is the
        ...
        Returns:
            ...
            the fmax-vs-area frontier on the confirmed rung; what was refused and why.
        """
    ...
    def interconnect_dse_loop(
        ...
        A larger design has an interconnect in it; this is how the orchestrator responsible for
        that design asks for one, rather than running a script and parsing its output.

        Args:
            db_path: Where the campaign accumulates; re-running resumes it.
            problem: The requirement in plain language, e.g. "28 masters into 32 banks, 128-bit,
                must run at 600 MHz". Either this or the explicit numbers below.
            clients: Number of client ports.
            banks: Number of memory banks every client must reach.
            width_bits: Datapath width.
            target_mhz: The frequency the fabric must meet. A hard constraint.
            bank_rows: Physical row count used when placing banks.
            max_rounds: Runaway guard on a directed run.
            rounds: How many widening rounds to run.
            budget: Evaluation grant per round.
            llm_round: Fabrics proposed by a local model; 0 for a deterministic run.
            decide_on_finalists: Place this many top fabrics WHOLE to decide between them. The
                cheap screen mis-ranks, so 0 makes the run fast and its frequency claim unmeasured
                — which the result reports rather than hiding.

        Returns:
            The decision, the placed finalists, what was refused and why, and what the run could
            not establish.
        """
        return flux_interconnect_dse_loop(
            db_path, problem=problem, clients=clients, banks=banks, width_bits=width_bits,
            target_mhz=target_mhz, bank_rows=bank_rows, max_rounds=max_rounds, rounds=rounds,
            budget=budget, llm_round=llm_round, decide_on_finalists=decide_on_finalists,
        )
