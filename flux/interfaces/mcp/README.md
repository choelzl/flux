# interfaces/mcp/ — the third surface: every flux_chia_nodes function as a real MCP tool

docs/agent-surface.md: "One definition, three surfaces" — a typed Python function, a CHIA
`@ChiaFunction` node, and an MCP tool, generated together. `interfaces/chia_nodes/` builds the first
...
- `calibrate`/`conformance_check`/`check_validity` default to a relative `calibration_db_path` —
  ...
  `interfaces/chia_nodes/README.md` notes for the underlying nodes.
