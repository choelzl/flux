...
neither does synthesis (`evaluator/hammer/` is still an empty directory) — but RTL simulation
now does (`evaluator/rtl/`, a real Verilator adapter). Findings 1-6 below predate it: building
...
is the first real attempt at that second question, using `evaluator/rtl/`'s real measurement
...
principles call a bug. `next_stage` always named `rtl_sim (not implemented)` at the time this
finding was written, since `evaluator/rtl/` didn't exist yet — the signal was real, the automated
follow-through wasn't. *(Update: `evaluator/rtl/` now exists and is wired into calibration via
`reference_source="rtl_sim"` — see `calibration/README.md` and
`tests/integration/test_calibration_against_real_rtl.py`. The synthesis-fidelity stage above it,
`evaluator/hammer/`, is still unbuilt, so `next_stage` still has nowhere further to go once `rtl`
itself is exhausted.)*
...
`evaluator/zigzag/architecture_translator.py` used to give every memory a flat, explicitly-fake
...
independently verified. `evaluator/rtl/`'s real Verilator simulation of a hand-written
...
Escalation now has somewhere real to escalate *to* (`evaluator/rtl/`, registered as `"rtl"` —
Finding 7 uses it directly), but nothing in this repo *automatically* walks a `Result`'s
`escalation.next_stage` and invokes it yet — that wiring (presumably a CHIA node, docs/calibration.md)
doesn't exist.
