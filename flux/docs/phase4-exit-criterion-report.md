...
`interfaces/chia_nodes/src/flux_chia_nodes/dse_loop.py`'s `flux_agentic_dse_loop`
...
loop runs somewhere that has one (see `interfaces/mcp/README.md`'s "No container/deployment story"
gap).
...
introducing new search/evaluation logic. It does **not** include the synthesis stage
(`evaluator/hammer/` is still blocked on tooling, per its README) in this loop shape — that
remains natural future work once `evaluator/hammer/` is unblocked, not required to call this
...
`interfaces/chia_nodes/src/flux_chia_nodes/dse_loop.py` for the full write-up.

**Update ([decisions.md D24](../../docs/decisions.md))**: the mapping axis's half of that finding
is closed. `evaluator/timeloop`'s translator now forces its architecture-side spatial constraint
...
**Update ([decisions.md D25](../../docs/decisions.md))**: a real `evaluator/booksim` bug was
...
real on the first attempt: `evaluator/timeloop`'s translator reads `attrs.size_kb` generically,
...
also found and fixed a real, separate gap: `interfaces/mcp`'s `agentic_dse_loop` MCP tool had never
...
`search/agentic/README.md` and `interfaces/chia_nodes/src/flux_chia_nodes/dse_loop.py` for the full
write-up.
