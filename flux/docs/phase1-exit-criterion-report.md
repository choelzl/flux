...
[docs/roadmap.md Phase 1](../../docs/roadmap.md#phase-1--spine-68-weeks) sets this exit criterion: *"Take
...
Both `evaluator/zigzag/architecture_translator.py` and
`evaluator/timeloop/architecture_translator.py` translated this *same* document into their
...
`evaluator/zigzag/architecture_translator.py` gave every translated memory a flat, explicitly-fake
...
2. **Objective function mismatch** (`evaluator/timeloop/reference/mapper.yaml`'s `edp` vs
...
3. **Confirmed, quantitatively.** ZigZag's cost model charges cycles for memory-access latency
   ...
   contract (docs/gap-analysis.md G1, G4) predicts once you can actually run the same IR through two
   backends.
4. **Is the auto-search leaving an easy win on the table? Tested directly, once Mapping IR
   translation existed to test it with (`evaluator/zigzag/mapping_translator.py`, added after
...
6. **So which analytic estimate is actually closer to real hardware? Checked directly, with a
   real Verilator simulation, not another analytic model.** `evaluator/rtl/`'s hand-written
...
- **The Evaluator ABI is genuinely backend-agnostic** at the call-site — driving both evaluators
  ...
  substitutability docs/architecture.md principle 1 ("contracts over monoliths") is meant to buy.
...
- **No calibration against real silicon or RTL exists** (Phase 2, docs/roadmap.md) — neither number
...
- **Mapping IR translation exists for both backends now**, but each only for its own narrow,
  ...
  (`evaluator/zigzag/mapping_translator.py`) is a single shared flat (unblocked,
  single-loop-per-dim) temporal *and* spatial order across all operands. Timeloop's
  (`evaluator/timeloop/mapping_translator.py`) is temporal-only — its spatial mapping stays
  ...
  can express that Timeloop can't" comparison (the docs/ir.md canonical case) still isn't
...
`evaluator/zigzag/architecture_translator.py` was built first, translating a narrow Architecture
