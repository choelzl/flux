...
_PROTO_CONTRACT = """\
THE STAGE: a PYTHON PROTOTYPE of `{op}`. The RTL module nlu_{op} is TRANSPILED by the
...
def prototype_library(library: str, cap: int = 2000) -> str:
    ...
    stream write is nothing to a Python prototype), capped at `cap` characters. Measured:
    7,000 of the 18,000 characters of a repair prompt were such lines."""
    if not library:
        return ""
    lines = library.splitlines()
    head = [ln for ln in lines if not ln.startswith("  * [")][:2]
    keep: list[str] = []
    size = sum(len(h) for h in head)
    for ln in lines:
...
def op_prototype_prefix(op: str, *, knowledge: str, library: str = "") -> str:
    """The prototype stage's static prefix (D491): the sheet, the library's PAPER excerpts
    (D500: capped, no source-file lines), then the PROTOTYPE contract -- not the RTL
    interface and not the "reply with a SystemVerilog module" shape, which in one prompt
    with the prototype's own reply shape had the model answer the Python stage in Verilog."""
    return "\n\n".join(p for p in (knowledge, prototype_library(library), _PROTO_CONTRACT.format(op=op)) if p)
