...
def logic_depth(code: str, *, xs: np.ndarray | None = None, entry: str = "design") -> dict[str, Any]:
    ...
    steps: list[tuple[int, str, str]] = []               # every step on the path (levels, signal, what)
    ...
    while node is not None:
        ...
        if cost[node] >= 8:
            steps.append((cost[node], node, _describe_op(by[node], by)))
        if worst is None or cost[node] > worst[0]:
            worst = (cost[node], node, _describe_op(by[node], by))
        node = pred[node]
    path = sorted(blocks.items(), key=lambda kv: -kv[1])
    out = {"depth": int(level[prog.out]), "path": path, "n": len(prog.temps)}

    def base_of(name: str) -> str:
        return re.sub(r"(__\d+)?(_\d+)?$", "", name) or name

    if worst is not None and worst[0] >= 16:
        out["worst"] = {"levels": worst[0], "signal": base_of(worst[1]), "what": worst[2]}
    # D506: the costliest steps on the path, widths named -- what to narrow first. A path of
    # 234 levels was reported as "design 90, from_fixed 29, pack_fp16 31 ..." and the model
    # could not see that a 24x32-bit product and the 56-bit normalise behind it were half of it.
    seen: set[str] = set()
    costly = []
    for lv, name, what in sorted(steps, key=lambda t: -t[0]):
        b = base_of(name)
        if b in seen:
            continue
        seen.add(b)
        costly.append({"levels": lv, "signal": b, "what": what,
                       "block": prog.owner.get(name) or (name.split("__")[0] if "__" in name else "design")})
        if len(costly) >= 4:
            break
    out["costly"] = costly
    return out
...
def _describe_op(t: _Temp, by: dict[str, _Temp] | None = None) -> str:
    """One deep operation, named for the report (D496), with its operands' widths (D506)."""
    e = t.expr

    def w_of(n: ast.expr) -> int | None:
        if by is not None and isinstance(n, ast.Name) and n.id in by:
            return by[n.id].width
        return None
    ...
    if isinstance(e, ast.BinOp):
        ...
        if isinstance(e.op, ast.Mult):
            wl, wr = w_of(e.left), w_of(e.right)
            widths = f"a {wl}x{wr}-bit multiply ({t.width}-bit product)" if wl and wr else f"a {t.width}-bit multiply"
            return (widths + " -- narrower factors (drop the bits the result never needs: an FP16 result "
                    "has 11 significant bits, so a 16-bit table value times an 11-bit significand is enough), "
                    "or a table")
        ...
        if isinstance(e.op, (ast.Add, ast.Sub)):
            return f"a {t.width}-bit add"
    if isinstance(e, ast.Compare):
        return f"a {t.width}-bit compare"
    if isinstance(e, ast.Subscript):
        return "a table read (address decode)"
    if isinstance(e, ast.Call):
        name = e.func.id if isinstance(e.func, ast.Name) else getattr(e.func, "attr", "")
        if name in ("where", "select"):
            return "a select"
        if name in ("minimum", "maximum", "clip", "abs"):
            return f"a {t.width}-bit {name}"
        return f"a {t.width}-bit {name or 'step'}"
    return type(e).__name__


def logic_depth_note(depth: dict) -> str:
    """The deepest single step on the critical path, as advice (D496), and the costliest
    steps after it with their widths (D506): what to narrow first."""
    w = depth.get("worst")
    lines = []
    if w:
        lines.append(f"the deepest single step is `{w['signal']}` (~{w['levels']} levels): {w['what']}")
    rest = [c for c in depth.get("costly") or [] if not w or c["signal"] != w["signal"]]
    if rest:
        lines.append("the other costly steps on the path: " + "; ".join(
            f"`{c['signal']}` in {c['block']} (~{c['levels']} levels): {c['what']}" for c in rest))
    wide = [c for c in depth.get("costly") or [] if "-bit product)" in c["what"]]
    if wide:
        lines.append("a wide product is paid for TWICE: in the multiply, and in the normalise "
                     "(`from_fixed` counts leading zeros across the whole product); keep the product to "
                     "the bits the result needs")
    return ". ".join(lines)
