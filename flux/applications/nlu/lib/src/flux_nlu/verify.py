...
def build_sim(source: str, *, top: str, latency: int, opcode: int | None,
    ...
    # slang first, when the machine has it (D411): the strictest open-source SV
    # frontend, with diagnostics a repair prompt can actually act on -- Verilator's
    # "syntax error, unexpected IDENTIFIER" cost this campaign whole rounds where
    # slang names the construct and often the fix. Verilator still compiles what
    # slang passes; a slang-clean source that Verilator refuses is recorded as the
    # discrepancy it is.
