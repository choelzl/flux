...
```bash
nix develop --command flux task run applications/bankmap/bankmap.problem.yaml --db demo-bankmap.db --tui
```
...
`params.topology` names the network between requesters and banks; each one reduces to stages the
...
chose is then the hardware every stage checks against (D372). On the 32-bank request this
