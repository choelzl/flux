...
class Stage:
    ...
    #: then on is the concrete wiring every stage checks against.
...
class MappingRequest:
    """One bank-mapping study: the document's `params:` (review 2 step R3, the shape of D519
    and D533). The record's home is the loop's `--db`, not a field here."""
    ...
    #: The document's `params:` keys, each one line of `bankmap.problem.yaml`. The interconnect
    #: arrives as words (`topology`, with `stage_capacities` and `lanes` refining a staged tree)
    #: and as explicit sharing points (`stages`), and leaves as the `Stage`s the checker, z3 and
    #: the pigeonhole understand (D364); `notes` is what the topology assumes, never a knob.
    ...
    def from_params(cls, params: dict[str, Any]) -> "MappingRequest":
        """The request the document's `params:` mean. A key the document cannot mean is a LOAD
        error, not a knob silently ignored in the middle of a run: a copied document with a typo
        would otherwise run the default ask and report it as the one written."""
        ...
        if spec or not explicit:
            ...
            # Explicit stages and no topology named: the default crossbar's "adds no conflict
            # point" note would be printed beside a stage that IS one, which is the kind of
            # sentence a reader rightly stops trusting a report over.
