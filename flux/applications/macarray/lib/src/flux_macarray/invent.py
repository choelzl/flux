...
def parse_module(name: str, reply: str) -> tuple[str, str] | None:
    """(source, idea) from a reply, or None when no module of that name is in it. The module
    comes out of the fence by the harness's reader (D557); the IDEA line is this world's."""
...
def refusal_reason(source: str) -> str | None:
    ...
    harness's screen (sequential logic, size casts, system tasks; D557), then this world's one
    rule of its own."""
...
def check_multiplier(name: str, source: str, shape: Shape, vectors: list[dict[str, Any]],
    ...
    try:
        ...
        # D552: the line counted in the MODEL'S text (the lint pragmas are ours), quoted, with
        # a caret and a hint -- "dut.sv:13:29: syntax error" alone sent six repairs astray
        prefix = LINT_PRAGMA.count("\n") if not source.startswith(LINT_PRAGMA) else 0
        return "did not compile: " + explain_diagnostic(str(exc), source, prefix_lines=prefix)
