...
def _print_report(out: dict) -> None:
    ...
    if cfg:
        ...
        # BOTH stages on one line when the same design was screened and confirmed. Printing the
        ...
        if ref:
            ...
            verdict = "not established — the shipped configuration was not measured on this stage"
    ...
    stage = ("screened only — NOT confirmed at full length"
    ...
    for stage, key in ((1, "stage1_best"), (2, "stage2_best")):
        ...
        if best.get("config") == cfg:
            ...
            print("  per trace        " + "  ".join(
                f"{k}={v:.4f}" for k, v in best["speedups"].items()) + "   [screen stage]")
            print("  proposed by      " + best["proposed_by"])
            continue
        print(f"\nSTAGE {stage} ({goal})  [{stage}]")
...
def _print_frontier(out: dict) -> None:
    ...
    stage = "confirmed at full length" if confirmed else "SCREENED -- ordering only"
    decision = out.get("decision")
    stack = (out.get("stage2_best") or out.get("stage1_best") or {}).get("prefetchers")
    print(f"FRONTIER  speedup vs storage, {stage}; each row is faster than everything smaller")
...
def main() -> int:
    ...
    # search on the cheap stage, confirm the finalists on the expensive one. Exposing the counts
