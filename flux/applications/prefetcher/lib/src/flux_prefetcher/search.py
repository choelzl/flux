...
#: A search POLICY (D446): yields `(wave, provenance)`, receives the wave's scored designs,
#: returns what it found. The loop drives it (`flux_prefetcher.loop`), and `drive` runs it
#: synchronously against a `Measure` for tests and for callers outside the loop.
Steps = Generator[tuple[list[Design], str], list[ScoredConfig], Any]


def drive(policy: Steps, measure: Measure) -> Any:
    """Run a policy to completion with `measure`, returning what the policy returns."""
    try:
        wave, provenance = next(policy)
        while True:
            wave, provenance = policy.send(measure(wave, provenance))
...
def climb_steps(start: ScoredConfig, *, moves: Callable[[ScoredConfig], list[Design]],
                better: Callable[[ScoredConfig, ScoredConfig], bool],
                ...
                log: Callable[[str], None] = lambda _m: None, name: str = "climb") -> Steps:
    """Local search from `start`, as a policy: yield a wave of moves, keep the best of what
    comes back, repeat (D446). `climb` below is the same thing driven synchronously.
...
def climb(start: ScoredConfig, *, moves: Callable[[ScoredConfig], list[Design]],
    ...
    """`climb_steps`, driven synchronously by `measure`."""
    return drive(climb_steps(start, moves=moves, better=better, seen=seen, budget=budget,
                             wave_size=wave_size, patience=patience, provenance=provenance,
                             admit=admit, refuse=refuse, log=log, name=name), measure)
...
__all__ = [
    "Climb", "Design", "Measure", "Steps", "bingo_moves", "climb", "climb_steps", "compose_moves",
    "design_of", "drive", "faster", "faster_by", "holds_floor", "identity", "identity_of",
    "partner_moves", "shrink_spread", "smaller", "storage_bytes",
]
