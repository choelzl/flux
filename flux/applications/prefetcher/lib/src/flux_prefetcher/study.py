...
class PrefetcherRequest:
    ...
    # stage and the answer is confirmed on the expensive one. `decide_on_finalists` is how many
...
class PrefetcherResult:
    ...
    #: The shipped configuration, measured on the same stage as the decision. The reference the
    ...
    #: The IPC-vs-storage frontier on the stage this report quotes, smallest first: every design
    ...
    def improved_on_incumbent(self) -> bool:
        ...
        Both numbers must come from the same stage: an incumbent screened at 2M+3M against a
