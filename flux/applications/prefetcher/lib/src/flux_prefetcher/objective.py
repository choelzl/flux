...
def improved_on_incumbent(decision: Score | None, incumbent: Score | None) -> bool:
    """Did the search beat the configuration the project ships?

    Compared against the INCUMBENT, not against no-prefetcher. An earlier version asked
    ...
    was worth doing, and that has a `no` answer -- and no answer at all without an incumbent
    measured on the same stage (D351: a screened incumbent against a confirmed decision
    compares fidelities, not designs).
    """
    if decision is None or incumbent is None:
        return False
    return decision.geomean_speedup > incumbent.geomean_speedup


__all__ = [
    "BENCHMARKS", "RETENTION_FLOOR", "Baseline", "IncompleteMeasurement", "Score", "geomean",
    "improved_on_incumbent", "retention_threshold", "score", "stage2_admissible",
]
...
def met_requirement(decision: "Score | None", incumbent: "Score | None") -> bool:
    """Was tuning worth doing? The question is answered against the INCUMBENT measured on the
    same stage, never against no prefetcher (which Bingo beats in almost any legal
    configuration): returning the shipped configuration is not an improvement, a decision
    slower than the incumbent (stage 2 trades speed for area) is not one either, and without
    an incumbent measurement the question is unanswered, not answered yes."""
    if decision is None or incumbent is None:
        return False
    return decision.geomean_speedup > incumbent.geomean_speedup
