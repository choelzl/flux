...
loop since D446: `world.py` is the world, this module holds the search POLICIES (each stage as
a generator of waves, `flux_prefetcher.search.Steps`), the finalist and reference rules, and
the loop runs them from `world.World.search`.

Importable on purpose (D345/D346): an orchestrator running a larger design can hand this a
requirement and get a configuration back without knowing ChampSim exists. `flux task run` is one
caller; `flux_chia_nodes.prefetcher_dse_loop` is the other, and it is the one that makes the measurements
run on Ray.

MEASUREMENT IS INJECTED, not imported. `measure_batch` defaults to a local thread pool, and the
caller may pass its own. That keeps this module CHIA-agnostic:
search does not know about Ray, and
...
from .measure import Measurer, _dedupe_pairs, _label, _mark, _score_designs
from .search import (
...
def _stage1(seed_pool: list[tuple[BingoConfig, str]], traces: dict[str, Path],
    ...
    return drive(stage1_steps(seed_pool, budget, seen, refused, log, max_storage),
                 _scorer(traces, baseline, measurer, refused))
...
# ---- the study ---------------------------------------------------------------
__all__ = ["DEFAULT_TRACES", "FLUX_ROOT", "check_against_reference", "compose_steps", "reference_steps",
           "shrink_steps", "stage1_pareto_steps", "stage1_steps", "tune_steps"]
