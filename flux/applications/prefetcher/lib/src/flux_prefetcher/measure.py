...
class Recorder(Records):
    ...
    def trial(self, cfg: Any, types: Any, *, stage: str, strategy: str,  # type: ignore[override]
              metrics: dict[str, float] | None, error: str | None = None, wall_s: float = 0.0,
              **kw: Any) -> None:
        """One measured or refused candidate: Bingo-typed (`cfg`, `types`), or the loop's
        generic shape (`candidate` document, `key`) handed straight to `Records.trial` (D446)."""
        if self.store is None:
            return
        if not isinstance(cfg, BingoConfig):
            Records.trial(self, cfg, types, stage=stage, strategy=strategy, metrics=metrics,
                          error=error, wall_s=wall_s, **kw)
            return
        try:
            ...
            seq = self.store.begin_trial(
                ...
                strategy_kind=strategy, seed=None, deterministic=True, stage=stage)
            result = None
            if metrics is not None:
                result = Result(
                    ...
                    provenance=Provenance(evaluator="champsim_bingo@0.1",
                                          inputs={"stage": stage, "stack": "+".join(types)}),
    ...
    def known(self, *, stage: str = "screen", types: tuple[str, ...] = ("bingo",)
              ) -> list[tuple[BingoConfig, float]]:
        """What this campaign has already measured on `stage` for `types`, best first.
    ...
    if max_storage is not None:
        ...
        for cfg, types, knobs in designs:
            ...
            if size > max_storage:
                ...
                if recorder:
                    recorder.trial(cfg, types, stage=measurer.stage, strategy=provenance,
