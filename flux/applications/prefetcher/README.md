...
```bash
nix develop --command flux task run applications/prefetcher/prefetcher.problem.yaml --db demo-prefetcher.db --tui   # params.measurements sets the budget
```
...
`params.strategy: pareto-uct` replaces stage 1's hill-climb with a Pareto-UCT tree (D368, after
...
go on the compose menu; `params.invent_rounds: N` writes N more during a run, each asked to beat the tallest
...
| | |
|---|---|
| `world.py` | the hooks the document runs on: the search path, the gate, the two stages, the decision, the report |
| `invent.py` | the invention round the world runs before the simulator is built |
...
The evaluator lives at `applications/prefetcher/evaluator/` (it is this application's: it reads `flux_prefetcher`'s config, so it lives with it, registered by name in the ABI registry like every other evaluator, D428), and the loop at
