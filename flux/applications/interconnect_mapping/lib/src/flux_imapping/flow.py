"""The interconnect_mapping study's arithmetic: the evaluation, the little loops, the
certificates and the conclusion. The study runs on `flux_loop` as the hooks of
`world.World` (D446, D519; review 2 step R3), built from
`interconnect_mapping.problem.yaml`; this module is what those hooks compute with.

Shape (docs/decisions.md D378):
1. Curated field (`solutions.catalog`) measured on TRAIN workloads.
2. Searches extend the field: a hill-climb over injective XOR tap sets, bankmap's proven
   z3 fold, and model rounds when the document asks for them (a model may suggest tap
   sets; every proposal passes the injectivity gate and the same evaluator, or is refused
   with the reason).
...
4. Four costs per solution: A = area score (structural gate-units; the document's `phys`
   stage grounds the finalists' blocks in um2 through Yosys + OpenSTA),
...
def interconnect_loop(train: list[Workload], holdout: list[Workload], mem: Memory,
    ...
    Every screen is its own task in the TUI; `say` is the loop's log."""
