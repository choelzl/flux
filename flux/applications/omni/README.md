...
flux task run applications/omni/omni.problem.yaml --db demo-omni.db      # params.plan_file replays a saved plan, no model

# with a model: the agent plans round by round
flux task run applications/omni/omni.problem.yaml --db demo-omni.db --tui   # params.prompt: "Find the array width in {4,8,16,32} that \
  minimizes latency for the bundled GEMM workload on zigzag; report width and latency."

# see what the agent sees
flux task check applications/omni/omni.problem.yaml
```
...
1. **Catalog by introspection, never by hand.** `flux_omni.catalog` replays
   ...
   fourth surface). A hand-list would rot; there isn't one. `params.tools` narrows the
...
4. **Every run is a replayable artifact.** `omni_run.json` records the prompt, the
   ...
   file loads straight back into `params.plan_file`, so any model-authored run re-executes
...
- `world.py` -- the hooks the document runs on: the round loop, the gate, the executor as the stage, the conclusion
