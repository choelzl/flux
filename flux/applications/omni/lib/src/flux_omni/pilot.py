...
from .catalog import ToolSpec, build_catalog
...
def run_omni(
    ...
    *,
    ...
    """One omni run, ON THE LOOP (D458): `OmniProblem.search` is the round loop, the gate
    is `validate_step`, running a tool is the measurement, and `flux_loop` owns the record,
    the feedback drain, the timing tree and the step budget.
    ...
    from .problem import OmniProblem

    t0 = time.monotonic()
    workdir = Path(workdir)
    workdir.mkdir(parents=True, exist_ok=True)
    catalog = build_catalog(tools)
    problem = OmniProblem(prompt, catalog, workdir, tools=tools, max_calls=max_calls,
                          max_steps_per_round=max_steps_per_round,
                          wall_clock_budget_s=wall_clock_budget_s, started=t0)
    run_loop(problem, LoopRequest(
        db=db_path or "", steps=max_rounds, prototype=False, compute=False,
        critique_rounds=0, patching=False, params={"study": "omni"}),
        proposer=proposer, feedback=feedback)

    outcomes = list(problem.outcomes)
    conclusion, done = problem.conclusion_text, problem.done
    llm_calls = problem.llm_calls
    ...
    if problem.budget_spent and not done and outcomes and not conclusion:
        raw = proposer.propose(
            ...
            + "\n\n## Executed results\n" + "\n".join(
                ...
                for i, o in enumerate(outcomes)))
        llm_calls += 1
        problem.raw_replies.append(raw)
        conclusion = raw.strip()

    # The conclusion is omni's answer, and the record is where the next run of this same
    # prompt reads it back (D401). The loop writes a conclusion only when it DECIDED a
    # design; omni decides in words, so it writes its own.
    if problem.records is not None and conclusion:
        try:
            problem.records.conclude({"conclusion": conclusion, "done": done,
                                      "rounds": problem.rounds, "llm_calls": llm_calls})
    ...
    provenance.write_text(json.dumps({
        ...
        "refusals": [r.render() for r in problem.all_refusals],
        "raw_replies": problem.raw_replies,
        "notes": problem.notes,
    ...
    return OmniReport(
        prompt=prompt, outcomes=tuple(outcomes), refusals=tuple(problem.all_refusals),
        conclusion=conclusion, done=done, rounds=problem.rounds, llm_calls=llm_calls,
