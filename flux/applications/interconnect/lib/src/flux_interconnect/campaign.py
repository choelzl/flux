"""The interconnect application's campaign pieces (D428): the enumerated
`interconnect_topology` search and the model-proposed `generative_interconnect`
strategy, registered with `flux_search_campaign` by name so the campaign package
never imports this application. Moved here from `flux_search_campaign.strategies`,
where they were the one application-specific block in the orchestrator."""
...
from flux_search_campaign.objective import Objective
from flux_search_campaign.strategies import (Proposal, RemembersOutcomes,
                                             StrategySpec, candidate_key)

__all__ = ["InterconnectGenerativeStrategy", "STRATEGY", "interconnect_variant_label",
           "topology_candidates", "variant_label"]
...
interconnect_variant_label = variant_label      # the name the campaign package used


def topology_candidates(objective: Objective, base_arch: dict[str, Any],
                        workload: dict[str, Any] | None = None) -> list[Any]:
    """The `interconnect_topology` search kind: one candidate per declared fabric
    variant (D261), or the space DISCOVERED from the problem statement (D262). Each
    returned object has the `to_dict()` shape `_grid_proposals` wraps."""
    ...
    if variants is None:
        ...
        variants = enumerate_space(
            ...
            # `or None` would turn an EMPTY family list back into "no filter", which
            # is "enumerate everything" — the exact bug this list exists to avoid, and
            # one that already bit once inside enumerate_space itself (D309). A round
            # built from named variants asks for no families and must get none.
            families=objective.search.get("families"),
        )
...
class InterconnectGenerativeStrategy(RemembersOutcomes):
    ...
    def __init__(
        ...
        self._tried_shapes: list[str] = []
        # Fabrics a single reply offered beyond the one asked for (docs/decisions.md D338).
        # Entries are (raw spec, prompt sha, response sha) — the shas of the call that produced
        # them, so a queued proposal's provenance still points at the exchange it came from.
        self._pending: list[tuple[Any, str, str | None]] = []
        for key in self._visited:
            ...
            shape = variant_label(variant)
            if shape not in self._tried_shapes:
                self._tried_shapes.append(shape)
            try:
    ...
    def _signature(self, spec: dict[str, Any]) -> tuple:
        ...
        # THE shared definition, not a local one (D311). A proposer that decides "same fabric"
        # differently from the enumerator will re-propose what the space already holds, or refuse
        # something genuinely new.
        from flux_interconnect.perturb import structural_key_of

        return structural_key_of(topo)
    ...
    def _tried_note(self, limit: int = 14) -> str:
        """The SHAPES already tried, not just how many (docs/decisions.md D331).

        `_coverage_note` says "xbar_staged x900", which tells a model where the space is thick and
        nothing about what to avoid. The dedup check then refused its proposal as "structurally
        identical to a fabric already proposed" — twelve times out of a hundred and twenty-three
        generative trials, each one a real model call and a screening slot spent re-proposing
        something the store already held.

        Naming all of them would drown the prompt, which is why only counts were shown. But a
        model does not re-propose a random fabric out of thousands; it re-proposes the OBVIOUS
        ones, and those are few. The shallowest shapes are listed, because a two-stage fabric over
        28 clients is what anyone reaching for a first answer writes down.
        """
        if not self._tried_shapes:
            return ""
        # Only the KINDS this prompt actually invites. Ranking the whole store by depth surfaced
        # meshes and rings, which the proposal format does not offer and the model therefore never
        # duplicates — the list has to be about what it is likely to write, not about what exists.
        offered = ("xbar_staged", "clos", "butterfly", "hybrid")
        plausible = [s for s in self._tried_shapes if s.startswith(offered)]
        # MOST RECENT first. Three rankings were tried before this one — shallowest, then
        # filtered-shallowest, then spread-across-depths — and each was a guess at which shapes a
        # model would re-propose, with no evidence behind it. `_visited` is built in store order,
        # so the tail is what this study has been working on lately, which is a real signal rather
        # than a theory about the model's taste.
        #
        # It is still a heuristic: it cannot know that the prompt's own worked example is the
        # likeliest thing to be repeated. What it does guarantee is that the fabrics the search
        # just tried are named, which is where a proposer asked to continue is most likely to land.
        shortest = list(reversed(plausible))[:limit]
        if not shortest:
            return ""
        return ("ALREADY TRIED — proposing any of these again is refused, and the call is "
                "wasted:\n  " + "\n  ".join(shortest) + "\n")
    ...
    def propose(self) -> Proposal | None:
        ...
        # SERVE WHAT IS ALREADY IN HAND before paying for another call. A model asked for one
        # fabric often returns several; this used to be refused as ambiguous, which spent an
        # evaluation on a punctuation habit AND threw away work the model had already done. The
        # local model is about three quarters of a run's wall clock (D336), so a reply carrying
        # six fabrics is five calls that do not have to happen.
        while self._pending:
            raw_spec, queued_prompt_sha, queued_response_sha = self._pending.pop(0)
            try:
                spec = self._validate(raw_spec)
            except Exception:  # noqa: BLE001 — a bad extra is dropped, not repaired: it was
                continue       # never the model's headline answer and cost nothing to skip
            return self._as_proposal(spec, prompt_sha=queued_prompt_sha,
                                     response_sha=queued_response_sha)
        ...
        try:
            ...
            if isinstance(parsed, list) and len(parsed) > 1:
                # The first is the answer; the rest are queued rather than discarded. Refusing a
                # multi-fabric reply was 8 of 26 fallbacks in a measured run (D331).
                self._pending.extend((extra, prompt_sha, response_sha) for extra in parsed[1:])
                parsed = parsed[0]
            spec = self._validate(parsed)
        ...
        return self._as_proposal(spec, prompt_sha=prompt_sha, response_sha=response_sha,
                                 used_fallback=used_fallback, fallback_reason=fallback_reason,
                                 repair_note=repair_note)

    def _as_proposal(self, spec: dict[str, Any], *, prompt_sha: str,
                     response_sha: str | None, used_fallback: bool = False,
                     fallback_reason: str | None = None,
                     repair_note: str | None = None) -> Proposal:
        """One place that turns a validated spec into a Proposal.

        Both paths reach it — the reply just parsed, and one taken off the queue — and they must
        agree on everything a trial records. Two copies of this is how the dedup signature and the
        stored one drifted apart before (D302).
        """
        import copy
...
#: What the campaign resolves for `strategy.kind == "generative_interconnect"`. Visited
#: keys are STORE-WIDE, not just this campaign's: a proposal costs a model call, and one
#: that a sibling campaign already measured costs that call and adds nothing (D300).
STRATEGY = StrategySpec(InterconnectGenerativeStrategy, needs_llm=True, store_wide_visited=True,
                        search_kinds=frozenset({"interconnect_topology"}))
