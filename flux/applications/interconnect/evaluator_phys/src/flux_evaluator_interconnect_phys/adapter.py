...
stage that measures real ASAP7 silicon.
...
than chance. A stage that measured area and frequency while inheriting the modelled throughput
...
from flux_interconnect import abi_result as _result
...
class InterconnectPhysicalEvaluator(SequentialBatch):
    ...
    def __init__(self, *, target_period_ps: float = _TARGET_PERIOD_PS, cache: Any = None) -> None:
        """`cache`: a `flux_cache.MeasurementCache` makes the fits and throughputs below
        toolchain-keyed and persistent (D436); without one they are memoised in-process,
        single-flight per key -- several fabrics escalated concurrently would otherwise each
        start the same three-placement fit and each pay for it."""
        from flux_cache import SingleFlightMemo

        self._target_period_ps = target_period_ps
        self._fit_memo = SingleFlightMemo(cache, namespace=f"interconnect_phys/fit@{target_period_ps:g}/")
        self._throughput_memo = SingleFlightMemo(cache, namespace="interconnect_phys/throughput/")
    ...
    def _measure_throughput(self, topo) -> tuple[float, str]:
        ...
        got = self._throughput_memo.get_or_measure(key, lambda: self._measure_throughput_uncached(topo))
        return got[0], got[1]

    def _measure_throughput_uncached(self, topo) -> tuple[float, str]:
        ...
        try:
            ...
            # timing, and the only thing that ever contradicted it was the decision stage failing
    ...
    def _fit_block(self, inputs: int) -> tuple[tuple[float, float], tuple[float, float]]:
        ...
        got = self._fit_memo.get_or_measure(inputs, lambda: self._fit_block_uncached(inputs))
        return (got[0][0], got[0][1]), (got[1][0], got[1][1])     # lists back from a cache
    ...
    def _fit_block_uncached(self, inputs: int) -> tuple[tuple[float, float], tuple[float, float]]:
        ...
        for width in _CANDIDATE_FIT_WIDTHS:
            ...
            report = run_ppa_flow(
                ...
                # stage it feeds have to be synthesised the same way or the screen is ranking
        ...
        return (_linear_fit(areas), (intercept, _WIDTH_DELAY_SLOPE_PS))
