...
A second, independent implementation of docs/04.md §6's `Strategy` Protocol
...
Deterministic: every run takes an explicit `seed` (docs/04.md §9's "explicit seeds everywhere"),
...
Not implemented: warm-start against `flux_store.ResultStore`; parallel/multi-chain annealing;
budget-aware stopping (`max_iterations` is a fixed count, not tied to `Budget.wall_clock_s` or
`Budget.usd` yet).
