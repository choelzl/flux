...
space (`generate_flat_mapping_candidates`); `strategy.py` implements docs/04.md §6's
...
Not implemented: warm-start against `flux_store.ResultStore` (every candidate is evaluated fresh,
every run); `evaluate_batch` (candidates are evaluated one at a time via `evaluate`); multi-op
...
rather than silently approximating); every other strategy docs/04.md §6 names (LOMA-style,
annealing, CP/MIP, gradient, Bayesian, evolutionary, LLM-agent) — `search/annealing/` etc. are
still empty.
