...
def render_guidance(notes: Sequence[Note], *, max_chars: int = 1200) -> str:
    ...
    is announced rather than silent: the model is
...
def guidance_lesson(text: str, *, reaches: str | None) -> str:
    """The lesson line a run keeps for one operator note (D388): what was said, and
    honestly where it went -- `reaches` names the prompt it joins ("the next invention
    prompt"), None says there is no model role in this run to hand it to."""
...
def note_sink(records: Any, acknowledge: Callable[[str], None] | None = None,
    ...
    """The `on_note` every consumer hands `drain_guidance` (D429): persist the note into
    the campaign record when there is one (a typed line must never vanish silently,
    D388; a resume re-shows it, D403), then acknowledge it the run's own way -- a
    lesson line, a log line, a printed echo. Five applications carried this by hand and
    one of them forgot the record."""
