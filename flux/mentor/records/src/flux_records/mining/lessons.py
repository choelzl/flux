...
_GENERIC = {"max", "min", "words", "per", "cycle", "cycles", "total", "value", "mhz", "bits",
            "ps", "um2", "mm2", "mw", "w", "ipc", "rows", "cy", "rate", "count"}
...
# The things a study designs, in every study's word (D442): a claim about "no <thing>" or
# "every <thing>" is universal whichever word the model chose.
THINGS = "configuration|candidate|design|fabric|mapping|operator|prefetcher|policy|topology"
_UNIVERSAL_MISS = re.compile(
    rf"\b(?:no (?:{THINGS})|none of|not (?:reached|met) by any|"
    rf"unsatisfied by all|every (?:{THINGS}) fail|"
...
def achieved_maxima(campaign_db: str | Path) -> dict[str, float]:
    ...
    the store's own `metric_maxima` (D440), {} for a store that is not there or unreadable."""
    from flux_store import CampaignStore

    if not Path(campaign_db).exists():
        return {}
    try:
        with CampaignStore(campaign_db) as store:
            return store.metric_maxima()
    except Exception:  # noqa: BLE001 -- an unreadable store bounds nothing
        return {}
