...
clipped, source-cited, under a character budget. The NLU study and the interconnect flow had
each written this loop; this is the one copy, and a problem's `mentor_sections` can call it
for its library, the design-guidance corpus or a standard alike."""
...
def library_context(queries: Iterable[str], *, standard_id: str | None = "library",
                    k_per_query: int = 2, max_chars: int = 2600, clip: int = 320,
                    header: str | None = LIBRARY_HEADER, prefix: str = "  * ", cite: bool = True,
                    index: Any = None) -> str:
    """Lines of `prefix[source] text...` for the top `k_per_query` hits of each query, each
    chunk once, each text clipped to `clip` characters, stopping when `max_chars` would be
    exceeded; "" when nothing is found or the index is unavailable (a run is the same without
    papers). `index` overrides the default index (tests)."""
    try:
    ...
    try:
        for q in queries:
            ...
            for hit in knowledge_lookup(q, standard_id=standard_id, k=k_per_query + 3,
                ...
                seen.add(c.id)
                taken += 1
                text = c.text if len(c.text) <= clip else c.text[:clip] + "..."
                src = f"[{c.source_path.rsplit('/', 1)[-1]}] " if cite else ""
                line = f"{prefix}{src}{text}"
