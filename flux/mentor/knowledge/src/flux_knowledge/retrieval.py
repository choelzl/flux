...
in `interfaces/chia_nodes/`/`interfaces/mcp/`, docs/decisions.md D11 — this module only builds the first
of the three).
...
def _cached_default_index() -> BM25Index:
    ...
    if _default_index_cache is None:
        # `mentor/knowledge/` holds the corpus (`corpus/` is its child; build_default_index
        # also accepts the sibling shape). Provenance paths are relative to the repo
        # root `flux/`, two levels up, so a chunk cites `mentor/knowledge/corpus/...`.
...
def knowledge_lookup(
    ...
    (e.g. `"riscv-unpriv"`). This is the typed-function surface docs/agent-surface.md describes; the
    CHIA node (`interfaces/chia_nodes/knowledge.py`) and the MCP tool (`interfaces/mcp/tool.py`) are
    the other two, over this same function.
