...
chiplets, DRAM detail, thermal)") — NoC was already real via `evaluator/booksim`/`evaluator/
...
data movement between dies is a genuinely separate, NoC-style concern (`evaluator/booksim`'s own
...
(like `evaluator/booksim`'s NoC traffic and `evaluator/cacti`'s SRAM characterization) its
...
class ThermalEvaluator(SequentialBatch):
    ...
    def __init__(self, *, timeout_s: float = 300.0) -> None:
        ...
        self._source = ToolSource("thermal", lambda d: ensure_3dice_binary(d, timeout_s=self.timeout_s))
