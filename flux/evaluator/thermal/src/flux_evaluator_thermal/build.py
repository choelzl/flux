...
(evaluator/booksim, /noxim, /cacti, /gem5). See docs/decisions.md D64 for the full build-friction
story; this module is where the fixes actually live, matching the split `evaluator/gem5` already
...
subprocess calls make, the same mechanism `evaluator/gem5`'s own module docstring already
...
from flux_evaluator_abi.tools import ToolRun, run_tool, tails
...
# docs/decisions.md D38 already established for evaluator/gem5's own pin.
...
def _run(cmd: list[str], *, cwd: Path, timeout_s: float, env: dict[str, str] | None = None) -> ToolRun:
    """One tool launch through the ABI's runner (D429): `.returncode`/`.stdout`/`.stderr`
    as before, timed as a `tool:` phase; a missing binary or a timeout raises RuntimeError."""
    return run_tool(cmd, cwd=cwd, timeout_s=timeout_s, env=env)
...
def ensure_3dice_binary(work_dir: Path, *, timeout_s: float = 300.0) -> Path:
    ...
    cache this per-instance, the same pattern `evaluator/cacti`/`evaluator/gem5` use). Returns
    ...
    if install_clean.returncode != 0:
        raise ThermalBuildError(
            ...
            f"{tails(install_clean, stdout=False)}"
        )
