# evaluator/thermal — real thermal simulation via 3D-ICE
...
thermal)") — NoC was already real via `evaluator/booksim`/`evaluator/noxim`; thermal had no real
...
- **Reproduces 3D-ICE's own bundled reference test exactly** before any Flux-side translator code
  ...
  standalone first" discipline `evaluator/booksim`'s own torus88 reproduction (docs/decisions.md
...
same honest gap `evaluator/booksim`'s NoC traffic and `evaluator/cacti`'s SRAM characterization
...
separate concern, `evaluator/booksim`'s own territory — nothing here models it, and nothing here
...
No new CHIA node or MCP tool was needed: `"thermal"` is registered in `interfaces/cli/src/flux_cli/
...
Package: `flux-evaluator-thermal` — like `evaluator/cacti`/`evaluator/gem5`/`evaluator/booksim`/
`evaluator/noxim`, deliberately **not** one of `flake.nix`'s `localSrcDirs` (a heavy, optional,
external-simulator adapter most dev work never imports) — callers add `evaluator/thermal/src` to
