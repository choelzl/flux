...
`evaluator/rtl`/`evaluator/systemc` already impose, not a new restriction) rather than silently
approximating.
...
Backs the `flux_check_validity` CHIA node (`interfaces/chia_nodes/`) and its matching MCP tool
(`interfaces/mcp/`) — see their READMEs for the real end-to-end verification against ZigZag/RTL/
Timeloop.
