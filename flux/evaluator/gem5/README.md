# evaluator/gem5 — real cycle-accurate CPU simulation

The second evaluator in this repo adapted *through* CHIA's own existing tool integration (after
`evaluator/cacti`, D36): `Gem5Evaluator` calls `chia.simulators.gem5.Gem5Node` (a real
...
This was a much bigger undertaking than `evaluator/cacti`'s plain-`make` build:
...
3. **A real structural mismatch, found and designed around, not ignored**: gem5 runs actual
   *compiled programs* on a modeled CPU — it has no way to consume Flux's abstract Workload IR
   ...
   `evaluator/rtl`/`evaluator/systemc` already use for their fixed `mac_array` design.
...
supports several others (`Gem5Isa` enum: ARM, X86, ...). Since D446 the configuration is this
adapter's own `gem5_config.py`, written against gem5's STANDARD LIBRARY (`gem5.components.*`,
the same API `configs/example/gem5_library/` uses): one core of a chosen type at a chosen clock,
no caches, one DDR3 channel. It replaced `configs/deprecated/example/se.py`, which upstream has
moved under `configs/deprecated/` and no longer maintains. What is still not wired is everything
the IR cannot yet say: a cache hierarchy, more than one core (the standard library indexes the
cycle counter per core, and no single counter then stands for the board), a board other than
`SimpleBoard`, and any benchmark but the clone's bundled RISC-V hello (there is no
Workload-IR-to-binary compiler in this repo). `Gem5Node`'s source-state capture/restore
