...
class Limiter(str, Enum):
    ...
    THERMAL = "thermal"  # docs/decisions.md D64 — evaluator/thermal's real 3D-ICE-backed evaluator
    NONE = "none"        # D440: no bottleneck claimed -- a record written without a cost model
...
class Escalation:
    ...
    `next_stage` is a REGISTERED EVALUATOR NAME (D448), so the recommendation is a step a
    caller can take: `flux_evaluator_abi.make_evaluator(next_stage)` builds it. Whoever
    emits one validates it with `escalation_stage(name)`; `from_dict` does not, so a stored
    result naming a backend this checkout no longer registers still reads back.
    """
...
class Result:
    ...
    def refusal_for(self, metric: str) -> str | None:
        ...
        not an error the ABI forbids, and `evaluator/rtl` does it routinely (it populates its
