...
#: What `flux eval` and the CHIA nodes ask an evaluator for when the caller names no metrics
#: (D558: was the CLI facade's, the one thing it held that the ABI did not).
...
def evaluator_name_for(provenance: str) -> str:
    ...
    `'timeloop-docker@image'`, `'champsim_bingo@1'`) back to a registered name --
