...
class SequentialBatch:
    ...
    adapter in this repo declares it as a base -- the one place the sequential body lives --
    and the `Evaluator` protocol below stays what a backend is checked against structurally.
    Real concurrency comes from wrapping an evaluator (`flux_chia_nodes.ChiaParallelEvaluator`),
    not from a per-adapter batch loop; batch *performance* inside an adapter is a Phase 3
    concern (docs/roadmap.md)."""
