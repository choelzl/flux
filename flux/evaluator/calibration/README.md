...
escalation policy / stage selector, drift-detection CI. The single highest-leverage gap this
...
simulation or silicon. RTL simulation now exists (`evaluator/rtl/`, real Verilator) and is wired
...
`apply_escalation_policy()`'s `next_stage` names `rtl` — a real, invokable stage
(`evaluator/rtl/`'s `RTLEvaluator`, registered as `"rtl"` in `interfaces/cli/registry.py`). There is no
stage *above* that yet (`evaluator/hammer/` is still empty), so escalating past RTL-sim has
...
`flux_conformance_check` CHIA node (`interfaces/chia_nodes/`); verified end to end against the real,
...
Not implemented: holdout-corpus enforcement. (The synthesis-fidelity stage above RTL-sim exists now — `evaluator/openroad`, docs/decisions.md D225.)
