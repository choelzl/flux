...
and this repo's workload IR has no sparsity annotation for an adapter to translate, so the stage
...
`flux_evaluator_abi` `Evaluator` protocol as every other stage, so it is interchangeable with them.
