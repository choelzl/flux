...
The Evaluator ABI's synthesis stage. The design it places is the **deterministic combinational
...
Why not `mac_array.sv` (the design `evaluator/rtl` simulates): its operand memories are loaded
...
(`evaluator/rtl`'s job, cycle-accurate) and `energy_pj` (power x another stage's runtime would be
that stage's number wearing this one's provenance). A campaign wanting all three escalates through
both stages. The workload enters through the derived spec's own precision and lane count — the
