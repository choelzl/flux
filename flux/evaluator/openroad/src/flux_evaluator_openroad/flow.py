...
class PpaReport:
    ...
    cell_count: int
    flow_depth: str  # "placement" (v0.1); routing stages extend this vocabulary
    yosys_log_tail: str
    openroad_log_tail: str
    critical_path: dict[str, Any] | None = None   # D526: the worst path as data (`parse_critical_path`)
    ...
    def metrics(self) -> dict[str, float]:
        """The report as the metric dict every consumer used to project by hand (D440):
...
def openroad_threads() -> int:
    ...
    cores over the four placements the loop's pool runs at once (64 cores: 16 each), at least
    one. Before D570 every run was `-threads 1`, OpenROAD's default: a 53,842-cell route on
    one core of sixty-four."""
    import os
...
def _run(cmd: list[str], *, cwd: Path, timeout_s: float, what: str) -> str:
    """One tool launch through the ABI's runner (D429): timed as a `tool:` phase where it
    is called from (D295), a missing binary, a timeout or a non-zero exit raised as
    `OpenRoadError` naming `what` and quoting the tool's tail."""
...
def _yosys_synth(
    ...
    *, chparams: dict[str, int], yosys_bin: str, timeout_s: float,
    full_mapping: bool = True, macro_stubs: list[Path] | None = None,
    ...
    before the design, so their instances survive synthesis as instances. `keep_nets`
    (D527): wire-name patterns (yosys globs, `*_d*`) whose names survive synthesis -- the
    `keep` attribute holds them through `opt_clean -purge` -- so a timing path can be read
    back to them; meant for register boundaries, which abc does not optimise across anyway."""
    ...
    if sv_frontend == "slang":
        ...
        # `read_slang` elaborates the top itself, so `hierarchy -chparam` never runs on this
        # path and parameter overrides were silently DROPPED: chparams={"DATA_W": 8} produced a
        # bit-identical netlist to the default. slang takes overrides as `-G name=value` at read
        # time, which is the only point at which they can still take effect here.
...
def _openroad_tcl(
    ...
    *, clock_period_ps: float, clock_port: str | None, flow_depth: str = "placement",
    ...
    tcl = (
        ...
        # Constant nets reach TritonRoute as signal type POWER and it refuses them
        # ("[ERROR DRT-0305] Net one_ of signal type POWER is not routable"), so every
        # flow_depth="routed" run died before this. Tie cells give those constants a real driver.
        # Names from platform/asap7/config.mk's TIEHI_CELL_AND_PORT / TIELO_CELL_AND_PORT --
        # resolved here rather than read from the .mk, whose values are Make expansions of
        # $(PRIMARY_VT_TAG), not literals.
        ...
        # `net` names the net each pin drives (D526): the RTL's own signal names where
        # synthesis kept them, so a path reads back to the prototype's blocks
...
def run_synthesis_flow(
    ...
    *,
    ...
    The screening stage below placement: the same Yosys mapping `run_ppa_flow` uses, then static
    timing on the netlist with no parasitics -- optimistic by whatever the wires would have
    cost, and identical in method for every candidate, so it ORDERS a design space in a few
    seconds where placement takes tens. `area_um2` is the liberty-summed cell area from
    `stat -liberty`; `flow_depth` says "synthesis" so nobody quotes it as placed silicon.
    `report_power` runs without parasitics too, from the liberty tables alone.
    """
...
# The critical path as DATA (D526, review step 8): the `full_clock_expanded` report parsed
# into its steps -- the pin, the cell, the net it drives when the report says it, the
# step's own delay and the arrival time -- with the start, the end and the slack. Captured
# from openroad 26Q2 on a placed NLU part:
...
def run_ppa_flow(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="flux-openroad-") as td:
        ...
        # The full ABC mapping is the flow (D564, Cedric: "always full; the -fast results are too
        # bad to justify it"): -24% area and -43% power on the macarray's decision PE, 3 s a
        # placement there; D228 had measured 420 s on an 8-lane array and left -fast the default.
        netlist, cell_count = _yosys_synth(
