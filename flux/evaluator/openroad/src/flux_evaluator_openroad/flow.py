...
def _run(cmd: list[str], *, cwd: Path, timeout_s: float, what: str) -> str:
    ...
    try:
        with phase(f"tool:{cmd[0].rsplit('/', 1)[-1]}", why=what):
            proc = subprocess.run(
...
def _yosys_synth(
    ...
    *, chparams: dict[str, int], yosys_bin: str, timeout_s: float,
    ...
    # The same total row carries the liberty-summed cell area, which is what the synthesis rung
...
def run_synthesis_flow(
    ...
    *,
    ...
    The screening rung below placement: the same Yosys mapping `run_ppa_flow` uses, then static
