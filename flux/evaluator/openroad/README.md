# `evaluator/openroad/` — real physical-design PPA on ASAP7

The synthesis-fidelity stage (docs/decisions.md D225, closing docs/roadmap.md's #1 immediate
...
- **What it places**: `flux_generation.derive_design_spec`'s combinational dot-product datapath
  ...
  away (measured: LANES 8 vs 32 differed by ten cells). `evaluator/rtl` still owns
  `latency_cycles` for that design; this stage deliberately reports neither latency nor energy.
