# evaluator/cacti — real circuit-level SRAM characterization
...
Neither `evaluator/zigzag` nor `evaluator/timeloop` give any physically-grounded memory number
...
- **CACTI genuinely builds and runs here**: cloned, built via a plain `make` (no `flex`/`bison`/
  `cmake` gap this time, unlike `evaluator/booksim`/`evaluator/noxim` — confirmed by actually
...
`evaluator/booksim`/`evaluator/noxim` already use for `interconnect.noc`. `Candidate.workload`
