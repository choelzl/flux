...
building a bespoke one, a genuinely different shape from `evaluator/booksim`/`evaluator/noxim`
...
Fills a real gap, not a redundant third memory model: neither `evaluator/zigzag` nor
`evaluator/timeloop` give any physically-grounded SRAM number — both cost memory access
...
`evaluator/booksim`) its content drives nothing — CACTI has no workload concept at all, only a
...
class CactiEvaluator(SequentialBatch):
    ...
    def __init__(self, *, timeout_s: float = 120.0) -> None:
        ...
        self._source = ToolSource("cacti", self._build)        # cloned and built once (D437)
    ...
    def _ensure_cacti_binary(self) -> Path:
        return self._source.ensure(provided=self._provided_cacti_binary)

    def _build(self, work_dir: Path) -> Path:
        repo_dir = clone(_CACTI_REPO_URL, work_dir / "cacti", what="CACTI", timeout_s=self.timeout_s)
        binary_path = repo_dir / "cacti"
        build_step(["make", "-j2"], cwd=repo_dir, what="CACTI build", timeout_s=self.timeout_s,
                   expect=binary_path,
                   hint="needs git, g++/gcc, make on PATH; plain Makefile build, no "
                        "flex/bison/cmake needed (confirmed by actually building it, "
