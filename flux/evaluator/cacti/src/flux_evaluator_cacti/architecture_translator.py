...
`evaluator/booksim`/`evaluator/noxim` already use for `interconnect.noc` (not a whole SoC's
...
- **`word_width_bits`**: `size_kb` alone doesn't determine a real SRAM's `depth` (word count) —
  ...
  explicitly, the same "extend additively, don't guess" pattern `evaluator/booksim` used for
  `interconnect.noc`.
...
def architecture_ir_to_sram_spec(arch: dict[str, Any]) -> SRAMSpec:
    ...
    if len(memory_nodes) != 1:
        raise NotExpressibleError(
            f"architecture {arch_id!r}: evaluator/cacti v0.1 needs exactly one "
...
def architecture_ir_to_technology_um(arch: dict[str, Any]) -> float:
    ...
    if not match:
        raise NotExpressibleError(
            ...
            "(e.g. 'n28') — evaluator/cacti can't derive a technology from it."
        )
    ...
    if nm < _MIN_SUPPORTED_NM:
        raise NotExpressibleError(
            ...
            "composition stage's cacti_scale_from_nm option."
        )
