# evaluator/systemc — the coarse-grain pre-check stage

A fast, real SystemC simulation of the exact same `mac_array` design `evaluator/rtl` models
cycle-accurately — docs/calibration.md's fidelity chain gets a new stage between the analytic
evaluators (ZigZag, Timeloop) and RTL-sim: fast functional correctness + a timing pre-check,
escalating to `evaluator/rtl` for the cycle-accurate, independently-simulated number.
...
- **Functional correctness is checked for real**, every run, against the exact same golden
  reference `evaluator/rtl` uses (`flux_evaluator_rtl.generate_test_vectors` — both adapters
...
- **The timing model is loosely-timed, not cycle-by-cycle**, and its formula is proven, not
  ...
  mid-run.
- **No recompilation per shape** (`evaluator/rtl`'s real limitation): B/C/K/LANES are runtime
  arguments to the compiled binary, not Verilog compile-time parameters, so one build serves
  every shape. This is the actual point of a coarse-grain stage — not just a faster simulation,
...
not exact) — the escalation to `evaluator/rtl` for the authoritative number stays necessary
regardless.
...
dependency, unlike `evaluator/hammer/`).

See [docs/evaluator-abi.md](../../../docs/evaluator-abi.md) and
