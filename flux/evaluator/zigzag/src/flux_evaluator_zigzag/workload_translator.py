...
static integer bounds. `data_dependent` and `compute_kernel` ops (docs/decisions.md D1) have
no ZigZag equivalent at all; dynamic bounds (`{dyn: [...]}`, docs/ir.md) have no ZigZag
...
def einsum_op_to_zigzag_layer(op: dict[str, Any], layer_id: int) -> dict[str, Any]:
    ...
    in1_dims, in2_dims, out_dims = list(parsed.in1), list(parsed.in2), list(parsed.out)
    ...
    for dim in all_dims:
        ...
        if not isinstance(size, int):
            raise NotExpressibleError(
                ...
                "loop size, not a distribution — this is docs/gap-analysis.md G5's dynamic-shape gap, not "
                "a translation bug."
            )
