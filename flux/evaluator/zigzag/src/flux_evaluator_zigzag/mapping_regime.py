...
`evaluator/rtl`'s fixed K-unrolled `mac_array.sv` schedule never gets.
...
def reduction_dims(op: dict[str, Any]) -> list[str]:
    ...
    # The IR's own grammar (D467), the same one `workload_translator` accepts: a reduction dim
    # is shared by both inputs and absent from the output.
