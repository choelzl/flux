# evaluator/stream — real multi-core/layer-fused DSE via Stream
...
- **Real multi-core architecture translation** (docs/decisions.md D82, this package): a new
  ...
  `evaluator/zigzag/architecture_translator.py`'s own real output (structurally identical) —
...
No new CHIA node or MCP tool was needed: `"stream"` is registered in `interfaces/cli/src/flux_cli/
...
Package: `flux-evaluator-stream` — in `flake.nix`'s `localSrcDirs` (like `evaluator/zigzag`/
`evaluator/timeloop`, not the heavy "clone-and-build-on-first-use" adapters): `stream-dse`/
