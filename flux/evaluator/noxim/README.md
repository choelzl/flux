# evaluator/noxim — a real, independent second NoC evaluator

A second real NoC simulator (docs/decisions.md D32) alongside `evaluator/booksim`, via
...
- **Real, hard-won CLI details**: Noxim silently `exit(0)`s with *zero* simulation output if
  ...
  explicitly. Noxim also refuses `packet_size < 2` outright — `evaluator/booksim`'s IR-level
...
## v0.1 scope — narrower than `evaluator/booksim`'s, by design
...
`evaluator/booksim` so far, not that this pairing is broken.
...
`latency_cycles`, mirroring `evaluator/booksim`'s own v0.1 scope) — real numbers are already in
