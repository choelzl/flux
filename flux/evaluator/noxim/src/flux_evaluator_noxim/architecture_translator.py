...
part of it (docs/decisions.md D16/D25) — a real, honest, narrower scope than `evaluator/booksim`
...
- `packet_size`: Noxim refuses anything below 2 flits outright (`Error: packet size must be >=
  ...
  "different tools, different sensible defaults" pattern `evaluator/booksim`'s own
...
- Injection process: Noxim's `-pir` requires an explicit time-distribution argument (`poisson`,
  ...
  as `evaluator/booksim`'s own quantified ~5.4% gap against `mesh88_lat`'s unmodelled
...
def architecture_ir_to_noxim_args(arch: dict[str, Any]) -> dict[str, Any]:
    ...
    if not noc:
        raise NotExpressibleError(
            f"architecture {arch_id!r} has no interconnect.noc block; evaluator/noxim needs "
    ...
    if topology not in _VALID_TOPOLOGIES:
        raise NotExpressibleError(
            ...
            "docs. evaluator/booksim is the only evaluator here that can simulate torus/3D/6D "
            "candidates; evaluator/noxim only ever covers the 2D-mesh slice."
        )
    ...
    if not dimensions or not isinstance(dimensions, list) or len(dimensions) != 2:
        raise NotExpressibleError(
            ...
            f"for evaluator/noxim (Noxim's mesh is hard 2D — mesh_dim_x/mesh_dim_y, no third "
    ...
    if routing_function != "dim_order":
        raise NotExpressibleError(
            ...
            "is not translatable — evaluator/noxim v0.1 only maps 'dim_order' to Noxim's 'XY' "
    ...
    if noxim_traffic is None:
        raise NotExpressibleError(
            ...
            f"Noxim equivalent — evaluator/noxim v0.1 only translates {sorted(_TRAFFIC_MAP)} "
    ...
    if packet_size < _MIN_PACKET_SIZE:
        raise NotExpressibleError(
            ...
            ">= 2', confirmed by actually running it) — evaluator/booksim's IR-level default of "
            "1 is not expressible here; request packet_size >= 2 explicitly for a candidate "
            "that's also meant to be checked against evaluator/noxim."
        )
