...
v0.1 scope, narrower than `evaluator/booksim`'s by design, not by oversight: Noxim has no torus
...
`evaluator/booksim` exactly (inline Architecture IR dict with `interconnect.noc`, inline
...
No independent functional checker exists (same honest gap `evaluator/booksim`/`evaluator/
zigzag`/`evaluator/timeloop` already have) — `Result.validity.ok` is a placeholder `True`.
...
# choice `evaluator/booksim` makes for `sim_type: "latency"`, just a different mechanism).
...
# error anywhere (docs/decisions.md D181). `evaluator/timeloop` and `evaluator/dramsim3`
...
class NoximEvaluator(SequentialBatch):
    """Runs a real Noxim simulation of a translated Architecture IR `interconnect.noc` block.
    A second, independent NoC evaluator alongside `evaluator/booksim` — same Evaluator ABI
    ...
    def __init__(self, *, timeout_s: float = 600.0) -> None:
        ...
        self._source = ToolSource("noxim", self._build)        # cloned and built once (D437)
    ...
    def evaluate(self, candidate: Candidate, budget: Budget, metrics: frozenset[str]) -> Result:
        ...
        if not isinstance(candidate.workload, dict):
            raise NotExpressibleError(
                ...
                "traffic (see evaluator/booksim's equivalent note — the same real "
    ...
    def _build(self, work_dir: Path) -> tuple[Path, Path, Path]:
        repo_dir = clone(_NOXIM_REPO_URL, work_dir / "noxim", what="Noxim", timeout_s=self.timeout_s)
        build_step(["./build.sh"], cwd=repo_dir, what="Noxim build", timeout_s=self.timeout_s,
                   hint="needs git, g++, make, cmake, and curl/wget on PATH (nix develop "
                        ".#default provides cmake; the rest are already there for "
                        "evaluator/booksim). build.sh self-provisions SystemC 2.3.1 and "
                        "yaml-cpp from source under bin/libs/")
        return (repo_dir, repo_dir / "bin" / "noxim",
                repo_dir / "bin" / "libs" / "systemc-2.3.1" / "lib-linux64")
