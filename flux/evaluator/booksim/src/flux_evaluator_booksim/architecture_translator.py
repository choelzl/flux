...
derive traffic — see `evaluator/booksim/README.md` for the honest reasoning, not a silent
mismatch.
...
def architecture_ir_to_booksim_config(arch: dict[str, Any]) -> dict[str, Any]:
    ...
    if not noc:
        raise NotExpressibleError(
            f"architecture {arch_id!r} has no interconnect.noc block; evaluator/booksim needs "
    ...
    if topology not in _VALID_TOPOLOGIES:
        raise NotExpressibleError(
            ...
            f"{_VALID_TOPOLOGIES} — evaluator/booksim only translates the k-ary n-cube family "
    ...
    if not dimensions or not isinstance(dimensions, list):
        raise NotExpressibleError(
            ...
            "mesh) and evaluator/booksim didn't find one."
        )
