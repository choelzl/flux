# evaluator/booksim — real NoC simulation (2D and 3D), real chiplet D2D interconnect
...
- **The Architecture IR didn't need new top-level structure** — `interconnect.noc` already
  ...
  still validate. `evaluator/booksim` raises `NotExpressibleError` for those, in Python, not a
...
No independent functional checker exists yet (unlike `evaluator/rtl`'s self-check against a
Python reference) — `Result.validity.ok` is a placeholder `True`, the same honest gap
`evaluator/zigzag`/`evaluator/timeloop` already have. `ir/architecture/examples/noc-torus-2d-v1.yaml`
...
real-world constraint, closed by `evaluator/thermal`'s real 3D-ICE integration instead
...
to carry (the same honest gap `evaluator/thermal`'s `attrs.power_w` already named) — so
`latency_cycles` is caller-declared directly, not derived.

**This is real D2D *interconnect* modeling only — not the same thing as `evaluator/thermal`'s
...
"fetch an external resource once, cache it" shape `evaluator/timeloop` uses for its Docker
image).
...
or not). Actually built as `evaluator/noxim` (docs/decisions.md D32) — see that package's README
