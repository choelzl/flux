# evaluator/dramsim3 — real DRAM bank/refresh-timing simulation via DRAMsim3
...
chiplets, DRAM detail, thermal)") — NoC (`evaluator/booksim`/`evaluator/noxim`), chiplet D2D
...
- **By far the simplest external-tool build in this repo's whole adapter set**: one real,
  empirically-found fix, versus 3D-ICE's four-fix saga (`evaluator/thermal`) or gem5's five
  (`evaluator/gem5`). DRAMsim3's own bundled `CMakeLists.txt` declares `cmake_minimum_required(
...
- **Needed zero `flake.nix` changes at all** — a genuine simplification versus every prior
  ...
  `evaluator/noxim`'s own build, docs/decisions.md D32).
- **Reproduces DRAMsim3's own bundled reference config run exactly** before any Flux-side
  ...
  works standalone first" discipline `evaluator/booksim`'s own torus88 reproduction (docs/
...
`evaluator/booksim`'s own NoC traffic and `evaluator/cacti`'s SRAM characterization already
...
`interfaces/cli/src/flux_cli/registry.py`'s evaluator registry the same way `"thermal"`/`"cacti"`/
`"gem5"` already are, so it's reachable through the existing generic `flux_evaluate` node/tool
immediately — the same shape `evaluator/thermal` already established (its own new evaluator
...
Package: `flux-evaluator-dramsim3` — like `evaluator/cacti`/`evaluator/gem5`/
`evaluator/booksim`/`evaluator/noxim`/`evaluator/thermal`, deliberately **not** one of
`flake.nix`'s `localSrcDirs` (a heavy, optional, external-simulator adapter most dev work never
imports) — callers add `evaluator/dramsim3/src` to `PYTHONPATH` explicitly. Needs only `git`,
