...
docs/gap-analysis.md G6 still names as open once evaluator/booksim (NoC) and evaluator/thermal
...
`evaluator/booksim` already has for NoC traffic). `Candidate.mapping` must be `None` — DRAMsim3
...
import flux_ir
from flux_evaluator_abi.tools import tails, ToolSource
...
def _parse_dramsim3_output(text: str) -> dict[str, float]:
    ...
    if len(channels) != 1:
        raise NotExpressibleError(
            ...
            "evaluator/dramsim3 v0.1 only supports exactly one."
        )
    return {m.group(1): float(m.group(2)) for m in _STAT_LINE_RE.finditer(text)}


class DramSim3Evaluator(SequentialBatch):
    ...
    def __init__(self, *, timeout_s: float = 300.0) -> None:
        ...
        self._source = ToolSource("dramsim3", lambda d: ensure_dramsim3_binary(d, timeout_s=self.timeout_s))
    ...
    def _ensure_binary(self) -> tuple[Path, Path]:
        return self._source.ensure(provided=self._provided_dramsim3)
