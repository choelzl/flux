# evaluator/hammer — the synthesis/P&R fidelity stage (not yet built)

The stage above `evaluator/rtl/` in docs/calibration.md's escalation chain: real synthesis and
...
on the same worker (`obj_dir` is path-based, on-worker). This means building `evaluator/hammer/`
should be a thin Flux Evaluator ABI adapter that *calls* `HammerNode`, translating Architecture
IR into Hammer's config format — the same "adapters, not forks" pattern as `evaluator/zigzag/`
and `evaluator/timeloop/` — not a from-scratch subprocess/CLI wrapper.
...
Hammer's YAML config format, same shape as `evaluator/zigzag/architecture_translator.py`) and an
