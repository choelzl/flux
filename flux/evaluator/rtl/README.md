# evaluator/rtl — the RTL-sim (Verilator) backend adapter

The third `Evaluator` implementation, and the first that's a real *simulation*, not an analytic
cost model — the escalation stage docs/calibration.md's diagram describes (analytic → RTL sim/synth
→ calibration store), started here on a small, hand-written design rather than a large
third-party RTL generator (docs/decisions.md D2's build-vs-reuse discipline: prove the ABI
...
- Every run **actually compiles and simulates** `mac_array.sv` via real Verilator
  ...
  detail — `evaluator/systemc/`'s coarse-grain model self-checks against the exact same
  reference, so both adapters agree on what "correct" means for the identical design (see
  `evaluator/systemc/README.md`).
...
[docs/calibration.md](../../../docs/calibration.md) (the escalation-stage diagram this adapter is the first real
instance of), and
