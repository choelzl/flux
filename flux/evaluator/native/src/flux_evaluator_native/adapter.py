...
from flux_evaluator_abi.tools import ToolSource
...
class NativeEvaluator(SequentialBatch):
    ...
    def __init__(self, *, timeout_s: float = 300.0) -> None:
        ...
        self._source = ToolSource("native", self._load, valid=lambda m: True)   # built and loaded once (D437)
    ...
    def evaluate(self, candidate: Candidate, budget: Budget, metrics: frozenset[str]) -> Result:
        ...
        if candidate.mapping is not None:
            raise NotExpressibleError(
                ...
                "evaluator/thermal/evaluator/dramsim3 already use for their own "
    ...
    def _load(self, _work_dir: Path) -> ModuleType:
