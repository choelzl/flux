# evaluator/native — native, in-repo roofline evaluator
...
with a 3-dim bound (the same shape `evaluator/rtl`/`evaluator/systemc`/`validity/roofline.py`
...
be `None`" scope `evaluator/thermal`/`evaluator/dramsim3` already use for their own
...
No new CHIA node or MCP tool was needed: `"native"` is registered in `interfaces/cli/src/flux_cli/
...
the same limit `evaluator/rtl`/`systemc`/`validity/roofline.py` already share).
...
`evaluator/native/src` to `PYTHONPATH` explicitly. Needs only `cargo`/`rustc` (already on
