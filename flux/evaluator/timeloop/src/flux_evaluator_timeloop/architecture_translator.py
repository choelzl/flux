...
v0.1 scope and conventions — narrower even than evaluator/zigzag's equivalent, because
...
- Every `memory`-class hierarchy entry becomes one Timeloop `!Component` (class `DRAM` if its
  ...
  "no per-operand-residency-yet" simplification as evaluator/zigzag's translator, for the same
