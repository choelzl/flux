# evaluator/timeloop — the Timeloop+Accelergy backend adapter
...
- `Candidate.arch` — an inline Architecture IR document translates to literal Timeloop
  ...
  a deliberately narrower subset than `evaluator/zigzag`'s equivalent: **exactly one spatial
...
`evaluator/zigzag`, and both Timeloop runners produce identical numbers for it (64 cycles at
