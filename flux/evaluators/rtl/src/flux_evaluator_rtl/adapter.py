"""RTL-sim backend adapter implementing the Flux Evaluator ABI (docs/04.md §4.4, §5's escalation
rung) via a real Verilator simulation of a hand-written mac_array.sv.
...
class RTLEvaluator:
    ...
    def evaluate(self, candidate: Candidate, budget: Budget, metrics: frozenset[str]) -> Result:
        ...
        if not einsum_ops:
            raise NotExpressibleError(
                ...
                "cannot simulate data_dependent or compute_kernel ops (docs/00-decisions.md D1)."
            )
        if len(einsum_ops) > 1:
            raise NotExpressibleError(
    ...
    def evaluate_batch(
        ...
        # *interface* is satisfied here; batch *performance* is a Phase 3 concern (docs/05.md).
