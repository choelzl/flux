"""ZigZag backend adapter implementing the Flux Evaluator ABI (docs/04.md §4.4).
...
class ZigZagEvaluator:
            ...
            with tempfile.TemporaryDirectory(prefix=f"flux-zigzag-arch-{arch_hash[:12]}-") as tmp:
                ...
                try:
                    ...
                    if "dictionary changed size during iteration" not in str(exc):
                        raise
                    # A real zigzag-dse==3.8.5 bug (not this adapter's), confirmed by reading its
                    # source: LayerTemporalOrdering.is_complete()
                    # (zigzag/workload/layer_attributes.py) does
                    # `for layer_dim in all_loops: ... del all_loops[layer_dim]` — deleting from
                    # a dict while iterating it — whenever the *provided* temporal ordering
                    # includes any dim whose multiplied factor is 1. That happens whenever this
                    # translator emits a temporal loop of size 1, which it does whenever
                    # `spatial.size` fully consumes a dim's bound (e.g. spatially splitting a
                    # size-4 dim onto a size-8-or-larger array slot at size 4, leaving no
                    # remaining temporal iteration) — found by exhaustively enumerating every
                    # flat-mapping candidate (search/exhaustive/), not hand-guessed; see
                    # tests/integration/test_search_exhaustive_live.py.
                    mapping_id = (
                        candidate.mapping.get("id", "<no id>")
                        if isinstance(candidate.mapping, dict)
                        else "<auto-generated>"
    ...
    def evaluate_batch(
        ...
        # call) is a Phase 3 concern (docs/05.md); the ABI's batch *interface* is what's being
        # satisfied here.
    ...
    def _to_result(
        ...
        # there's no calibration store yet (Phase 2, docs/05.md) to derive a CI from residuals.
        # A bare scalar is a bug per docs/04.md's design principles — so ci_low==ci_high==value
        ...
        return Result(
            ...
            # No independent validity checker exists yet (Phase 4, docs/05.md) — `ok=True` is a
