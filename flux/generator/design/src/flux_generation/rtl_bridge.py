...
Scope (v0.1, mirroring `evaluator/rtl`'s own): exactly one single-spatial-dim compute node
...
# fix is `$readmemh`-backed memories (what evaluator/rtl's own reference testbench uses), not a
...
# workload gets 8-bit operand ports and an exactly-sized accumulator, so a physical-design stage
...
def derive_design_spec(
    ...
    if len(ops) != 1:
        raise DerivationError(
            ...
            "spec for exactly one (the same v0.1 scope evaluator/rtl itself has)."
        )
    op = ops[0]
...
class DerivedGemmDesign:
    ...
    so its measured cycle count is directly comparable to `evaluator/rtl`'s for the same pair —
