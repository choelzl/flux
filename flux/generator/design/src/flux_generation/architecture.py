...
(`chia.models.ollama.OllamaLLM`) lives in `interfaces/chia_nodes/generate_architecture.py`.

**Real, checked structural finding this design leans on, not assumed**: `evaluator/rtl`'s own
...
from flux_llm import ask_until, strip_markdown_fence


@runtime_checkable
class LLMProposer(Protocol):
    """`propose(prompt) -> str`, this generator's own face since D508 (`flux_llm` has one
    interface, `Proposer -> Reply`); the chia node puts the one proposer behind it."""
...
from flux_evaluator_abi import DEFAULT_METRICS, make_evaluator
...
def generate_architecture_candidate(
    ...
    *,
    ...
    held: dict[str, Any] = {"result": None}

    def _parse(reply: str) -> dict[str, Any] | None:
        """A YAML mapping, or None -- the text of an unparsable reply is what the repair
        prompt shows, so a parse failure is a refusal like any other rather than the end."""
        try:
            got = yaml.safe_load(strip_markdown_fence(reply))
        except yaml.YAMLError:
            return None
        return got if isinstance(got, dict) else None

    def _verify(candidate_arch: dict[str, Any]):
        """Schema, then the backend: the two real checks, each its own kind in the transcript."""
        try:
            flux_ir.validate("architecture", candidate_arch)
        except (ValueError, flux_ir.SchemaValidationError) as exc:
            return (str(exc), "schema error")
        try:
            held["result"] = evaluator.evaluate(
                Candidate(workload=workload, arch=candidate_arch, mapping=None), Budget(),
                requested_metrics,
            )
        except ValueError as exc:  # every adapter's NotExpressibleError subclasses ValueError
            return (str(exc), "evaluation error")
        ...
        if objective_metric not in held["result"].metrics:
            raise GenerationError(
                f"backend {backend!r} does not emit metric {objective_metric!r}; it returned "
                f"{sorted(held['result'].metrics)} — pick an objective_metric from that set."
            )
        return ""

    # The bounded ask-check-repair round is `flux_llm.ask_until` (D450), the one every
    # generator here shares; what stays this module's own is the prompt, the YAML reading and
    # the two real checks above.
    got = ask_until(
        _architecture_prompt(workload, base_arch, objective_metric, minimize),
        ask=llm_proposer.propose, parse=_parse, check=_verify,
        repair=lambda _arch, why, reply: _repair_prompt(strip_markdown_fence(reply), why),
        attempts=max_repair_attempts,
        unparsable=lambda reply: (
            "the reply is not a YAML mapping"
            + ("; it looks cut off before the document ended" if len(reply) > 400 else "")))
    transcript: list[str] = list(got.transcript)
    if not got.ok:
        return GenerationResult(
            success=False, attempts=got.attempts, final_arch=None,
    ...
    final_arch: dict[str, Any] | None = got.value
    declared_result = held["result"]
    ...
    return GenerationResult(
        success=True, attempts=got.attempts, final_arch=final_arch,
