...
- **(b) RTL conformance within the calibrated uncertainty band** —
  ...
  `evaluator/rtl`'s own real Verilator measurement checked against `backend`'s calibrated CI. A
  real, checked structural finding this design leans on: `evaluator/rtl`'s own
...
any `LLMProposer` (`propose(prompt) -> str`). `interfaces/chia_nodes/generate_architecture.py` supplies
...
Real RTL conformance only exists for architectures `evaluator/rtl`'s own hand-written
`mac_array.sv` design can express — exactly one compute-class hierarchy node with exactly one
spatial dim (see `evaluator/rtl/README.md`). A generated candidate that adds a second compute
...
This dataflow parallelises the *reduction* across lanes, while `evaluator/rtl`'s `mac_array.sv`
...
contribute is coverage: `evaluator/rtl` refuses any candidate `mac_array.sv` cannot express, and a
