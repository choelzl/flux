...
(docs/decisions.md D39, shared in D453). Follows this repo's existing IR conventions loosely
(`docs/ir.md`: `schema_version` + `id` + a typed structural list + `attrs`-shaped fields) without
pulling in the full `flux_ir` machinery (canonicalization, content hashing) -- a plain validated
dict in, a plain dataclass out.

Deliberately declarative, not LLM-authored: `ports` and `test_vectors` drive a
*deterministically generated* driver (each language harness's own `driver_gen.py`) that does the
actual VCD tracing and pass/fail checking. Only the DUT module's *internal behavior* is left for
a model to fill in -- the thing doing the checking is never the thing being checked, the same
independence `flux_chia_nodes.validity` applies to evaluated candidates.

v0.1 dtype support is deliberately small: `"int"` and `"bool"`, the two builtin types both
harnesses instantiate trivially (an `sc_in`/`sc_out` template without a custom `sc_trace`
overload; a plain Verilog net). Wider types (structs, packed unions) are a real, un-implemented
extension, not silently supported. How a dtype and width SPELL in a language belongs to that
language's harness (`flux_codegen_systemc_harness.cpp_type`,
`flux_codegen_rtl_harness.compose._verilog_type`), never here.
"""
...
#: The dtypes a spec may declare. How each SPELLS in a language is that
#: language harness's business, not this package's (D453).
