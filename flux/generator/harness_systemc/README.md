...
`interfaces/chia_nodes` (D40) will use an LLM to generate the DUT module's *behavior*. This package
...
checking is never the thing being checked, the same independence `interfaces/chia_nodes/validity.py`
...
Shared with the RTL harness since D453 (`flux-codegen-harness-spec`) and re-exported here; how a
port SPELLS in C++ (`sc_int<N>` for a sized one) stays here, as `cpp_type`.
