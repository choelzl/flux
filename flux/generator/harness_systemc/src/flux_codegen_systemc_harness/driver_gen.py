...
convention `evaluator/rtl`/`evaluator/systemc` already use so downstream tooling can parse it
identically.
...
from .spec import DesignSpec, cpp_type
...
def _generate_combinational_driver_cpp(spec: DesignSpec, *, vcd_stem: str) -> str:
    ...
    for p in spec.ports:
        lines.append(f"    sc_signal<{cpp_type(p)}> sig_{p.name};")
