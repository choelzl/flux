...
What a harness is **told** and what it **reports** is `flux-codegen-harness-spec`
(docs/decisions.md D453): `DesignSpec`/`Port`/`TestVector`, the netlist a composite is wired
from, and `HarnessRunResult`. This package depends on and re-exports them, so every caller is
unchanged. Until D453 those types came from `flux_codegen_systemc_harness` — the Verilog harness
depending on the C++ one for its own spec — and the two composition parsers, written as mirrors,
had each gained a fix the other lacked (see D453: a declared 16-bit top port emitted 32 bits
here).

What is Verilog's and lives here: `driver_gen.py` (SystemVerilog testbench generation),
`build.py` (real Verilator invocation), `compose.py`'s named-port instantiation, `synth.py`/
`asap7.py` (real Yosys) and `keywords.py`'s Verilog reserved words.
...
workaround `evaluator/rtl/adapter.py` already found and documented — `-j 1`, not `-j 0`, combined
with `--timing`).
