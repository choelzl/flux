"""A deterministic GEMM wrapper matching `evaluator/rtl`'s own `mac_array.sv` schedule
...
def gemm_cycles(*, B: int, C: int, K: int, lanes: int) -> int:
    ...
    K-groups and `evaluator/rtl` refuses a ragged one outright, so a candidate with
