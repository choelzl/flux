...
def run_once(workers: int, workdir: Path, log) -> dict:
    import interconnect_dse as demo

    db = str(workdir / f"speedup-{workers}.db")
    log(f"# escalation_parallelism={workers}: cold store, {SCOPE['breadth']}/"
        f"{SCOPE['max_stages']}")
    demo.ESCALATION_WORKERS = workers  # the demo reads this when it steps the campaign
    t0 = time.time()
    demo.run_round(db, 1, SCOPE["max_stages"], SCOPE["breadth"], None)
    wall = time.time() - t0
...
def main() -> None:
    ...
    demo.check_toolchain()  # fail here, not thirty minutes in
