...
Every real evaluator adapter in this repo (`evaluator/zigzag`, `evaluator/timeloop`) requires
...
`evaluator/stream/README.md`'s "Not modelled at all" section already established for a different
...
(`interfaces/chia_nodes/`, including the new `result_db_path`/`n_samples` parameters), and its MCP tool
(`interfaces/mcp/`) — `corpus_root` (a local filesystem override, mainly for tests) is deliberately the
...
**unmodified** evaluator — reusing the exact multi-op aggregation `evaluator/zigzag`/
`evaluator/timeloop` already proved (docs/decisions.md D59/D62), no evaluator-side changes needed
...
already, correctly, has no translation for `data_dependent` ops — but `evaluator/zigzag`'s own
