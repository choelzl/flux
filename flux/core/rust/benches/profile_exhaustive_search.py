...
assertions — this is a diagnostic script, run manually), which is why it lives in
`core/rust/benches/` rather than `tests/`.

Usage (from flux/, inside `nix develop`, which puts every local package on PYTHONPATH):
    python3 core/rust/benches/profile_exhaustive_search.py
...
# core/rust/benches/<this> -> flux. `parents[2]` is `core`, and every use below appends
# "core/..." to it, so the paths resolved to flux/core/core/... and did not exist. The
# reorganisation (D296) added a level here that this was never moved past (D335).
