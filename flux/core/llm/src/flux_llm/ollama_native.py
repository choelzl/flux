...
class NativeOllamaProposer:
    ...
    def propose(self, prompt: str, schema: dict | None = None) -> str:
        """`schema` (a JSON Schema) constrains DECODING to that shape (D413): the
        server can only emit conforming JSON, so "no header", "no fence" and
        truncated-structure failures become impossible rather than parsed around.
        Measured need: those formatting refusals cost the NLU campaign more rounds
        than every real design error combined. Without a schema this is unchanged."""
        ...
        with phase("llm: generating (model)", why=f"~{len(prompt) // 4} tok prompt",
                   ...
                   think=think, prompt=prompt) as out:
            with urllib.request.urlopen(req, timeout=self.timeout_s) as resp:
                payload = json.loads(resp.read())
            # Kept for the caller's provenance, and because `done_reason` is the field that
            # would have named this bug on day one: "length" means the model was cut off
            # mid-answer, which is a different failure from a model that answered badly.
            self.last_metadata = {
            ...
            out["tokens"] = (f"{payload.get('prompt_eval_count')} in, "
                             f"{payload.get('eval_count')} out, "
                             f"done_reason={payload.get('done_reason')}")
            out["reply"] = text
            if thinking.strip():
                out["thinking"] = thinking
            if not text.strip():
                if thinking.strip() and payload.get("done_reason") == "stop":
                    # The model FINISHED (stop, not length) yet left the answer channel
                    # empty: qwen habitually writes the entire reply -- final answer
                    # included -- inside the think channel and never exits it. The
                    # answer, if there is one, is at the end of that trace, and the
                    # callers all run parse gates that search a reply for their header,
                    # fence or JSON anyway. Hand the trace to those gates instead of
                    # discarding tens of minutes of generation (D410); a gate that finds
                    # nothing refuses exactly as it would have, with a better record.
