...
**v0.1 scope, same discipline as `evaluator/`:** only a **pure chain of MatMul/Gemm nodes** (an
...
against `evaluator/zigzag`'s bundled `tpu_like` default, which needs loop dims literally named
