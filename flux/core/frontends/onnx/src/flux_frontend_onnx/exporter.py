...
cost-model consumer (Stream; this repo's own `evaluator/*` adapters) reads only shapes/dtypes to
...
def _parse_gemm_op(op: dict[str, Any]) -> tuple[str, str, str]:
    ...
    expr = op.get("expr")
    parsed = parse_einsum(expr) if isinstance(expr, str) else None   # the IR's grammar (D467)
    if parsed is None or not parsed.two_dimensional or len(parsed.out) != 2:
        raise NotExpressibleError(
    ...
    if parsed.gemm() is None:
        raise NotExpressibleError(
            ...
            f"transposed output — found ({parsed.out[0]}, {parsed.out[1]})."
        )
    return batch_dim, reduction[0], output_dim
