...
Three panels, the standard DSE story, rendered from the record a run already keeps:
  ...
  results       the objective space: cost against quality, the frontier stepped through the
                report's stage, screened-to-confirmed connectors showing what the cheap stage's
...
CONFIRM_HUE = "#eb6834"         # categorical 2 (orange): confirmed stage + frontier
SURFACE = "#fcfcfb"
INK = "#0b0b0b"
INK_2 = "#52514e"
MUTED = "#898781"
    ...
    try:
        ...
        for t in store.trials(campaign_id):
            ...
            points.append(Point(quality=float(q.value), cost=float(c.value),
                                label=t.candidate_key or "", confirmed=(t.stage == "decide"),
                                phase=t.phase or ""))
