"""A search a model directs, one step at a time, from a menu the application registers (D446:
`flux_directed_search`, folded into the loop as the orchestrator's menu policy).
...
WHAT THE POLICY OWNS, because every long search needs it and none of it is domain knowledge:
...
`DirectedSearch.steps()` is a generator -- one `StepRecord` per action taken -- so a problem's
`search` hook drives it and yields the candidates each step produced to the loop; `run()` is
the same thing driven to the end.
    ...
    def allowed_values(self) -> dict[str, list[str]]:
        ...
        for variant in self.variants:
            for name, value in variant.items():
                out.setdefault(name, set()).add(str(value))
...
class DirectedSearch:
    ...
    def __init__(self, actions: list[Action], *, ask: Callable[[str], str] | None,
        ...
        self._log = log
        # SEEDED FROM WHAT THE STORE ALREADY HOLDS, not empty. Starting empty meant every run
        # re-enumerated all five families against a store that already contained them: 78% of
        # this repo's screen trials are cache hits, 94% over the most recent runs, and one such
        # round spent 2,448 seconds — 38% of a whole run — re-deriving 3,307 topologies to find
        # nothing. The evaluations were cached; the enumeration was not, and nothing above it
        # knew the work had been done. A search whose memory ends when the process does is not
        # resuming, it is repeating.
        self._done: list[str] = list(done or [])
        self.report = SearchReport(steps=[], unexplored=[], stopped_because="not started")

    _REPEAT_GRACE = 2  # consecutive already-done picks refunded before one costs a step
    ...
    def _menu(self) -> str:
        ...
        for action in self._actions.values():
            ...
            for param, allowed in sorted(action.allowed_values().items()):
                lines.append(f"       {param}: one of {', '.join(allowed)}")
    ...
    def _remaining_note(self) -> str:
        ...
        # The REPEATABLE actions, named. Only actions that declare a finite variant set can appear
        # in the list above, so a menu where one action enumerates families and the rest do not
        # showed "not yet done: <five families>" every single round, whatever else was available.
        # A model reading that steers to the one action the line mentions: over four rounds of a
        # real run it chose enumerate twice against an exhausted space and never once chose the
        # Monte-Carlo step, which had no declared variants and so was never listed as outstanding
        # (D314). Naming them does not recommend them; it stops the line from arguing for one
        # action by omission.
        repeatable = sorted(a.name for a in self._actions.values() if not a.variants)
        always = (f" Always available, and never 'done': {', '.join(repeatable)}."
                  if repeatable else "")
        if not keys:
            return ("every enumerable option has been taken, so enumerating again will find "
                    f"nothing new.{always}")
    ...
    def steps(self) -> Iterator[StepRecord]:
        """The policy, one taken step at a time: plan, run the action, record it, yield the
        record; stop on the model's word, the cap, or `barren_limit` steps that found nothing.
        `self.report` is the report so far and, after the last step, the whole of it."""
        ...
        while step < self._cap:
            ...
            if key is not None and key in self._done:
                # A repeat costs a model call and used to cost a STEP too, so a model that
                # re-picked the same perturbation spent one of its budgeted steps discovering
                # that. Observed twice in a fourteen-step run -- 14% of the study, on nothing.
                # The step is refunded, but only `_REPEAT_GRACE` times in a row: a model looping
                # on one choice must not be able to spin the search forever.
                repeats += 1
                self._log(f"    {key} already done, moving on"
                          + ("" if repeats <= self._REPEAT_GRACE else " (out of retries)"))
                if repeats > self._REPEAT_GRACE:
                    step += 1
                    repeats = 0
                continue
            repeats = 0
            ...
            # A refusal that explains itself, out loud. An action that declines -- no near-miss to
            # repair, nothing feasible to anneal from, a fabric that is not in this store -- put
            # its reason in the step record and printed nothing, so a reader saw the orchestrator
            # choose an action and then saw the run move on with no sign anything had happened.
            # Indistinguishable from a step that ran and found nothing.
            # `progressed is False` -- an EXPLICIT refusal -- not `was_productive`. Those are
            # different things and conflating them mislabelled real work: an annealing step that
            # screened 19,200 proposals and placed four of them gained no fabric the store had
            # never seen, so `was_productive` was False, and it was reported as "declined: 19,200
            # screened proposals -> 4 placed". A step that ran and found nothing new is not a
            # step that refused to run.
            if outcome.progressed is False and outcome.detail:
                self._log(f"    declined: {outcome.detail}")
            if key is not None:
                self._done.append(key)
            record = StepRecord(step, name, dict(plan), reason, outcome.gained, outcome.detail)
    ...
    def run(self) -> SearchReport:
        """`steps()`, driven to the end."""
        for _record in self.steps():
            pass
        return self.report
