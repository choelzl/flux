...
NODES: dict[str, Node] = {n.name: n for n in (
    ...
    Node("DSE", "orchestrator", False, "the drawing's DSE box: a batch of candidates the search policy proposes"),
    ...
    Node("plan", "orchestrator", True, "the loop plan: parts and order, budgets, roles"),
    Node("pipeline", "evaluator", False, "the register sweep: each depth transpiled, built and measured"),
    Node("screen", "evaluator", False, "a part measured alone on the shallow stage"),
    Node("sub-loop", "orchestrator", False, "a child loop run as one part"),
)}
