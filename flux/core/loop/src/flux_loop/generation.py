...
def _generate_with_model(problem: Problem, subgoal: str | None, method: str,
    ...
    help_text = COMPUTE_HELP if req.compute else ""
    ...
    def wrap(pair: tuple[str, dict | None]) -> tuple[str, dict | None]:
        ...
        body, schema = pair
        return (_compose(prefix, _compute_block(state, key), proto_block, body, help_text),
                _with_compute(schema) if req.compute else schema)
    ...
    grad = Gradient(req.regress_after)      # D423: the fast check's trend, and the revert
    ...
    for attempt in range(attempts + 1):
        if mode == "patch" and cand is not None and req.patching:
            ...
            with _phase(f"generate: {tag}", why=f"attempt {attempt + 1}",
                ...
                _take_compute(state, key, reply, tag)
