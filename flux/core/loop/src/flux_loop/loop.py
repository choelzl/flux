...
def run_loop(problem: Problem, request: LoopRequest, *, proposer: Any | None = None,
    ...
    if request.db:
        ...
        if objs and state.records is not None:
            ...
            # from the record and nothing else (the latest row stands)
            state.records.remember("objectives", {"objectives": [o.to_doc() for o in objs]})
        if state.records is not None:
            # D562: what earlier passes measured between the stages reaches the problem before
            # a step is spent -- the margin on a shallow stage is the record's, not a guess
        ...
        if state.depth == 0 and getattr(state.records, "campaign_id", None):
            ops.register(state.records.campaign_id, state.workdir)     # D513: `flux status/stop` see this run
        try:
    ...
    # D505: the AGENT takes the halves the request names -- the orchestrator's decisions
    # (`agent` fills the role a problem left to its own hooks), and the plan the pass follows
    # (a document, hand-written or the agent's, applied before anything is divided)
    ...
    if request.ahead and problem.stages() and problem.subgoals() and lad is not None and getattr(lad, "alone", None):
        # D563: the tools work while the model thinks -- only where the ladder DECLARES the stage a
        # part is measured on alone (the NLU's placement); a problem whose parts are never
        # measured alone would pay a tool run for nothing
    ...
    try:
        while step < request.steps:
            ...
            if kind == "improve":
                ...
                # The drawing's DSE box, whole (D546, D547): the policy's proposal AND the
                # batch through the gate and the first stage. The proposal is the world's
                # generator running -- the macarray verifies 96 designs on Verilator there --
                # and outside a phase that time showed as "unattributed".
        ...
        if (not state.improved and not state.pool and not todo and not live
            ...
            # D506: every design sent back STOOD and nothing was admitted -- the pass changed
            # nothing, and the next one would not either (live: 73 passes in four hours, each
            # re-verifying seven parts and authoring vectors to decide the same "stand").
            # D518: or NOTHING was sent back at all -- every part meets the goal alone and the
            # whole still misses it (live: nine passes in twenty-five minutes, each deciding
            # the same 788 MHz); a pass that changes nothing is a rest either way
            state.stopped = (("at rest: every ladder is spent (" + ", ".join(dict.fromkeys(state.rested)) + ")")
                             if state.rested else "at rest: nothing was due on any part") + "; nothing changes until the ask does"
...
def _improve_step(problem: Problem, state: LoopState, item: Improve) -> list[Scored]:
    ...
    if cand is None and len(state.rested) > rested_before:
        # D509: the design STANDS -- a rest on the ledger, never a refusal (the record held
        # 462 "gate refused" rows that said "nothing on the ladder is due")
...
def _forget_stale_compositions(state: LoopState) -> None:
    """A part changed: every composition measured so far was of the OLD set (D506, live: the
    confirm stage kept measuring a superseded composition -- 91 MHz, the unpipelined one --
    beside the new part at 636, and routed the part back with the stale number). They leave
    the pass's pool; the record keeps them as history."""
...
def _pick(problem: Problem, state: LoopState, todo: list, goals: list[str], human: str | None,
    ...
    """The orchestrator's pick for the next part (D569: one of three pieces of a part step --
    pick, draft, admit). Plans it, briefs it once, says it; runs a sub-loop pick itself and
    returns None. `taken` are parts another draft of this step already holds."""
    request = state.request
    say = state.say
...
def _parts_step(problem: Problem, state: LoopState, todo: list, goals: list[str], n: int) -> int:
    """Up to `n` parts DRAFTED at once (D569, review item 2): the orchestrator picks them one
    after another, their generation boxes run on worker threads -- the model's turns overlap
    each other and the tools -- and each is admitted in the order it was picked, on this
    thread, where the record, the cache and the state's lists have their one writer. Returns
    how many parts were worked, each a step of the budget."""
