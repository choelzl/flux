...
@dataclass
class Option:
    """ONE thing the loop could do next for a design an evaluator sent back (D505): a step
    of a problem's improve ladder, as data. `why` is the line an orchestrator reads (what it
    does, what it costs, what the numbers say); `due` is the code's own opinion (the rules
    take the first due option, in order); `run` performs it and returns what `Problem.improve`
    returns. The menu and the dispatch cannot disagree, and a model that picks (the agent
    orchestrator) picks from the same list the rules walk."""
...
class Verdict:
    ...
    seconds: float = 0.0        # D510: what the judgement cost, for the row
...
class LoopRequest:
    ...
    passes: int = 1                 # D551: passes the DOCUMENT asks for when the CLI says nothing; 0 = until the
                                    # campaign is at rest or `flux stop` asks (the macarray invents until stopped)
    ...
    max_tolerance: int = 4          # D504: the most regressions in a row the tolerance can grow to
    ...
    prototype_patience: int = 8     # D506: attempts a pass is granted after each new best, past the budget
    prototype_attempts_max: int = 90   # D506: the most a pass may grow to, however well it goes
    compute_timeout_s: float = 10.0
    tools: bool = True              # D505: the model CALLS tools inside a turn (compute, check, ...); the floor is a server with tool calls
    tool_hops: int = 6              # rounds of calls a turn may make before it must answer
    hop_share: float = 0.5          # D544: a round that may call tools may write this SHARE of the model's
                                    # context window (D506 fixed it at 24,000 tokens); 0 = the turn's cap
    knowledge_share: float = 0.5    # D548: the share of the model's context window the static knowledge (sheet,
                                    # library, mined) may hold; no window stated = no bound. Never a prompt budget
    compact: str = "rules"         # D549/D550: how what is over the window is compacted: "rules" (knowledge: repeats
                                    # dropped, the paragraphs nearest the part kept; a turn: results digested) or
                                    # "llm" (the model condenses a source once per run / writes what a turn's older
                                    # rounds established)
    compact_share: float = 0.6      # D549: a tool turn's conversation past this share of the window has its
                                    # older rounds compacted (thinking dropped, results cut to a head)
    tool_result_chars: int = 40000  # D543: what the model reads of a tool's result before it is cut (was 6000 in
                                    # the runner and 4000 in the compute tool: the check's report was cut mid-table
                                    # and the model said so instead of designing)
    agent: tuple[str, ...] = ()     # D505: the halves the agent takes: "tools", "orchestrate", "plan"
    plan_file: str | None = None    # D505: a loop plan document (hand-written, or the agent's, kept)
    ...
    calibrate: bool = True          # D542: the calibrate node between stages; `flow: {calibrate: off}` turns it off
    workers: int = 0                # D525: tool runs at once (a stage's candidates, a sweep's points); 0 = the box decides
    parallel_parts: int = 1         # D569: parts DRAFTED at once (the model's turn and the gate on worker threads);
                                    # admission, the record's row and the state stay on the loop's thread
    ahead: bool = True              # D563: an admitted part is measured alone on a worker thread while the model
                                    # writes the next part; `budget: {ahead: false}` keeps the tools in line
...
@dataclass
class PartState:
    """What the loop knows about ONE part beyond its admitted design (D509): its numbers
    measured alone, its depth proxies, and the improve ladder's counters. Was seven
    `state._x` dicts keyed by part that every reader guarded with `getattr`."""

    alone: dict[str, float] = field(default_factory=dict)   # its numbers measured alone on the deepest stage (D498)
    depth_by_digest: dict[str, Any] = field(default_factory=dict)   # prototype digest -> the logic-depth proxy (D496)
    redesigns: int = 0                     # different algorithms asked for this run (D499)
    redesign: str | None = None            # the note asking for one, while its pass runs
    optimise: dict[str, Any] | None = None  # {"depth": d0, "target": d} while a depth pass runs (D496)
    taken_from: str | None = None          # the record row's digest a taken design came from (D504)
    proto_passes: int = 0                  # prototype passes this run (the trace directory's number)
    timing: dict[str, Any] | None = None   # D526: the placed critical path of its design alone, as data
    shortlist: list[dict[str, Any]] = field(default_factory=list)   # D528: its admitted designs on record, best first
...
class LoopState:
    ...
    `attempts` and `prototype.py` keeps `prototypes`; `drain()` extends `human_notes`. A problem reads; the one thing it may add is a lesson."""
    ...
    plan: dict[str, Any] = field(default_factory=dict)   # D505: the LOOP PLAN this pass follows (applied)
    critiqued: dict[str, int] = field(default_factory=dict)     # part -> times sent back (D433)
    judged: int = 0
    step: int = 0
    stopped: str = ""                                    # D463: why the step loop stopped
    parts: dict[str, PartState] = field(default_factory=dict)   # D509: per-part numbers and counters
    versions: dict[str, str] = field(default_factory=dict)     # D510: the problem's transpiler/judge versions
    ...
    rested: list[str] = field(default_factory=list)      # D506: parts whose improve step chose to STAND
    sent_back: int = 0                                   # D518: designs the stages routed back this pass
    last_prompt_sha: str | None = None                   # D535: the last model turn's prompt, by digest, for the rows it makes
    improved: int = 0                                    # D506: improve steps that produced a design
    ...
    compositions: set = field(default_factory=set)   # D506: keys of the composed designs measured this pass
    ...
    on_stage: dict[str, Any] = field(default_factory=dict)      # own bookkeeping, read by the
                                                               # decision
    started: float = 0.0
    ahead: Any = None          # D563: `flux_loop.measure.Ahead`, the tools working while the model thinks
    tool_runs: int = 0         # D567: measurements the tools ran this pass, and what the cache served
    prompt_sha_by_thread: dict = field(default_factory=dict)   # D569: the last prompt's digest, per drafting thread
    cache_hits: int = 0
    ...
    def ledger(self) -> "Ledger":
        """The campaign's ledger (D509), over this run's record."""
...
class LoopResult:
    ...
    stopped: str = ""
    #: D506: the pass changed nothing and every design sent back stood -- every ladder is spent.
    #: A caller that loops passes (the TUI's `r loop`) stops here rather than spinning.
    at_rest: bool = False
