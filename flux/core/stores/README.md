...
([decisions.md D11](../../docs/decisions.md)): `interfaces/chia_nodes/`'s `flux_get_result`/
...
`interfaces/mcp/`'s `FluxTool`. `CorpusEntry` gained a real `.to_dict()` method for this (it didn't
have one before D11).
...
`interfaces/chia_nodes` — `flux_evaluate` and `flux_search` both take an optional `result_db_path`
that wraps their evaluator(s) in `CachingEvaluator` — and reachable over MCP via `interfaces/mcp/`'s
...
`interfaces/chia_nodes`/MCP surface, `flux_leaderboard`, holdout-safe by construction like
