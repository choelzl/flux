"""Content-addressed result/artifact store (docs/stores.md): every design point records inputs,
evaluator, and full lineage; deterministic replay is one query away. SQLite-backed for local use
— Postgres+S3 for shared deployments is future work, same interface (docs/stores.md).
"""
...
_VALID_KINDS = ("workload", "architecture", "mapping", "objective", "composition", "digest")   # digest: D576
...
class ResultStore:
    ...
    def __init__(self, db_path: str | Path) -> None:
        ...
        # D569: the connection may be used from any thread (a part drafted on a worker thread
        # records its prototype there), one statement at a time, every result fetched whole
    ...
    def documents(self, kind: str) -> list[dict[str, Any]]:
        """Every stored document of `kind`, oldest first (D576: the library's digests)."""
    ...
    def find_results(
        ...
        *,
        ...
        'do not filter on this'). This is the warm-start query surface docs/search.md describes
        ("every strategy can seed from prior runs") — see `flux_store.CachingEvaluator` for the
        Evaluator ABI wrapper built on top of it.

        `evaluator` matches exactly (e.g. `"zigzag@3.8.5"`); `evaluator_prefix` matches a leading
        substring (e.g. `"zigzag@"`, tolerant of adapter version drift) — pass at most one, since
        they'd otherwise silently AND together in a way that's easy to misread as an OR.
        """
