...
class CampaignStore:
    ...
    def begin_trial(
        ...
        *,
        ...
        seq = int(row[0])
        self._conn.execute(
            "INSERT INTO trials (campaign_id, seq, phase, stage, stage_index, candidate_json, "
            ...
            (
                ...
                campaign_id, seq, phase, stage, stage_index,
    ...
    def trials(self, campaign_id: str, *, phase: str | None = None,
        ...
        rows = self._conn.execute(
            "SELECT seq, phase, stage, stage_index, candidate_json, candidate_key, workload_hash, "
        ...
        for r in rows:
            ...
            out.append(Trial(
                seq=r[0], phase=r[1], stage=r[2], stage_index=r[3],
    ...
    def already_escalated(self, campaign_id: str, candidate_key: str, stage_index: int) -> bool:
        row = self._conn.execute(
            "SELECT 1 FROM trials WHERE campaign_id = ? AND candidate_key = ? "
            "AND phase = 'escalate' AND stage_index = ? AND status != 'interrupted' LIMIT 1",
            (campaign_id, candidate_key, stage_index),
        ).fetchone()
