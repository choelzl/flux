...
def task_rows(snap: dict[str, Any], cursor: int | None = None,
    ...
    if detail_rows is not None:
        off = max(0, min(detail_scroll, max(0, len(detail) - detail_rows)))
        if clamp is not None:
            clamp["dscroll"] = off                         # D487: no scrolling into the void
        window = detail[off:off + detail_rows]
        if off > 0:
            window[0] = f"↑ {off} more lines above"
        if off + detail_rows < len(detail):
            window[-1] = f"↓ {len(detail) - off - detail_rows} more lines below"
        for ln in window:
            put(ln)
    else:
        for ln in detail:
...
def standings_lines(st: dict[str, Any]) -> tuple[list[str], list[str | None]]:
    ...
    for key, label in (("goal", "OBJECTIVE"), ("now", "NOW"), ("composed", "WHOLE")):
        if obj.get(key):
...
def results_browse(snap: dict[str, Any], cursor: int | None = None, *,
    ...
    if detail_rows is not None:
        off = max(0, min(detail_scroll, max(0, len(body) - detail_rows)))
        if clamp is not None:
            clamp["dscroll"] = off
        window = body[off:off + detail_rows]
        if off > 0:
            window[0] = f"↑ {off} more lines above"
        if off + detail_rows < len(body):
            window[-1] = f"↓ {len(body) - off - detail_rows} more lines below"
        body = window
    for ln in body:
        lines.append(ln[:width]); ids.append(None); roles.append(None)
    return lines, ids, order, roles
