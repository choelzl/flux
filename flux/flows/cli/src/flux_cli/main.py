"""Flux CLI entry point (docs/05.md Phase 1: `flux eval`, `flux import`, `flux replay`).

Hand-written argparse, not generated from a shared `@flux_tool` decorator: doc 04 §7.2's "one
definition, three surfaces" (typed function / CHIA node / MCP tool) is `flows/`'s eventual shape,
not this CLI's current one — `flows/chia_nodes/` and `flows/mcp/` are both still empty. This is a
real, usable stepping stone, not that end state.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="flux", description="Flux CLI (docs/05.md Phase 1).")
