...
this repo (`ir/workload/examples/`, `ir/architecture/examples/`, `interfaces/chia_nodes/src/
flux_chia_nodes/*.py`, `interfaces/mcp/src/flux_mcp/tool.py`) — none of it is illustrative
...
## 8. Run a task document through the design loop (no code)

The design loop (`core/loop`, [decisions.md D421](decisions.md)) takes a task as a document:
statement, contract, parts (or `"parts": "decompose"` to let the model divide it, and `"brief":
`"propose"` to have it brief each part, `"critique": "propose"` to have it judge the division, each
admitted candidate and the decision), a gate (build
and test commands), costed stages, objectives, a budget ([D430](decisions.md), [D431](decisions.md)).

```bash
flux task check flux/core/loop/examples/digits.task.json     # validate; list the tools it needs
flux task run   flux/core/loop/examples/digits.task.json --db digits.db --out digits.txt
```

`flux task check` also refuses a document that asks for what it cannot measure -- an objective
naming a metric no stage produces, a cutoff on a metric its own stage does not measure
([D463](decisions.md)) -- before a run spends anything on it.

`run` prints the standard report (decision, frontier, established / not established, refusals,
notes) and writes the decided artifact. Without a model installed, `--replies replies.json`
feeds scripted replies (a JSON list) through the same loop -- what the tests and demos use. The
gate's `test` command prints its failures and the document's `count_re` (or `fail_re`) says how
they are counted; a stage is a `command` whose output carries metrics (`metrics_re`), or an
`evaluator` by registry name (`flux_evaluator_abi.available_evaluators()`), and may declare a
`cutoff` -- `{"metric": "fmax_mhz", "at": 600}` (a floor), `"below"` (a budget) or `"within":
0.9` (a band around this run's own best) -- so designs that cannot be the answer stop before the
costlier stage ([D454](decisions.md)). A task that is not one artifact divides into `subtasks`
instead of `parts`: each is a task document in its own right, inherits what it does not say
(contract, gate, stages, objectives, knowledge, params), runs as its own loop and hands its
decision back to the parent to compose; `"subtasks": "decompose"` lets the orchestrator name
them, up to `max_subtasks` ([D455](decisions.md)). WHO FILLS EACH ROLE is a field as well
([D460](decisions.md)): `"roles": {"orchestrator": "rules"}` has code decide the next step,
`{"orchestrator": {"given": {"parts": [...]}}}` takes the division from the document's author,
`{"orchestrator": "llm"}` says out loud that a model decides. The other two roles have an AI
half too: `{"evaluator": {"learned": {"metric": "bytes", "within": 0.9}}}` adds a screening stage
fitted on what this campaign already measured (it joins the chain only once the record can
support it, is tagged as modelled, and never becomes the last stage), and
`{"knowledge": "mined"}` adds what was extracted from this project's own data beside what the
document's `knowledge` field says. `flux task run --role
orchestrator=rules` switches one role of a document you did not write, and `flux task check`
lists what each role can be switched to. Who DRAFTS is a field too
([D456](decisions.md)): `"generator": {"command": [...]}` makes that command the generator (it
writes `{artifact}` and is told `{failure}` and `{attempt}`) and `"generator": {"catalog":
[path, ...]}` points at designs that already exist, so the whole loop -- gate, stages, cutoffs,
frontier, decision -- runs with no model in it at all. Commands may use
`{artifact}`, `{workdir}`, `{name}`, `{part}` and `{python}`. In Python the same is
`flux_loop.PromptProblem(flux_loop.load_task(path))` handed to `flux_loop.run_loop`; subclass
`Problem` when the task needs what a document cannot say (an exhaustive reference, a table
oracle, a prototype stage) -- see the site's "Build your own loop".
