...
the same discipline [landscape.md](history/landscape.md) uses for prior-art claims. Reference material —
doesn't change as this repo evolves, so (like landscape.md) it isn't kept in sync with current
build status the way [roadmap.md](history/roadmap.md) is.
...
[landscape.md](history/landscape.md)) for architecture search and fast PPA prediction.
...
- **Currently built vs. actually aimed at — not the same thing, and worth being precise about.**
  ...
  `evaluator/rtl`/`evaluator/systemc` *consume* RTL as ground truth for conformance checking,
  ...
  ([decisions.md D91](decisions.md), [roadmap.md](history/roadmap.md)'s Phase 3.5 — an LLM proposes a
  ...
  iteration loop** the same way, via `evaluator/hammer/` — a real, open, BSD-3-Clause PDK (ASAP7)
...
- **A seed of the "unit test" stage already exists, ahead of the rest of the flow.**
  `evaluator/rtl`/`evaluator/systemc` already self-check each candidate's own RTL/SystemC model
...
- **CHIA is the closest parallel, and it's a dependency, not a competitor** — and it's a bigger
  ...
  orchestration") — `interfaces/chia_nodes/` ships Flux's evaluator contract, search strategies, and
...
- **No AI/ML is used inside Flux's own cost models** — ZigZag/Timeloop/RTL-sim/SystemC/Booksim2
  ...
  future adapter. Not currently on [roadmap.md](history/roadmap.md)'s "Immediate next actions" list — a
