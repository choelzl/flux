...
| # | Gap | Status | Evidence |
|---|---|---|---|
| G1 | No evaluator contract; every framework is a monolith | **Closed** | `ir/` + `evaluator/abi/` (Phase 1) |
| G2 | No fidelity chain, no calibration, no uncertainty | **Closed** | `calibration/`, `validity/` ([decisions.md D10](decisions.md)) |
| G3 | Missing fast/PPA-aware fidelity stage for CHIA | **Closed** | `interfaces/chia_nodes/`, `interfaces/mcp/` |
...
- **G3.** CHIA's own evaluator cascade jumps from cheap-but-blind (ChampSim) straight to
  ...
  millisecond-scale, PPA-aware stage exists in between. Fix: insert analytical tensor-accelerator
...
- **G5.** Every tool in the field was designed around dense, static-shape, affine CNN layers —
  ...
  way, not just a missing feature: `evaluator/zigzag`'s own `workload_to_zigzag_layers` silently
...
- **G6.** NoC routing/congestion, chiplet/D2D links, DRAM bank/refresh behavior, and thermal were
  ...
  NoC (`evaluator/booksim`/`evaluator/noxim`, D6/D32) and thermal
  (`evaluator/thermal`, [D64](decisions.md)/[D65](decisions.md): real steady-state 3D-ICE
  ...
  now** ([D66](decisions.md)/[D67](decisions.md)): `evaluator/booksim` gained a second,
  ...
  named item of this gap: `evaluator/dramsim3` clones and builds real DRAMsim3 (University of
...
- **G9.** Nothing in the field is incremental (change one architecture parameter, re-evaluate
  ...
  (`evaluator/native/`'s `NativeEvaluator`, registered as `"native"` in `flux_cli.registry`, no
  ...
  reduction — built for an unrelated reason (giving `evaluator/cacti` a single-node document it
...
- **G13.** No agreed set of (workload, architecture-family, objective, constraint) problems exists
  ...
  two-layer feedforward block (checked against `evaluator/zigzag`'s translator source first — no
...
- **G16.** No DSE tool treats compute/token budget as a search constraint, despite CHIA reporting
  ...
  applies to the *escalation* cascade only — a real, naturally sequential stage-by-stage cascade
  ...
  a 12s budget lets the ~10s SystemC stage complete and cuts off before the ~10s RTL stage.
