...
`evaluator/stream` translates a fusion-only mapping document (`group` + a single-dim `tile`)
...
upstream (`stores/`, see [stores.md](stores.md)) — real today, used throughout `interfaces/chia_nodes/`
...
use), `backends` (screening + escalation stages by registry name), the `search` space (whose kinds
