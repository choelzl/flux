...
tool surveyed in [landscape.md](history/landscape.md) these are entangled together.
...
- **Explicit escape hatch** (`data_dependent`) with an attached *distribution* rather than a fixed
  ...
  pretending they're static ([gap-analysis.md](history/gap-analysis.md) G5). Consumed for real since
  [decisions.md D68](decisions.md): MoE `data_dependent` routing resolves which `top_k` of a
  declared candidate expert set actually ran, through `core/workload_dynamism/`'s sample-and-aggregate
...
Currently real: the einsum/affine core, used by every evaluator adapter, plus the
dynamic-shape/`data_dependent`/distribution machinery above ([decisions.md D63](decisions.md)/
[D68](decisions.md)/[D87](decisions.md), `core/workload_dynamism/`). The ONNX frontend that
...
- **Component classes + attributes + actions**, exactly the Accelergy pattern (see
  [landscape.md](history/landscape.md)), generalized beyond energy so the same plug-in mechanism could
...
- **Constraints are part of the architecture document**, machine-checkable and independent of the
  cost model — a direct anti-reward-hacking measure ([gap-analysis.md](history/gap-analysis.md) G14): the
...
- Thermal and NoC have declared schema slots, and both are real now: thermal via
  `evaluator/thermal` (3D-ICE, single- and multi-die, [decisions.md D64](decisions.md)/
  [D65](decisions.md)), NoC via `evaluator/booksim` (a real adapter, not a built-in — 2D/3D mesh
...
2-D arrays ([decisions.md D215](decisions.md)), and `evaluator/stream` consumes the
...
(`mapping_translator.py` in each) and nothing searches it -- the loop's worlds measure their own
...
upstream (`core/stores/`, see [stores.md](stores.md)) — real today, used throughout `interfaces/chia_nodes/`
...
document's `campaign:` ([D524](decisions.md)); see [usage-guide.md](usage-guide.md).
