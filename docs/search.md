...
| `search/architecture/` | Compute array width, NoC topology/dimensionality, memory-hierarchy size, joint width×memory size ([D26](decisions.md)), fusion tile — the first mapping-space axis ([D104](decisions.md)) — five axes | Real. Screens candidates with a fast evaluator, ranks, escalates the winner through the fidelity chain (`evaluator/systemc`, `evaluator/rtl`). CHIA-agnostic — the Evaluator ABI is the only interface it knows about. **Real wall-clock budget for the escalation cascade** ([decisions.md D71](decisions.md), following D69/D70's own precedent) — screening's own batched/parallel dispatch isn't interruptible, but escalation's real sequential stage-by-stage cascade through increasingly expensive simulators (coarse SystemC, then cycle-accurate RTL) is exactly where a real budget matters most; a caller declaring `wall_clock_budget_s` gets whichever stages completed within it, flagged via `stopped_early`. Threaded all the way to the real `flux_search` CHIA node/MCP tool, not just the library function. |
...
| `search/campaign/` | Long-horizon campaigns over a declared Objective IR document | Real ([decisions.md D216](decisions.md)–[D222](decisions.md)): grid, agentic and generative ([D233](decisions.md) — the LLM writes whole Architecture IR documents) strategies over width, memory-size, joint, NoC-topology and composition (`composition_width`, per-op engines, [D236](decisions.md)–[D238](decisions.md), with per-op width lists [D241](decisions.md)) spaces. Campaign state lives in the ResultStore's own SQLite file — the database is the checkpoint, SIGKILL-verified resume ([D217](decisions.md)); screening is calibrated ([D222](decisions.md)) and batchable ([D238](decisions.md)); escalation stages compose mixed-fidelity frontiers ([D226](decisions.md)). |

**One driver, and it is the loop** ([decisions.md D459](decisions.md)). `while not
strategy.done(): propose -> evaluate -> observe` is `flux_loop`'s batch path with a batch of
one: `flux_search_exhaustive.engine.StrategyProblem` proposes through the strategy, measures by
running the evaluator, and hands the outcome back to `observe`. Exhaustive, annealing and all
five agentic axes drive through it, so the wall-clock budget, the refusal posture (a candidate
the evaluator refuses reaches `observe` as the exception) and the per-candidate checkpoint are
one implementation. What the strategies gained: a campaign record -- pass `db_path` and every
evaluated candidate lands as a trial on the metric's stage, every refusal as a refused trial
carrying its reason. What they deliberately do NOT get is the loop's measurement cache: a
cached hit is a candidate the strategy never observed.

CP/MIP, gradient, Bayesian and evolutionary strategies are not started (no directory until one is real, D428)
...
Separately, building the NoC axis found a real `evaluator/booksim` bug (`torus` crashed
...
`evaluator/timeloop`'s translator no longer rejects spatial constraints outright — it forces its
...
`evaluator/timeloop` also reads memory-hierarchy `size_kb` generically, so conformance is real
...
`evaluator/noxim` (D32), a second, genuinely independent NoC simulator (SystemC-based, different
...
verdicts at face value (see `evaluator/noxim/README.md`).
