...
routing, no congestion, no topology-aware placement (unlike this repo's `evaluator/booksim`).
...
- **Loop** — a Python program forming a directed cyclic graph. **Nodes**:
  ...
  fully-agent-driven, tunable per node — the pattern this repo's `interfaces/chia_nodes/` and
  `interfaces/mcp/` are built directly on.
...
1. **The bottleneck moved to evaluation and verification** — parallelism alone is insufficient
   ...
   (accurate, slow); the missing stage — fast, PPA-aware, calibrated tensor-accelerator
