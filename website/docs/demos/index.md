...
- **A hosted model is opt-in.** `FLUX_LLM_REMOTE=1` with `FLUX_REMOTE_BASE_URL`,
  ...
  server (LocalAI, llama.cpp, OpenRouter) instead; the first prompt that leaves the machine is
  announced, and a missing key falls back to the local model out loud. Thinking is decided per
  request (`reasoning_effort: "none"` when a role wants the answer straight; nothing sent when it
  wants the model to think, so the server's own default stands), and a JSON schema is only sent
  on non-thinking requests.
