...
```bash
nix develop .#physical --command flux task run applications/nlu/nlu.problem.yaml --db demo-nlu.db --tui
nix develop --command flux task run applications/nlu/nlu.problem.yaml --db demo-nlu.db --screen-only
```
...
-- other operators, a 2-ULP budget, a 1 GHz clock -- is a copy of the document, not a program.
