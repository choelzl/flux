...
costs, fast stages order but never conclude, every model role behind a gate — and the
...
(function () {
  ...
  function check() {
    ...
    if (have("eval_fast") && !(have("eval_slow") || have("eval_phys"))) notes.push("analytical without a slow stage: fast numbers ORDER, they must never be quoted");
    if (have("calibrate") && !(have("eval_fast") && have("eval_slow"))) notes.push("calibrate bridges the stages: slow-stage residuals widen the fast stage's intervals, so it needs both");
    if ((have("eval_slow") || have("eval_phys")) && have("frontier") && !(wired("frontier","eval_slow") || wired("frontier","eval_phys"))) notes.push("the slow stage should take its finalists FROM the frontier");
    ...
    verdictEl.textContent = notes.length
      ...
      : "✓ shape holds: gate → fast stage → frontier → slow stage → decide → output, flywheel wired";
  ...
  function skeleton(notes) {
    ...
    if (have("eval_fast")) L.push("    scored = eval_fast(artifacts or pool)           # analytical stage ORDERS, never quoted");
    if (have("calibrate")) L.push("    scored = widen_intervals(scored, residuals(records))  # calibrate: the slow stage teaches the fast one its error");
  ...
  document.getElementById("preset").onclick = () => {
    ...
    // then the evaluation chain left to right into decide -> output.
