...
exercises every mechanism above. The first real client is
[`applications/nlu/lib/src/flux_nlu/problem.py`](https://github.com/choelzl/flux/tree/main/flux/applications/nlu/lib/src/flux_nlu/problem.py)
(FP16 operators proven by exhaustion, a table oracle, yosys/OpenROAD stages). Then:

1. **Say the problem as a `Problem` subclass** whose constructor holds its settings;
   the loop's own knobs (steps, repair attempts, patching, cooldowns, finalists) are
   `LoopRequest`'s and every demo flag maps onto one or the other.
...
EVERY application in the repository runs on this contract -- macarray, bankmap,
interconnect_mapping, prefetcher and interconnect in batches (D446), NLU and a task
document part by part, and omni as rounds of tool steps (D458); the
...
| the decision arithmetic: corners, the knee, target-and-floor | `flux_frontier.decide` | `orchestrator/frontier` |
...
| a local model call that returns text (or schema-constrained JSON), reasoning off | `flux_llm.local_proposer` | `core/llm` |
