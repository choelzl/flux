...
applications/<name>/
  ...
  README.md                 the problem, the chain, the standings, the flags
...
**The DSE methods are orchestrators** (D465). Declare the space --
`space() -> {"width": [8, 16, 32], "banks": [2, 4]}`, in a meaningful order -- and
`rig(orchestrator="sweep")`, `{"montecarlo": {"samples": 20, "seed": 3}}` or
`{"anneal": {"metric": "served"}}` proposes the points; the annealer reads the numbers of the
...
line instead of four kinds of thing. Evaluation's AI half is
`rig(evaluator={"learned": {"metric": "fmax_mhz", "within": 0.8}})`: a stage fitted on what this
campaign has already measured, which orders candidates before the real tools run and **can
never be the last stage** -- a predicted number is not an answer. Knowledge's is
...
Where each application sits against the contract -- all EIGHT entries run on it, the five older studies since D446 and omni since D458:
...
| interconnect | search | the model-directed action menu (`flux_loop.actions`) | objective document -> generative strategy | structural screen, routability | composed estimate -> bias correction -> whole-fabric placement |
